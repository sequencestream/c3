...
- scripts/e2e/e2e-guide.md: E2E tests, make sure e2e pass if relative paths are changed. E2E always run against an isolated config (`node scripts/e2e/isolated-server.mjs` — `--settings <temp>` + temp `C3_DB_PATH`) and never write the real `~/.c3/settings.json`; see the constraint at the top of the guide.
