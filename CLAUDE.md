...
over a WebSocket to a Vue 3 frontend.
...
pnpm start --project /abs/path [--port 3000]   # = node server/dist/cli.cjs start
...
- **`server/src/`**: `cli.ts` (commander entry); `server.ts` (Hono `/ws` upgrade, serves
  embedded frontend in prod, aborts in-flight run on each `user_prompt`); `claude.ts` wraps
  `query()` — `canUseTool` sends a `permission_request`, awaits a Promise in
  `pendingApprovals: Map<requestId, resolver>` (**60s timeout auto-denies**).
- **`web/src/`** — Vue 3: `lib/ws.ts` (WS client), `App.vue` (chat + permission dialog).
  Vite proxies `/ws` → `:3000`.

## Invariants & conventions

- **`settingSources: []`** — SDK ignores `~/.claude/settings.json`; c3 is sole permission
  authority. Don't remove. **`permissionMode: 'default'`** — `canUseTool` fires only for
  sensitive ops (Write/Edit/dangerous Bash); read-only auto-allowed.
