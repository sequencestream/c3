...
CREATE TABLE IF NOT EXISTS intents (
  ...
  review_session_id   TEXT,                        -- 当前或最近一次 Review 会话 c3SessionId; 与 spec_review_session_id (spec 只读审核) 语义不同 (v24→v25 新增; 历史行为 NULL)
  review_status       TEXT CHECK(review_status IN ('pending','approved','rejected')),  -- PR AI 评审结论: NULL=未评审, pending=待结论, approved=通过, rejected=发现问题 (v24→v25 新增; 历史行为 NULL)
  review_fix_rounds   INTEGER NOT NULL DEFAULT 0,  -- 闭环复审轮次 (review→fix 往返次数) (v24→v25 新增; 历史行为 0)
  fix_session_id      TEXT,                        -- 当前或最近一次 Fix 会话 c3SessionId (v24→v25 新增; 历史行为 NULL)
  fix_status          TEXT CHECK(fix_status IN ('pending','fixed')),  -- PR 修复结论: NULL=尚未进入 Fix, pending=待结论, fixed=已处理 (v24→v25 新增; 历史行为 NULL)
