...
--
-- 不绑工作区。本表刻意没有 workspace_name 列 —— 机器人不属于任何工作区, 它按 name 拥有一个独立
-- 工作目录 ~/.c3/robots/<name>/, 那里是它能触及的全部范围。因此 name 同时是显示名、目录名和身份,
-- 受路径安全约束 (^[a-z0-9][a-z0-9_-]{0,31}$) 且创建后不可改 —— 改名等于换一个机器人, 已有线程
-- 的会话历史会随之失去归属。
--
