...
- **确认修订(2026-07-31):** 交互式保存的人工授权由**用户在对话中的文字确认**承担——沟通智能体
  ...
  **纵深边界:** 没有保存弹框,意味着一次被诱导的 upsert 可以原地改写一条**已批准、已排队**意图的
  正文,而 spec 文件不变、批准仍在,自动化随后会把改写后的正文当作开发指令交给有写入权限的会话。
  因此不变量下沉到台账:带 `id` 的更新使 `title`/`content` 实际变化时,该意图的 spec 批准立即失效
  (清批准与批准人、否决旧审核结论、写 `spec_unapproved`),必须重新评审批准才能进入开发
  (意图管理规格 RM-R20)。这不恢复任何保存弹框——新建路径与元数据更新完全不受影响。
...
- 带 `id` 的保存使既有意图的 `title`/`content` 实际变化时,台账 MUST 在同一事务内撤销该意图的
  spec 批准并否决旧审核结论(RM-R20)。评审者应拒绝任何让改写后的意图沿用旧批准进入开发的路径,
  也应拒绝以恢复保存弹框来替代这条不变量。
