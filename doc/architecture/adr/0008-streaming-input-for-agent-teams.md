...
The run loop drove the SDK `query()` with a **string** prompt — a one-shot turn. The SDK ends a
...
to a background teammate (a background `Agent` tool call) it then yields a `result` for the current
...
in streaming-input mode. Under the string prompt they were issued but silently swallowed, so a
mid-run `set_mode` and `stop_run`'s interrupt were effectively no-ops.
...
3. **All sessions use streaming input; detect teams at runtime; fork on `result`.** Always drive
   ...
   more machinery (a controlled input stream) on every run, including those that never form a team.

## Decision

Adopt option 3.

- **Uniform streaming input.** Every run drives `query()` with a controlled async-iterable of user
  messages, seeded with the user's first turn. Pushing text appends a turn to the same live
  session; closing the stream ends it so the query terminates normally.
- **Runtime team detection.** On each tool-use block (before that turn's `result`), a team-tool
  check flags the run as a team and fires the team callback once. A team tool is `TeamCreate`,
  `SendMessage`, or a background `Agent` (`run_in_background === true`); a foreground `Agent` is
  not (it finishes within the turn).
- **`result` fork.** On `result` the run always emits a completed `turn_end`; then a **non-team**
  run closes its input (process exits — the one-shot behaviour, next turn resumes a fresh
  process), while a **team** run keeps the input open so the lead process stays alive to coordinate
  teammates across turns.
- **End only on explicit stop.** A team's input never auto-closes; the abort path closes it
  (plus interrupting the SDK), making `stop_run` / `delete_session` / `remove_workspace` the sole
  way a team session ends. "Team lead is finished" is equated with explicit user stop — there is no
  automatic team-teardown detection.
- **Live team turns.** While a session is a `team`, a `user_prompt` is pushed into the live lead's
  input stream (no second run, no resume) rather than rejected as a serial violation.
...
- **Harder:** every run carries the streaming-input plumbing; a team session is a long-lived
  process that consumes resources until the user stops it (by design — there is no auto-teardown).
  A new `team` session status, a `team_upgraded` wire event, and a team flag on the runtime are
  added to the protocol and registry.
- **Migration:** the run loop's prompt input is now a controlled stream, not a string; the run
  handle gains a push-input control; the run options gain a team callback; the session runtime and
  session status gain a `team` state; the server-to-client union gains `team_upgraded`. The
  one-shot exit semantics for normal sessions are preserved exactly (close on `result`).

## Compliance

- Runs MUST drive `query()` with the controlled input stream, never a string prompt. Reviewers
  reject a string prompt.
...
- Team detection MUST use the team-tool check (foreground `Agent` is not a team) and fire the team
  callback at most once per run.
