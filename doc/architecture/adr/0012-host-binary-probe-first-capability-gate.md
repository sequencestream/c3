...
1. 显式的 `<VENDOR>_PATH`；
2. `~/.c3/vendor/<vendor>/<version>/bin/<binary>{.exe}` 下 c3 托管的 CLI；
3. 降级的宿主 `PATH` 回退。
...
归档解包以归档所在目录为工作目录，只把文件名交给 `tar`，避免 Windows 盘符被 GNU tar
解释成远端归档地址。POSIX 安装生成 shell 入口；Windows 安装从 npm 平台包中定位原生
`.exe`，把它及所需运行时文件发布到 `bin`，版本探测与智能体运行均直接启动该原生入口。
Windows 发布版本目录时，对杀毒软件或索引器短暂占用导致的 `EPERM`、`EACCES`、`EBUSY`
执行有限退避重试；其他错误仍立即失败。

远端同步在服务器启动时以后台任务触发，启动流程不等待它：绑定端口与进入就绪状态与
registry 的快慢无关。每个厂商各自遵守 24 小时远端检查冷却，冷却期内不访问远端。后台任
务以异步子进程执行 npm 安装，不阻塞服务器事件循环；失败只写入该厂商的 manifest 状态并
记录日志，不改变启动结果。因此启动时打印的健康快照是**刷新前**的解析结果——本轮后台
检查的产物要等它落盘后的解析才可见。
