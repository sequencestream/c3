...
> **2026-06-07 简化(本 ADR 同日收敛)**:skill-repo 配置项收敛为仅 id、仓库地址、ref、子路径四个字段。
> 移除 vendor 字段(skill 跨 vendor 通用 → 固定挂入所有 build-link-capable vendor,即原「挂入全部 vendor」行为)、
> 移除信任级别三档与「锁定 commit」(用户始终要 ref 最新版,pinned/review/unreviewed 审批徒增复杂度 → 改为
> **静默挂载**:直取 ref 最新版软链,挂载前无任何信任弹窗)。因此信任级别与 vendor 范围两个类型删除、
> pinned `git cat-file` 防伪删除。仅保留 `.gitignore` 一次性追加确认(写用户文件)。详见下文标注。

## Context

c3 的后台 dev skill 现仅支持单个可选的字符串配置(斜杠命令前缀=触发器,见
共享协议定义中的 dev-skill 设置)。新增独立的外部 skill-repo 列表配置,
...
1. **目录布局**:扁平 `<vendorSkillsDir>/_c3_<id>/SKILL.md`(claude/codex 一致)。`_c3_` 前缀 + 配置 id 作目录名,便于按 id 反查/清理。
   ~~协议层的 vendor 范围取值~~ **已移除**:挂载固定挂入所有 build-link-capable vendor,不再有 per-repo vendor 选择。
   挂载阶段(2/3)只是软链到各 vendor 目录,不重复 clone。
2. ~~**pinned 防伪**~~ **已移除**(简化):不再有「锁定 commit」的信任级别,统一静默挂载 `ref` 最新版;
   仓库拉取始终用 `fetch + reset --hard FETCH_HEAD` 取远端 head,不做 `git cat-file` 防伪校验。
...
- skill-repo 配置项(id/仓库地址/ref/可选子路径)+ workspace 设置上的可选 skill-repo 列表落共享协议定义(信任级别与 vendor 范围两个类型已删)。
- kernel 配置层按 workspace 读取 skill-repo 列表并 fail-hard 校验(id/repo/ref);git 操作落独立的 skill-repo 操作层。
- 单测覆盖:配置非法报错(id/repo/ref/重复/与 dev-skill 冲突)+ GitHub URL 回填;`git clone`/pull/`ls-remote`/子路径 + vendor 共用缓存。
