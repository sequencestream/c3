...
8. **安装尝试 24 小时冷却**:每次安装尝试在下载前把时间戳写入 `~/.c3/sandbox/arapuca/install-state.json`,无论成功还是失败,24 小时内不再发起新的安装。进程内单飞锁只能挡住同一进程的并发探测;网络不可达时,每次 run、每次探测缓存失效、每次重启都会重新下载,浪费带宽、可能触发 GitHub 限流,并把真正的失败原因淹没在重复日志里。冷却时长直接沿用 vendor CLI 远端检查的固定 24h(不新增配置项),状态文件与 vendor manifest 相互隔离。冷却跳过是静默的,且不改变探测语义:调用方仍按原有方式回退宿主 PATH。取舍是用户即使已修复网络也需等窗口结束(重启不清除),与 vendor CLI 现状一致;时间戳缺失或损坏一律按「无冷却记录」放行,写盘失败只告警、不阻塞 run。
...
- 安装尝试的 24h 冷却由单测覆盖:成功/失败两条路径都在下载前落盘时间戳、窗口内跳过且静默、恰满 24h 放行、模拟进程重启仍跳过、损坏时间戳与写盘失败均不阻断安装与探测。
