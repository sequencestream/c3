...
| 外部 MCP 路由          | 公开的 `/mcp/v1`：c3 未拉起的 agent 凭长期 API key + `?workspace=` 只读访问意图/讨论并投递事件。与六条 `/internal/*-mcp/v1` 并列而非放宽——后者保留 loopback guard 与 per-run token。见 [external-mcp](../domains/core/external-mcp/external-mcp-spec.md) |
