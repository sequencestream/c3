...
> - 与 c3 的关系见 [`architecture.md`](architecture.md)（c3 的 agent-session 能力封装 SDK 的 `query()`）。

> 其它 vendor 的架构与本指南描述的 Claude Agent SDK 不同，分别封装在各自的 vendor 适配能力中；
...
> [WebSocket 线协议](../shared/api-conventions/websocket-protocol.md)。所有事件写入会话运行时缓冲
> 并投递给当前订阅的连接（ADR 0006）。
...
c3 显式探测并传入 `pathToClaudeCodeExecutable`：
...
canUseTool: async (toolName, input) => {
  // → 经 WebSocket 发 permission_request 给浏览器，阻塞等待用户决定
  const decision = await /* 等待浏览器返回的决定 */
...
用与 c3 命令发现相同的机制（streaming-input `query()` 配合命令枚举，
...
flowchart LR
  subgraph 配置
    R[配置项 skillRepos] --> V[校验仓库配置 fail-hard]
  end
  subgraph 挂载
    A[确保仓库 clone/pull] --> B[探测 vendor skill 支持?]
    B -->|full| C[建立软链]
...
- 确保仓库就绪:clone(首次)或 pull(后续 `git fetch + reset --hard FETCH_HEAD`,始终取 ref 最新 head),然后解析子路径。
...
挂载固定挂入所有 skill 支持判定为 full 的 vendor,各建一份软链(无 per-repo vendor 选择)。

### 6.3. 准入管制

- **vendor skill 支持探测**:结果缓存在 c3 运行时状态中,SDK 版本升级时主动失效重探;非 `full` 的 vendor 标灰、不建链,session 仍启动。
- **静默挂载**(2026-06-07 简化):无 trust 三档、无挂载前确认,直取 `ref` 最新版软链。
- **.gitignore**:首次挂载前请求 Human 确认追加 `_c3_*/` 条目;确认后永久静默(仅剩的人工闸)。
- **写操作审批**:挂载了外部 skill 的 session 中(与 trust 无关),写类工具走 `permission_request` 而非任何自动放行。

### 6.4. 生命周期

- 挂载在 run 启动流程中、vendor 驱动启动 **之前**完成。
...
- `interrupt()` 返回的 Promise 可能**异步 reject**（如 “ProcessTransport is not ready for
  ...
  `.catch()`** 兜住，否则会让进程崩溃。c3 在 abort 处理里正是这么做的（外部 SDK 行为）。
...
## 8. 版本升级评估留痕（0.3.183 → 0.3.195）

> 2026-06-28 升级。本节是 `0.3.183 → 0.3.195` changelog 的逐项评估账本——每条 SDK 变化都给出
> 「接入/不接入 + 依据 + 留痕去向」，便于人工逐项抽查。本次只升 Claude SDK，`@openai/codex-sdk`
> 与其它依赖号未动。**vendor 中性适配器面（`adapters/types.ts` 与 ADR-0011 capability ledger）
> 未被任何接入触及**，故 capability grid 不变。顶部四处维护语义（是否需本机 `claude`、
> `settingSources` 语义、会话存储路径、Skill 开关）经复核本次升级均未改变。
