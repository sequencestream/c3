...
1. **loopback 绑定** — relay 路由挂在 c3 主 Hono app 上，其网络可达性取决于主 server 的绑定地址（由使用者决定）。
   _（注 2026-06-13）：主 server 启动时当前**未指定 hostname**，Node 实际绑定 `0.0.0.0`（全网卡），故当主 server 未限定到回环时，relay 端点对 LAN 可达，token 验证是唯一闸。容器化路径**刻意不依赖**主 server 是否绑定回环，见下。_
...
- **Docker Desktop（macOS/Windows）**：`host.docker.internal` 由 Desktop 直通 host loopback，relay 保持 loopback 即可达，**零新网络暴露**。这是支持的目标路径。
- **Linux 原生 Docker**：`host.docker.internal` 经 `host-gateway` 解析到 bridge 网关 IP；要到达 host relay，host 须在该接口监听。当前因主 server 默认绑定 `0.0.0.0` 而可达，但**本设计刻意不背书该依赖**——若主 server 被限定到回环，Linux 原生下此路径不再连通。Linux 原生的 RELAY-in-sandbox 因此列为**已知限制**，正式支持留待方案二（容器内 relay sidecar，见 §6 与 ADR-0024）。
...
- **relay 网络面**：本任务**未引入**任何非 loopback 绑定，relay 与主 server 的绑定面未被本任务改动。
- **主 server 绑定面（记录用，用户 Q2-B 决策）**：主 server 启动时未指定 hostname → Node 绑 `0.0.0.0`。绑定地址由使用者决定；当主 server 绑定回环时，relay 端点也随之仅回环可达。Docker Desktop 路径不依赖这一点（host.docker.internal 仍直通 loopback）；Linux 原生 RELAY-in-sandbox 依赖主 server 在该接口监听，故列为已知限制 → 方案二。

**结论**：在 Q1-A（relay 不放宽 + host.docker.internal）下，容器内 codex RELAY 不扩大 relay 的网络暴露面，满足 deny-by-default 与回环隔离；唯一新增的是同宿主、随 run 销毁的容器对 relay 的访问，由不透明一次性 token + 真实 key 隔离兜底。
...
8. **容器内 RELAY 仅 Docker Desktop 路径受支持（ADR-0024 follow-up）** — sandbox 容器内的 codex RELAY 经 `host.docker.internal` 回连 host loopback relay（§2.6）。Docker Desktop 直通 host loopback，零新暴露；**Linux 原生 Docker** 下 `host.docker.internal` 只到 bridge 网关，到达 host relay 依赖 host 在该接口监听（当前因主 server 默认绑定 `0.0.0.0` 而可达，本设计不背书），故 Linux 原生 RELAY-in-sandbox 列为已知限制，正式支持待**方案二：容器内 relay sidecar**（容器内跑一份 relay，监听容器自身 loopback，零 host 暴露）。
9. **主 server 绑定面（记录用）** — 主 server 启动时未指定 hostname → Node 绑 `0.0.0.0`。绑定地址由使用者决定；当主 server 绑定回环时，relay 端点也随之仅回环可达。详见 §2.7。
