...
1. **loopback 绑定** — relay 路由挂在 c3 主 Hono app 上，其网络可达性取决于主 server 的绑定地址（由使用者决定）。
   _主 server 启动时当前**未指定 hostname**，Node 实际绑定 `0.0.0.0`（全网卡），故当主 server 未限定到回环时，relay 端点对 LAN 可达，token 验证是唯一闸。_
...
### 2.6 沙箱内 codex RELAY（进程级沙箱）

当被随机选中的 sandbox agent 是一个 **codex RELAY 路（`wireApi=chat`）** 的自定义 agent 时，codex 在进程级轻量沙箱（arapuca）内执行。进程级沙箱下 codex 是**宿主进程**——沙箱只用内核 MAC 收窄其文件系统权限，网络当前全开且不改变回环语义。因此沙箱内 `127.0.0.1` 就是宿主本机，codex 直接用 `http://127.0.0.1:<c3port>/internal/codex-relay/v1` 够到宿主回环上的 relay，无需任何容器回连桥或专用回连主机名。

```
┌── host ──────────────────────────────────────────────┐
│  c3 server（relay 绑 c3 loopback）                    │
│        ▲  http://127.0.0.1:<port>/…                  │
│        │  （同一宿主回环，进程直连）                 │
│  ┌─────┴───────────── arapuca 进程级沙箱 ──────────┐ │
│  │  codex（宿主进程，内核 MAC 收窄文件权限）        │ │
│  │    base_url = http://127.0.0.1:<port>/…         │ │
│  │    CODEX_API_KEY = <relay token>（继承宿主 env） │ │
│  └───────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────┘
```

进程级模型天然消解了容器方案为"容器 loopback ≠ host loopback"引入的全部机制：

| 关注点        | 进程级沙箱下的处理                                                                                              |
| ------------- | --------------------------------------------------------------------------------------------------------------- |
| base_url 可达 | 沙箱内 `127.0.0.1` 即宿主本机，relay base_url 保持宿主回环即可直连，不需改写为容器回连主机名                    |
| 回连桥        | 不需要 host-gateway / extra-hosts；不存在容器→宿主的跨边界问题                                                  |
| token 传递    | codex 进程直接继承宿主 env / envOverrides，`CODEX_API_KEY=<token>` 随进程环境传入，无 env-file 容器注入这一环节 |
| NO_PROXY      | 沿用 `NO_PROXY=127.0.0.1,localhost,::1`，防回环 hop 被 HTTP_PROXY 劫持                                          |

**安全暴露面**：沙箱内 codex 与宿主其它进程一样经回环访问 relay，relay 的绑定面与非沙箱路径完全一致，进程级沙箱不新增任何 `0.0.0.0`/bridge/LAN 绑定。真实凭证仍只存于宿主内存的 token registry，由 §2.3 的四层防御兜底（不透明一次性 token、run 结束注销、真实 key 不离开 c3、未知 token 返回 401）。
...
8. **主 server 绑定面（记录用）** — 主 server 启动时未指定 hostname → Node 绑 `0.0.0.0`。绑定地址由使用者决定；当主 server 绑定回环时，relay 端点也随之仅回环可达。详见 §2.3。
