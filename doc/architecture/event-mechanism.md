...
**`pr:merge` 的行为变更（交付上线后）**：它的 base **可能是一条交付分支**，而不再必然是主线——同一个
事件类型现在报告两个实质不同的事实（「改动进了某个交付」与「改动进了主线」）。为此 `data.ref` 增加两个
**可选**字段：`baseBranch`（合并目标分支名）与 `baseTarget`（`mainline` / `delivery-branch`）。订阅方据此
区分产出落在哪一侧；一个在每次 merge 上触发发布的自动化尤其需要检查它。旧事件形态（只有 `head`/`base`）
仍然合法，既有订阅方零改动；`baseTarget` 取值不在闭集内会在归一化时被丢弃，订阅方永远不会看到第三种值。
服务端自建 PR 的路径（`runServerSidePrCreate`）按 PR 是否绑定交付自动填这两个字段。这次语义漂移**没有
技术回退**（事件已发、自动化已执行），真正的防御是工作区首次创建交付时的一次性知情提示。
