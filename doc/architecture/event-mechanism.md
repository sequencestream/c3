...
| `run:started`           | `sessionId, workspacePath, kind: RunKind`                                                | 同上                                          | Schedule 事件触发分发                                                       |
| `run:settled`           | `sessionId, workspacePath, reason: RunEndReason, kind`                                   | 同上                                          | 各 domain 列表广播 + automation 编排 FSM + Schedule 触发                    |
...
- `RunKind = session | intent | discussion | schedule | consensus | tool | spec`
  ——单一真相的 run 来源分类。**注意 `schedule` 是「触发源」不是「run 类型」**：被 schedule 触发的目标用户 run 仍是 `session` kind，`schedule` 只标记 scheduler 自己那个无 socket 的 run。事件触发的 Schedule 因此只在 `session` kind 上 fire。
...
> 目前 `consensus`、`tool` 两种 RunKind 仍以「类型化标注 + 日志 tag」存在，**尚未经过总线**。
...
1. **RunKind 白名单**（仅 run:started/settled）：只有 `session` 源触发用户 schedule；
2. **workspace 匹配**：`schedule.workspaceId == event.workspacePath`；
3. **topic 专属过滤器**（上表）；
4. **串行门**：该 schedule 已有 in-flight 执行则跳过（防 event storm 堆叠）；
5. 命中 → `dispatchAndTrack(schedule)` → 执行并发 `run:*`（kind=`schedule`）回总线。
...
- [ADR-0018 — In-Process Event Bus in the Kernel Layer](adr/0018-event-bus-kernel-layer.md)：总线选型决策、RunKind 分类、per-launch→常驻订阅的重构。
