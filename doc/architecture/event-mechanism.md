...
| **模型可发布事件（归一化层）**   | ✅ 按 type 注册归一化器；当前注册 6 个 `pr:<op>` + `pr:operation` 别名 | `server/src/kernel/events/generic-event.ts` |
...
- [ADR-0027 — `<category>:<action>` 事件命名 + 多行订阅 + 级联表单](adr/0027-event-naming-and-multi-row-subscription.md)：统一事件命名规范为 `<category>:<action>`（大类:动作），引入多行订阅 `eventFilters[]`、大类通配 `:*`、以及级联表单 UI。模型可发布事件从「每种事件新增窄工具」修订为「type 判别 + 封闭归一化器注册」，及保留类型化 topic 适配层的边界。
