...
- `DISCUSSION_LIFECYCLE_PHASES = start | end`（`end` 的 reason 复用 `RunEndReason`）

> 一次性、无工具的 advisor 调用（共识投票、判定、编排者笔记、自动化标题生成）以 `sessionKind: tool` + `runKind: internal` **经过总线**：它们同样发 `run:started` / `run:settled`，因此在日志与审计里可见。它们没有会话视图，故只发这两个 topic、**不发 `run:bound`**（那个 topic 的常驻订阅会做会话模式持久化与 `session_started` 广播，对内部 run 是错误的副作用）；事件里的 id 是一个进程内唯一的合成 `internal:<kind>:<n>`，起落同 id 才配得成对，厂商真实会话 id 只进日志。没有工作区可归属时（如未绑定工作区的标题生成）只打日志、不发事件——不发一条指向空路径的事件。
>
> **触发侧的例外**：`tool` 是**仅显式点名**才参与自动化触发的场景。这类 run 频次远高于用户可见的 run，若落进「空 `eventSessionKindFilter` = 所有场景」的默认语义，一个既有的 `run:settled` 订阅会被每一次投票唤醒。因此空过滤器对 `tool` 一律不匹配，必须在 `eventSessionKindFilter` 里点名。`consensus` SessionKind 仍只作类型化标注（共识投票在会话账本里记为 `tool`）。

## 3.5 事件类型目录与命名规范（2026-07-14）

事件类型统一为 `<category>:<action>`（大类:动作）结构，由代码事实源 `EVENT_CATALOG` 常量
（`shared/src/event-catalog.ts`）维护已知目录。该目录是**建议清单，非封闭枚举**——未收录的
...
写在业务 metadata 之后**，键冲突时以真实身份为准——调用方无法用自己的 metadata 伪造事件身份。
...
**通用事件契约**（`shared/src/event-model.ts`）：`GenericEvent = { type, status?, description?, metadata?, data? }`——`type` 是必填非空的稳定判别值，`metadata` 是扁平 `Record<string,string>`，`data` 是 JSON 兼容的递归对象；`validateGenericEvent` 拒绝空 type、嵌套 metadata、非 JSON `data`。`GenericEventEnvelope = { workspacePath, sessionId, event }` 的信封字段由 per-run 绑定闭包在归一化成功后注入，原始事件里的同名键不得覆盖。
