...
| **订阅侧（含 Automation 订阅）** | ✅ 通用 `eventFilter`（type/status/metadata）单过滤器   | 常驻订阅 + `dispatchEventTriggers(view)`    |
...
**关键认知**：你要的「扩展性」绝大部分已在**总线核 + 订阅侧**就位——加新事件类型是「一行 topic + 一处 publish」，已有订阅者零改动（ADR-0018 已验证：降级链三个 agent topic 就是这么加进来的）。**模型可发布事件**这层经 kernel 归一化器注册表按 `type` 注册（[ADR-0026](adr/0026-generic-event-normalizer-registry.md)，§9.3），未注册即拒。**模型对外 MCP 工具**现在收敛为单一 `publish_event`：入参就是通用事件，`type` 选中注册的归一化器，加新可发布事件时**工具面零改动**，只需注册一个归一化器（见 [§7 扩展指南](#7-扩展指南)）。
...
| **模型可发布事件**   | `server/src/features/events/`（工具）+ `pr-events/`（PR 归一化，详见 §6） | `event`（当前 `type='pr:operation'`）                      |
| **c3 服务端自建 PR** | `dev-cleanup.ts`、`automation.ts`、`intents/index.ts`（create_pr）        | `event`（`type='pr:operation'`，仅 `create`/`success`）    |
...
| **Automation 事件触发分发**             | `run:started`/`run:settled`/`event`/`intent:lifecycle`                       | 见 §6/§7：各订阅桥把总线事件归一化为 `view={workspacePath, event, sessionKind?}`（envelope 直接透传，不再按 `event.type` 投影出 operation/result），统一交给 `dispatchEventTriggers(view)`；通用匹配器 `genericEventFilterMatches` 按 workspace→type→status→metadata 匹配，`getEventAutomations(type)` 按 `eventFilter.type` 取候选（PR 的 operation 多选表达为 `metadata.operation` 的 `OR` 条件） |
...
这是「事件机制」唯一对模型开放的入口，也是唯一的模型发布工具。设计原则：**单一通用工具（入参即 `GenericEvent`）、封闭类型集合、按 type 字段级安全归一化、per-run 信封不可伪造**。工具面对新增可发布事件零改动——扩展点在归一化器注册表（§9.3），不再是「每类事件一个窄工具」。

### 6.1 工具语义

`event` topic 有两个发布者：模型经 `publish_event` 工具发布，以及 c3 自建 PR 的服务端路径（dev-cleanup / automation / 手动 create_pr）在成功建 PR 后构造 `create` 事件。以 PR 为例（当前唯一注册 type `pr:operation`）：模型先用自己的工具（`gh` CLI / GitHub MCP 等）完成 PR 的创建/评审/合并/关闭/评论，或修改重提（`update`：已有 PR 被打回后 push 修复、重新提交/重新打开，**非新建 PR**），**操作完成或失败后**再调用 `publish_event`（`type='pr:operation'`）发布一条供应商中立的通用事件，供订阅了 `pr:operation` 的 Automation 匹配并触发后续动作。`update`/`success` 事件还会被 intent domain 消费，把 rejected/failed/closed 的意图 `prStatus` 复位为 `reviewing`（见 §5 订阅者地图）。服务端路径绕过模型直接创建 PR，因此也需要将结果通知总线。

### 6.2 入参 Schema（Zod，单一来源 `features/events/tool-defs.ts`）

入参直接对应 `GenericEvent`：`type`（必填非空，须为已注册类型）；`status` / `description` / `metadata`（扁平 `string→string`）/ `data`（JSON 兼容对象）可选。工具描述在 system prompt 中说明各 type 的字段约定——PR 事件把 `status` 填 result（`success`/`failure`/`error`）、`metadata.operation` 填 operation、`description` 填 errorSummary、`data` 承载 `{ pr, repo, ref, association }`（`association` 含 `intentId` + `intentTitle`，经安全归一后发布，让事件自解释）。
...
```
publish_event(core) ─► handler: normalizeEvent(core)   // 查表→§6.3 归一化→再校验；未注册/非法则不发布
...
| 表面                          | 文件                                            | 路径                                         |
| ----------------------------- | ----------------------------------------------- | -------------------------------------------- |
| 进程内 MCP（Claude）          | `server/src/features/pr-events/publish-tool.ts` | `createPublishEventMcpServer(binding, deps)` |
...
§6.3 的字段级归一化是 kernel 归一化器注册表（§9.3）里 `type: 'pr:operation'` 的**注册项**——**模型发布**与**三条服务端建 PR 路径**共用这**唯一一份**归一化实现。链路：
...
组合根用 `registry.register('pr:operation', normalizePrGenericEvent)` 装配注册表，并把 `normalizeEvent` 注入到三条 MCP 表面、三条服务端建 PR 路径（dev-cleanup / automation / 手动 create_pr，后者经 `KernelContext.normalizeEvent`）。

## 7. Automation 事件触发：单一通用过滤器（type/status/metadata）
...
`Automation` / `CreateAutomationInput` / `UpdateAutomationInput` 以单一 `eventFilter: GenericEventFilter | null` 取代 `eventTopic`/`eventReasonFilter`/`eventPrFilter`/`eventIntentFilter`/`eventMetadataFilter`。`eventSessionKindFilter` **保持独立字段**——它是 run 生命周期的强制安全边界，不是业务过滤器。
...
| ------------------ | ------------------------------------------------------------------------------------------------ | -------------- |
| `run:started`      | `{ type:'run:started', metadata? }`（无 status）                                                 | 有（安全边界） |
| `run:settled`      | `{ type:'run:settled', status:reason, metadata? }`                                               | 有（安全边界） |
| `pr:operation`     | 归一化 envelope 直接透传：`type='pr:operation'`、`status=result`、`metadata.operation=operation` | 无             |
| `intent:lifecycle` | `{ type:'intent:lifecycle', status:phase }`                                                      | 无             |
...
```
模型(完成 PR 操作后) ─调用─► publish_event({ type:'pr:operation', status:result,
...
模型可发布事件的安全原则是「**type 判别 + 封闭归一化器注册**」，而**不是**「每种事件复制一个窄工具 + 一套字段级归一化」。单一 `publish_event` 工具入参就是 `GenericEvent`，但它**不是** `payload: unknown` 的任意透传：`type` 必须已注册，否则拒绝。这是对旧结论「每种事件新增 `publish_<x>_event` 窄工具」的**有意修订**（[ADR-0026](adr/0026-generic-event-normalizer-registry.md)）：用中间路线同时拿到**通用性**（一个工具 + 一条发布链路）与**字段级安全**（封闭注册表 + 逐 type 归一化器）。
...
3. **工具面零改动**：模型直接用 `publish_event` 带上新 `type` 即可；成功归一化后由 per-run 闭包补 `GenericEventEnvelope` 落到单一 `'event'` topic。需要反应的消费者订阅 `'event'`、判别 `event.type` 后投影自己关心的字段（PR 见 §6.6）。

`pr:operation` 是首个注册 type（见 §6.6）。旧的「每种事件新增窄工具」判据（payload 几乎一致才用判别联合）已不再适用——注册表 + 单一通用工具让多类型与字段级安全兼得。
...
| PR 归一化器 + 消费侧投影             | `server/src/features/pr-events/tool-defs.ts`（`normalizePrGenericEvent`/`projectPrOperationEvent`/`runServerSidePrCreate`） |
| `publish_event` 进程内 MCP（Claude） | `server/src/features/pr-events/publish-tool.ts`                                                                             |
| `publish_event` HTTP MCP（Codex）    | `server/src/transport/pr-event-mcp/index.ts`                                                                                |
