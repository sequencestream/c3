...
| **Automation 事件触发分发**             | `run:started`/`run:settled`/`event`/`intent:lifecycle`                       | 见 §6/§7：run topic 直接分发；`event` topic 先判别 `event.type==='pr:operation'`、从 `metadata.operation`+`status` 投影出 operation/result，再走既有 `pr:operation` 分发（topic/workspace/PR filter/串行门）。持久化的 automation 配置仍用 `eventTopic:'pr:operation'` 判别，仅总线 topic 收敛为 `'event'`                                                                     |
...
分发核心 `dispatchEventTriggers(topic, payload)`（`features/triggers/index.ts`）的匹配顺序（`topic` 仍是 `ScheduleEventTopic`，PR 事件由 `'event'` 总线桥投影后以 `'pr:operation'` 调入）：
...
eventBus.publish('event', GenericEventEnvelope)
  ...
  ▼
Automation 事件桥(scheduler-startup) ─► 投影 operation/result ─► dispatchEventTriggers('pr:operation', e)
