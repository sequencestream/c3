...
> API 契约记录"c3 与 LS 如何通信"的线缆边界。需要这些内容时按 ID 引用,不在此处复述。
...
| `C3_LS_PUBLIC_URL`                    | 激活时   | 进程自身基址(本地监听);`C3_LS_BASE_URL` 留空时作对外 URL 回退                          |
| `C3_LS_BASE_URL`                      | 反代后   | 对外可见基址,构造 OAuth 回调 / 微信 notify / Cookie Secure;留空回退 `C3_LS_PUBLIC_URL` |
...
- **缺 OAuth / 签名私钥 / 对外基址(BaseURL 或回退 PublicURL)任一**:激活面返回清晰的 "unavailable",而非半可用流程。
...
[license-server API 契约](../shared/api-conventions/license-server-api.md)记录一次,他处按 ID 引用。
...
(state 为无状态 HMAC 签名,透传回跳目标);回调地址用对外基址(BaseURL 或回退 PublicURL)构造;未就绪(缺库/OAuth/签名私钥/对外基址)→ `503`。
