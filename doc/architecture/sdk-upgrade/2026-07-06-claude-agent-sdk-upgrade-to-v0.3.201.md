...
```
Warning: canUseTool will not be invoked: permissionMode 'bypassPermissions' auto-approves
...
**结论：预期且安全，非配置冲突，非升级回归。** 逐点核实（代码追踪）：

- **触发面**：警告仅在 `permissionMode === 'bypassPermissions'` 时 `process.emitWarning`。c3 中该
  permissionMode 只由 `toPermissionMode(actionMode, 'never-ask')`（`claude/modes.ts` 的
  `bypassPermissions` cell）产出，即用户显式选了 never-ask 工具门的**标准 work 会话**。此时「callback
  之前就自动放行一切」正是 never-ask 的定义语义。
- **默认路径不触发**：Claude work 会话默认工具门为 `always-ask`，无精确 cell → 回退到同 actionMode 的
  `default`（`build × on-sensitive`）→ `permissionMode: 'default'` → canUseTool **正常被调用**，无警告。
  `plan` / `acceptEdits` 同理仍 gated。
- **只读门安全（关键安全核查）**：三个 read-only 门在 Claude 路径均**不会**落到 bypassPermissions——
  intent/spec 门经 `intentDriverModeForVendor`/`specDriverModeForVendor` 强制 `always-ask`，discussion 门
  `DISCUSS_TOOL_GATE = 'on-sensitive'`，三者都映射到 `default` 模式，canUseTool 始终生效，只读写拦截不被
  旁路。因此警告永远不会出现在只读门会话上，「gateway 被 shadow」不构成安全洞。
- **非回归**：`bypassPermissions` shadow 掉 canUseTool 是该模式一贯行为；0.3.198 只是新增了这条
  `process.emitWarning` 提示，行为本身未变。

**降噪处理（已实施）**：该警告在 never-ask 会话每个 turn 都打，实盘持续刷屏，故加了一个**外科式**过滤：
`kernel/agent/adapters/claude/sdk-warning-filter.ts` 用一层窄 `process.emit` 包装，**只丢 `warning` 事件上
`code === 'CLAUDE_SDK_CAN_USE_TOOL_SHADOWED'` 这一条**，其余事件与其余警告(含任何真实误配置告警)原样透传。
由 `kernel/agent/index.ts`(拥有全部 Claude `query()` 调用点)顶层 `installClaudeSdkWarningFilter()` 幂等安装
一次。选 `process.emit` 包装而非 `process.on('warning', …)`：Node 的 warning 监听器**不会**替代默认打印，只有
在 emit 前拦截才能真正静默单条 code；`--no-warnings` 又会无差别静默全部。纯谓词 `isSuppressedClaudeWarning`
单测覆盖「丢目标 code / 放行其它 code / 放行无 code / 不碰非 warning 事件 / 容忍 null 载荷」。
...
- `pnpm lint`：0 error。
- `pnpm vitest run`：251 test files passed，1 skipped；3504 tests passed，1 skipped（含新增
  `sdk-warning-filter.test.ts` 6 例；权限网关测试全绿，`createCanUseTool` 仅注释改动）。
