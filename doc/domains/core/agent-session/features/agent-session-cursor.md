...
规范化器是有状态的(每运行一个实例)。**关键事实:SDK 的 `assistant` 与 `thinking`
帧都是 token 级增量**(由内部 `text-delta` / `thinking-delta` 构造),而 c3 全局
契约是**一个 canonical 文本块 = 一整段话**:claude / codex 的适配器交出的都是完整
块,下游(浏览器气泡、automation 完成度裁判、讨论回写)也都按"一条发出的文本 =
一条消息"读取。故规范化器**累积整段,只在 span 结束时发出一次**;原样转发增量会
把一次回复打散成几十条转录记录,并让那些消费者拿到孤零零的一个 token。

- **文本**累积进当前打开的 span;被工具调用、推理帧或轮次终局**结束**时整段发出,
  下一个增量开启新块。这样"工具调用之后说的话"不会被追加回它之前,转录顺序与模型
  实际产出一致。
- **收尾**:流耗尽而没有终局 `status` 帧时,由 driver 调用 `flush()` 交出仍然打开
  的 span,否则回复的最后一段会永远留在累加器里。
- **thinking** 仅来自原生 `thinking` 帧:增量拼接为单一块,空文本的收束帧(仅带
  `thinking_duration_ms`)结束并发出该 span。**绝不**从文本或事件名启发式
  伪造 thinking。
