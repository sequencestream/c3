...
- **AUTH-R8(改密关卡)** —— 更改一个既有账户的密码需要证明该
  ...
  失败返回 `invalid`。最小长度只有一个来源:服务端的 `MIN_PASSWORD_LEN` = 4;
  控制台的「添加账号」与「修改密码」弹窗**不复用该常量**(前端硬编码同一数值),
  但按同一数值把关——不足时提交按钮保持禁用,并就地说明是哪一条不满足
  (用户名为空 / 用户名已存在 / 密码不足 4 位 / 当前连接不是管理员),
  因此可提交的表单不会被服务端回以 `invalid`。(名册变更还额外受 AUTH-R10 把关——只有管理员可以
