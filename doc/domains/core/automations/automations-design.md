...
- **时区:** cron 字段按**系统级 IANA 时区**(配置的系统时区,见 [system-setting](../../settings/system-setting/system-setting-spec.md))解释,
  ...
  重新计算。
- **执行时段:** 每分 / 每时频率的 `hour` 字段可以只承载一个连续整点区间,即**半开区间**
  `[起:00, 止:00)`:`8-17` 表示 08:00 起、18:00 止(不含 18:00);跨午夜写成被 00:00 切开的两段
  `22-23,0-7`(22:00 至次日 08:00)。小时步长锚定在区间起点,跨午夜时两段各自步进
  (`0 22-23/2,0-7/2` 触发于 22、0、2、4、6 点)。这只是 `hour` 字段的一种写法:**服务端与调度器
  不感知"时段"概念**,仍按同一 cron 语义解释;全天即通配 `*`。
...
| 启动收尾遗留 running | 幂等地把上一次进程遗留的 `running` 执行改写为 `failed`(见调度引擎) |
...
服务重启时,上一次进程可能在写回终态之前崩溃,留下 `status='running'` 的执行日志。若不处理,这些行会
被「进行中」口径(列表绿点与顶部角标,见 `flow-automation-execution.md`)当作仍在运行而无限期点亮。

- 在数据库可用后、调度器接受新执行(cron/event 调度)之前,运行一次**幂等**启动收尾:同一事务内把
  全部遗留 `status='running'` 的执行改写为 `status='failed'`,以本次启动时间填充 `finished_at`,并写入
  统一的「服务重启中断」错误标识以区别正常执行失败。
- 只处理启动前已存在的 running 行;不做进程存活探测、超时扫描或周期清理。收尾完成后再启动调度,确保
  新进程创建的执行不会被误收尾。已有终态记录及其输出、退出码、会话 id 不改写,故重复调用为空操作。
- 启动收尾数据库操作失败时**不得静默启动调度器**,以免旧 running 与新执行并存持续误报;应记录可定位
  错误并保持自动化调度未启动,服务器其他能力按现有数据库降级策略继续运行。
- 这是一次轻量收敛:被记为失败的执行即使其外部子进程随后完成,也不会恢复成功。
