...
| `eventSessionKindFilter` | `SessionKind[]` \| null                                     | 用于运行生命周期事件触发(`eventFilter.type` = `run:started` / `run:settled`)的**可选**会话类型过滤(SCH-R18,2026-07-04)。缺失 / `null` / `[]` 语义等价 = 不按会话类型过滤(覆盖所有 SessionKind,含无会话来源事件);非空时作为精确白名单,仅所列来源命中。创建/更新时缺失或为空**被接受**(持久化规范化为 `NULL`)。保持独立字段(仅作用于运行生命周期类型)。cron / pr / intent 时为 null。遗留的运行生命周期行迁移为 `['work']` 保留原行为。                                                       |
