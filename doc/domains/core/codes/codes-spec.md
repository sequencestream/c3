...
- **CODE-R10**: 文件名搜索以查询词作条目基础名的不区分大小写子串匹配:连字符与扩展名不构成匹配边界(查询 `sandbox` 命中 `sandbox-architecture.md`),命中的 `match` 为完整基础名。查询词不被解释为 glob 或相对路径,也不分词或改写;文件名匹配不读取文件内容,并仍受 CODE-R9 的 glob 准入约束。
