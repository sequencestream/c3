...
- **被依赖方:** `web-console`(渲染交付一级页面:列表 + 详情两 Tab + 状态分段选择器 + 分支初始化区);`intent-management` 的三条建 PR 路径(手动 `createPrForIntent`、自动化队列 RM-A5、会话结束清理 RM-R26,共用 `pr-target.ts` 的 `resolvePrTarget`;面向交付的 PR 创建受 `branch_ready` 闸门拦截,自动路径在闸门未过时不建 PR 且不回退主线)与意图详情页(渲染 `Intent.linkedDeliveries` 与按交付分组的 PR)。
