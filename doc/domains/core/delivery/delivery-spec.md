...
- **范围:** deliveries 台账 CRUD + 取消、六态状态机与守卫、按工作区计算的「需要用户处理」角标、交付一级页面(列表 + 详情两 Tab + 分段选择器 + 常驻缺口 + 合并区)、`pr:merge` 一次性知情告知、交付分支生命周期(create/bind 初始化 + 孤儿分支防御 + 多仓拒绝 + 终态手动清理)、意图↔交付关联/解除(merged 禁解 + 解除时关闭未合并 PR + 关联时 diff 膨胀提示)、交付 PR 生命周期(先查 forge 事实的幂等创建 + 三类失败分层 + `delivered` 原子写 + 跨交付闸门重算)。
...
| DR-R40 | `pr:merge` 事件的 `ref` 增加可选 `baseBranch`(合并目标分支名)与 `baseTarget`(`mainline` / `delivery-branch`),让订阅方区分产出落在交付分支还是主线。旧事件形态(只有 `head`/`base`)仍然合法,既有订阅方零改动;`baseTarget` 取值不在闭集内会在归一化时被丢弃,订阅方永远不会看到第三种值。语义漂移本身**没有技术回退**(事件已发、自动化已执行),真正的防御是 DR-R7 的前置一次性知情提示                                                                                                            |
