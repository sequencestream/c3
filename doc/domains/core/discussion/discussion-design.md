...
更早的历史，因此重复发送会随着轮次数膨胀 token。**参与者增量提示语**还
**不再重复**首轮完整提示语中的静态字段（`You are a participant`、
讨论类型、目标、背景）——这些字段连同全部历史都在被恢复会话的上下文
窗口中，重复只消耗 token 且不利于提示语缓存；增量提示语直接以
`Current stage` 开头，只携带本轮所需的动态上下文（当前子话题、水位后的
新消息、组织者指导与作答约束）。组织者增量提示语仍保留讨论头部。
轮次成功后，编排器会推进该智能体的 `last_seq`：
