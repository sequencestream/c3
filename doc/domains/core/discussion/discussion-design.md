...
并以一个安全的 `advance` 作为默认值，使无法解析的回复永远不会卡死）。对象定位采用
**平衡计数**：从首个 `{` 起按深度扫描并跳过字符串字面量内的字符（含转义），因此
`conclusion` 值内部的 `{`/`}` 以及对象之后的附加文本都不会挪动对象边界。步骤解析将阶段、单阶段轮次上限
...
- `advance` —— 移动到下一个工作流阶段；一条非空的组织者 note（例如摘要）
  ...
  归位到议程长度（记录“议程已完成”）。
- `conclude` —— 追加最终结论（组织者消息），设置结论，标记为 `completed`。**结论防泄漏**：
  结论文本对用户可见，因此控制信封本身**永远不得**成为结论。严格解析成功时只取
  `conclusion` 字段（为空时取 note）；严格 `JSON.parse` 失败而关键词判定为结论意图时，
  若去围栏后的文本仍形如控制信封（以 `{` 开头且含 `"action"` 字段），只用受限的宽松提取
  抢救字符串型 `conclusion` 值（并处理常见转义），抢救不到（字段缺失 / 为空 / 边界不可判定）
  则返回**空结论信号**，由编排器改用最近一次摘要或本地化的 `conclusionFallback`——
  绝不把原始信封或其结构片段原样透出。普通散文回复不受影响，仍按去围栏后的全文作为结论。
...
**纯粹的决策/解析**：组织者决策解析（JSON / 围栏代码块 / 结论值内含成对花括号 /
对象后附加文本不扩大边界 / 严格解析失败的信封抢救出 `conclusion` / 无法抢救时不
透出原始信封（空结论信号）/ 关键词兜底 / 无效
