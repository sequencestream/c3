...
Each participating agent can also have one current vendor transcript mapping in
`discussion_agent_sessions`; that mapping is projected into `session_metadata` for the unified
Sessions page with the discussion as owner, but discussion messages remain the transcript SoT.
