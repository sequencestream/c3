...
- **启动讨论后默认落「研究会话」（2026-08-04）**：调研跑批是创建后的第一阶段，因此右侧
  面板的默认落点跟着它走。新建讨论时服务端先回 `discussion_detail`、随后才启动调研并在
  厂商报出 session id 时回写 `researchSessionId`，所以详情到达那一刻「研究会话」标签
  **尚不存在**，落点先按既有链停在「过程会话」（其中正展示实时调研流，内容不为空）；
  一旦调研存活且该标签出现，只要用户在本讨论内**没有亲手点过任何标签**，活动标签就
  **自动跟随**过去，并复用既有的「单一活动会话」对齐（未对齐时补发一次
  `select_session`）。手动点击标签会**永久停用**本讨论的自动跟随（切换到另一个讨论时
  复位，该讨论重新获得自己的默认落点）；调研**结束**只是让该规则不再成立，
  **绝不产生反向跳转** —— 自动启动编排后用户仍留在正在阅读/追问的「研究会话」。
  调研运行中从列表打开一个已绑定会话的讨论仍一步落到「研究会话」；已完成讨论落结论、
  调研失败的 `draft` 落「过程会话」并保留手动 Start 兜底，均不变。落点规则是纯前端
  内存态（`discussion-view` 的 `defaultDiscussionTab` / `correctActiveTab`），
  不新增协议字段，也不调整服务端时序。
- 前端：discussion-view 的“+”打开一个内联创建表单（类型下拉 / goal / context）；
  ...
  **Start** 按钮是一个手动兜底，**仅当调研已结束/中止且编排
  ...
  表单关闭时重置。**左侧列表**包含：
  一个头部**折叠/展开**开关（状态经 `c3.discussionListCollapsed` 持久化，跨页面切换与刷新后保持），收窄面板并隐藏次要行信息（类型 /
