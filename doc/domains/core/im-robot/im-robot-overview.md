...
- **负责人:** maintainer
- **状态:** 活跃
- **依赖:** 位于 `~/.c3/c3.db` 的本地 SQLite 存储(`im_robots` / `im_robot_threads` / `im_robot_context_turns` / `im_robot_turns` / `im_identity_challenges` / `im_identity_bindings` / `im_group_workspace_scopes` / `im_identity_audit`);`agent-session` 的 run 生命周期(`robot` 会话种类 + 无人值守回合);`permission-gateway` 的 `robot` 门;`agent-config` 的 vendor 与 agent(含组引用)解析;服务端统一出站 HTTP 通道与代理解析;`auth` 域的 `user_workspace_scopes` 与 policy epoch。
- **被依赖方:** 无。没有其它域读取机器人数据。
- **exposes-api:** true —— 十五条 WebSocket 消息(机器人名册读写、启用、外发确认、回合审计,以及 IM 身份绑定/挑战/撤销与群可见范围管理),不新增 HTTP 路由,不进外部 MCP 工具目录。
- **ADRs:** [0046](../../../architecture/adr/0046-im-robot-outbound-authorization.md)、[0047](../../../architecture/adr/0047-robot-local-reads-scoped-to-run-root.md)、[0048](../../../architecture/adr/0048-robot-im-context-as-bounded-local-persistence.md)、[0049](../../../architecture/adr/0049-im-identity-binding-and-call-level-scope.md)
...
- **能力上限 L0–L3。** L0 受控播报、L1 只读问答、L2 定向作答、L3 受控写入分别有独立授权面。
  六个 c3 MCP 写工具只有在管理员逐项勾选且调用级工作区作用域通过时执行;L2 capability grant、本地
  工具运行根与沙箱权限均不由该勾选替代。
- **默认停用,启用是一次独立的授权动作。** 创建不启用;启用需要管理员、需要凭据、需要一次记录在案的
  外发范围确认。缺任何一条,服务端拒绝启用。
- **只发最终回答与固定进度提示。** 工具调用、工具结果、中间推理、文件内容一概不外发;长回合执行期间
  只发已注册的固定进度提示(已收到/步骤/仍在处理)——这是结构性的,不是开关。
...
- **IM 身份绑定与调用级作用域。** Web 本人发起挑战、私聊消费令牌;未绑定只收固定引导。Conversation 含 `scope_hash`;L1 只读与显式勾选的六个 c3 MCP 写工具每次调用重算个人范围 ∩ 群白名单。
