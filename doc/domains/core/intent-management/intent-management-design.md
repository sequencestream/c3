...
codex driver 转译成其原生的 streamable-HTTP 服务器条目;cursor 边界写入其 on-disk `mcpServers` 配置(外部 HTTP 服务器)。
...
- **厂商转译。** 中立的远程 MCP 描述符(type、url、可选的 bearer-token 环境变量)
  ...
  streamable-HTTP MCP 形式,cursor 边界写入其 on-disk `mcpServers` 配置(外部 HTTP 服务器);三者指向同一条回环路由。
