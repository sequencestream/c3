...
意图列表提供两条并存的创建路径：独立意图会话仍通过智能体的 `save_intents` 批量提案与人工确认；“增加意图”则通过 `create_intent` 立即登记一条固定初值的空白 draft。创建结果中的服务端 ID 只负责精确 UI 落点，账本状态仍以 `intents` 快照为准。结果和快照允许任意顺序到达，工作区切换、失败或目标消失时必须丢弃落点。
