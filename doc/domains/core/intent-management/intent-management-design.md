...
- **运行变体**: 运行循环的追加系统提示词 / 禁用工具 / MCP 服务器 / 网关等选项;为判定器提供一个无工具的 one-shot
- **运行时 kind + 启动器**: 运行时上的运行 kind(`session` 或 `intent`);一个共享启动器
- **WS 分支 + 编排**: 八个消息分支;沟通会话的 viewer 管理;开发轮助手 + 自动化广播
...
- **意图数据:** app 保存按工作区路径为键的 intents,
  ...
  状态。
- **工程进度:** 意图详情头部按意图字段派生只读进度。意图、[规范]、工作、[PR]、[评审]、[修复]
  依次展示,SDD 关闭时省略规范;有效规格模式为 `fast` 且该意图尚无规范数据(`specPath`、
  `specSessionId` 皆空)时同样省略 —— fast 不走规范前置,展示一个永不发生的
  「规范 / 未开始」只会误导;反向生成的规范落地后规范段回来,此时确有批准步骤可看。
  仅 worktree 工作区在末尾展示 PR、评审、修复。工作是否完成只取决于
  ...
  已关闭/失败,其余(有行且聚合 `reviewing`)为进行中。
  评审段在 PR 之后,仅 `needsReview(impactLevel)`(`L5` 跳过首次评审,未定级按需要评审的
  失败安全处理)且存在 PR 行或已有评审结论时出现,状态由 `reviewStatus` 映射:未评审为
  未开始,`pending` 为进行中,`approved` 为已完成,`rejected` 为已关闭/失败。修复段紧随评审,
  仅 `fixSessionId` 非空或 `fixStatus` 非 null 时出现(修复只发生在被拒评审之后),状态由
  `fixStatus` 映射:未修复为未开始,`pending` 为进行中,`fixed` 为已完成。进度条在窄屏横向滚动。
...
交付支的下拉是**候选过滤后**的视图,不是交付账本的全量:只列分支已就绪、分支名非空、且未被写入闸门(RM-R41)阻塞的交付。交付账本只增不减,不过滤的话已结账与验证期的交付会随版本推进单调累积,用户每次新增意图都要在一堆无效项里翻找;而验证期与终态的交付即便选中,建出的意图一开发就会被写入闸门拒。判据取写入闸门的同一份共享实现,弹窗不重述状态清单 —— 两份清单正是「一个入口放行、另一个入口拒绝」的来源。候选随交付广播实时收缩,而「基准完整」判的是**选中项仍在候选集内**而非 id 非空,因此弹窗开着时已选交付被推进到验证期,该项消失的同一刻提交按钮禁用。这是展示口径而非门禁,服务端解析基准的规则不变;关联入口 `IntentLinkDeliveryDialog` 排的是终态,口径与此不同 —— 给一条已有意图补挂关联不必然立刻开发,验证期补挂是合理操作。
