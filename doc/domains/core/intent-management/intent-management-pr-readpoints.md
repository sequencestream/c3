...
| 文件:符号                                                  | 字段                      | 分类  | 用途                                                                                                       |
| ---------------------------------------------------------- | ------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `server/src/git.ts:674-712`                                | `prId`                    | 写    | `createGhPr()`:解析 `gh pr create` 输出 `/pull/(\d+)$` 得 number;base 必传(`--base`)                       |
| `server/src/git.ts:720-760`                                | `prId`                    | 写    | `createGlabMr()`:解析 `glab mr create` 输出 `/merge_requests/(\d+)` 得 number;base 必传(`--target-branch`) |
| `server/src/git.ts:769-781`                                | —                         | 派生  | `createForgePr()`:按 forge 路由创建;base 必传,由调用方解析后传入                                           |
| `server/src/git.ts:562-584`                                | —                         | 门禁  | `hasDiffAgainstBase()`:相对有效 base 的差异闸门(fetch 后优先远端 ref,不可解析即拒绝)                       |
| `server/src/git.ts:824-845,847-862,864-871`                | `prStatus`                | 读    | `getGhPrStatus`/`getGlabMrStatus`/`getForgePrStatus`:远端查状态                                            |
| `server/src/git.ts:895-910,916-931,938-945`                | `prId`                    | 写    | `closeGhPr`/`closeGlabMr`/`closeForgePr`:关 PR/MR                                                          |
| `server/src/features/intents/write-cores.ts:129`           | `prId`                    | 门禁  | `createPrForIntent()` 幂等守卫:已有 `prId` 拒绝重建                                                        |
| `server/src/features/intents/write-cores.ts:212-213`       | `prId`/`prUrl`/`prStatus` | 写    | 手动 create_pr 成功后 `setPrInfo(...,'reviewing',prUrl)` + `pr_created` 日志                               |
| `server/src/features/intents/write-cores.ts:215-226`       | `prId`/`prUrl`            | 写    | `runServerSidePrCreate()` 发布 pr:create 事件(携带已解析 base);返回帧带 `prId`/`prUrl`                     |
| `server/src/features/intents/write-cores.ts:291-305`       | `prId`/`prUrl`/`prStatus` | 写    | 意图取消:`closeForgePr` + `setPrInfo(...,'closed',prUrl)`                                                  |
| `server/src/features/intents/dev-cleanup.ts:191`           | `prId`                    | 门禁  | 手动 Start Dev 收尾:已有 `prId` 跳过重建                                                                   |
| `server/src/features/intents/dev-cleanup.ts:200-213`       | `prId`/`prUrl`/`prStatus` | 写    | 收尾 `createForgePr`(显式 base)+ `setPrInfo(...,'reviewing',...)`                                          |
| `server/src/features/intents/dev-cleanup.ts:222-232`       | `prId`/`prUrl`            | 写    | 收尾 `runServerSidePrCreate` 发布事件(携带已解析 base)                                                     |
...
| `server/src/features/intents/queue-dev-actions.ts:422-452` | `prId`/`prUrl`            | 读/写 | 队列 `createPrForIntent()`:调 `createForgePr`(显式 base)并回传 number/URL/base                             |
...
| `server/src/features/intents/create-pr-handler.test.ts:26-34,156-234,318-341`                                                                                                                                                                                                             | fixture           | createForgePr / hasDiffAgainstBase mock、base 贯通(含 forge override)与目标分支不可解析拒绝、响应帧 / 幂等守卫断言                                     |
...
| `server/src/wiring/run-domain-subscriptions.test.ts:104-106`                                                                                                                                                                                                                              | fixture           | createForgePr mock                                                                                                                                     |
| `server/src/git.test.ts:218-279,297-438`                                                                                                                                                                                                                                                  | fixture           | hasDiffAgainstBase(远端优先/本地回退/不可解析拒绝)、createGhPr `--base` 与 createGlabMr `--target-branch` argv、getGhPrStatus / getGlabMrStatus 期望值 |
