...
| Automation Queue(自动化队列)         | 按工作区运行的确定性调度内核(每次一条意图),对账 → 选择 → 开发 `automate` 意图、判定完成、提交/推送(RM-A1–RM-A20)                                 |
...
- **自动化队列(确定性调度内核)。** 一个按工作区运行的调度器,由**定时 tick 全量对账 +
  ...
  **资格:** 当 `automate` 且 `status ∈ {todo, in_progress}` 且所有已知 `dependsOn` 条目均为
  `done`(worktree 模式下还须其 PR 已合并)时,一条意图具备资格;SDD 开启时还要求 spec 已批准,
  ...
  **失败隔离:** 所有内核发起的 run 都被 await 并捕获异常(RM-A16);一次失败只影响该意图 ——
  ...
  的下游仍被依赖闸门挡住,既不跳过也不放行(RM-A6、RM-A17)。**权限对等:** 开发 turn 期间的权限
...
| RM-R5  | 一条意图只通过 `save_intents` 写入账本,其人工授权是**用户在对话中的明确文字确认**:智能体每轮(首次拟定与任何修改后)都必须把本轮全部意图的完整内容重新列出并等待确认,确认后才调用工具;工具一经调用,处理器直接持久化,不发 `permission_request`、不等待决定、不登记等待人工介入事件,过程中不出现任何确认弹框。异议不是确认。两个厂商都通过**同一条回环 HTTP MCP 路由**到达同一个处理器,确认语义不因厂商而分叉;意图网关将 `save_intents` 直接放行到处理器,绝不能为其弹出提示。校验失败 / 存储不可用 / upsert 失败 → 整批不写入,并如实告知智能体。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
> 2026-07-31:队列的驱动方式由「只被 `run:settled` 推动的内存状态机」换为「定时 tick 全量对账 + 事件合并标脏」的确定性内核。
> 硬闸门一律未放宽(RM-A12 全局并发、依赖 + worktree 模式下依赖 PR 已合并、SDD `specApproved`、RM-A11 人工决策守卫、续跑预算、workspacePath 工具闭包绑定),
> 且**未新增任何自动标 `done` 的路径**(RM-R9 例外范围不扩大)。`start_workflow` / `stop_workflow` / `workflow_status` 的外部契约向后兼容。
...
| RM-A3  | 队列**逐个**开发候选意图,顺序为**先优先级(P0→P3)再最早优先**。候选 = `automate` 且 `status ∈ {todo, in_progress}`。闸门按固定顺序求值,每一个都产出一个**结构化原因码**,绝不静默跳过:park → 强制跳过 → SDD `specApproved` → 依赖(worktree 模式下还要求依赖的 PR/MR 已 `merged`)→ 退避 → 冷却 → RM-A12 全局并发闸门。SDD 开启而 spec 未批准会显式产出 `blocked_spec_not_approved`,**不得**表现为「跳过」或让队列显示成功完成。未知的依赖 id(跨项目/已删除)不阻塞。当仅仅因为依赖的 PR/MR 状态已过期而无人可选时,队列发起 RM-R28 的一次性后台同步,并在其完成后重新对账。                                                                                                                 |
| RM-A4  | 一个开发 turn 结束后,一个独立的、**无工具**的完成判定器会读取该意图 + 工作会话的最后一条助手消息 + **代码改动证据**(未提交工作的 `git diff HEAD --stat` 以及近期提交的 `git log --oneline`,两者都**具备多仓库感知**),并返回 `done` / `in_progress` / `stuck`,**按 stuck → done → in_progress 的顺序判定**。完成情况**主要根据智能体的报告**来判定;改动证据是**支持性的佐证,而非前提条件** —— 提交/推送是 c3 自己在 `done` 判定**之后**才做的工作(RM-A5),因此没有 diff/log 本身不能一票否决完成。**空证据从不是一个 stuck 信号**。                                                                                                                                                     |
| RM-A5  | 在 `done` 时:队列提交任何未提交的改动(`feat: <title>`,当树已因智能体已提交而干净时跳过)并**总是推送**,然后通过感知代码托管平台的变更请求能力创建一个 PR/MR,再把该意图标记为 `done`。显式的工作区 `forge` 值 `github` 和 `gitlab` 会覆盖仓库来源检测;`auto`(或未设置)保留检测。GitHub 使用 `gh`;GitLab 使用 `glab`;产生的标识符与链接会被记录到既有的 PR/MR 追踪字段中。这是队列的自动 `done` 路径(两个自动 `done` 路径之一;进入协调 RM-R18 是另一个)——**调度内核不新增任何自动标 `done` 的路径**。一次被 **pre-commit lint 钩子**阻挡的提交会先由一次单独的开发智能体修复来**自愈**(RM-A13);任何**其他**提交/推送失败都是该意图的一次失败(RM-A6)。                                    |
| RM-A6  | **失败隔离取代整队停止。** 任何异常结束都只记在**那一条意图**头上,队列本身不停:开发 turn 出错;launch 抛异常;判定器返回 `stuck`;超过续跑上限(RM-A8);一个被拆除的 turn 携带未回答的 `AskUserQuestion`(RM-A11);**非 lint 类**的提交/推送失败;或 pre-commit lint 在单次自愈后仍未通过(RM-A13)。一次失败使该意图的连续失败计数 +1 并进入**指数退避**(30s 起、逐次翻倍、上限 15 分钟),退避截止前不重试;**连续第 3 次失败进入 park**。park 的意图不再被自动启动,但它**不是 `done`** —— 依赖它的下游继续被依赖闸门挡住,任何队列控制动作都不得越过。与之无依赖关系的其他意图照常被挑选和推进。真实推进到新的有效阶段后,连续失败计数与退避被清零。                                              |
...
| RM-A12 | **全局并发闸门(工作区范围)——语义不变。** 在挑选下一个候选意图之前,队列检查同一工作区是否有**任何** `in_progress` 意图拥有一个**真正在运行**的工作会话(`lastWorkSessionId` 非空**且**该运行存活)—— 包括手动启动(非 `automate`)的意图。存在这样一个时,队列不启动新意图:若它本身是一个可选中的候选就挂接观察(RM-A10),否则如实呈现 `awaiting_gate` 并逐条记录被该闸门挡住的候选。一个**悬空**的会话(存在于磁盘上但未运行)**不**阻塞。该闸门防止同一工作树上出现会在文件修改上冲突的并发工作会话,**不因 park、强制跳过或任何人工队列动作而放宽**。                                                                                                                                         |
...
| RM-A17 | **park 与解除。** 连续失败达上限、需要人工决策、或权限等待超窗的意图进入 **park**:不再自动 launch,并携带结构化 `park_reason` 与可展示摘要。**park 不是 `done`** —— 依赖它的下游继续被依赖闸门挡住,**既不跳过也不放行**;无依赖关系的其他意图继续参与选择。用户 **unpark** 清除 park 标记与原因并允许下一轮重新评估(同时清零连续失败,使解除后拥有完整重试预算);**成功推进到新的有效阶段**后连续失败与退避自动清零。                                                                                                                                                                                                                                                                     |
...
- **Claude Agent SDK** —— 沟通智能体使用一个禁用工具列表、一段追加的系统 prompt 预设;c3 自己的
  ...
  经由两个厂商共用的**回环 HTTP MCP 路由**接入(Claude 边界把中立描述符转译为 SDK 的 HTTP MCP 配置,
...
- **branchName / latestCommitHash / prId / prUrl / prStatus** —— 一条意图上的 Git/PR 追踪字段
  ...
  由手动会话结束清理(RM-R26)、自动化队列、定期 PR 协调能力,以及 **`pr:operation` update
