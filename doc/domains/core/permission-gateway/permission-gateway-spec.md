...
See [permission-gateway-models.md](permission-gateway-models.md) for full attributes.
...
permission concept into it (ADR-0011, the approval bridge). Two reference mechanisms exist:

- **Claude — in-loop.** The SDK's blocking sensitive-tool callback IS the suspend point;
  "write back" is resolving that promise. c3 blocks inside its own process (PG-R2 verbatim).
- **Out-of-loop vendor.** The vendor signals a pending permission as an out-of-band event; the
  bridge suspends a promise keyed by the vendor's permission id, raises the `permission_request`,
  and on the human decision writes the result back to the vendor's permission endpoint
  (allow / deny). Because the vendor does not own the lifecycle, the bridge takes on the
  responsibilities the vendor does not (PG-R11): timeout, write-back retry + stale-404 handling,
  event re-subscribe (there is no "list pending permissions" endpoint to reconcile against), and
  reply idempotency. A reply c3 never authored is audited as pre-approved (PG-R12).

Both reach the **same** browser registry (`permission_request` + the wait-for-decision path), so
the human experience and the default-deny invariant are identical across vendors.
...
- **agent-session** supplies the send channel (to push the request) and the transport;
  it calls the gateway from the SDK's sensitive-tool callback.
