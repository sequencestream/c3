...
exec arapuca run \
  --seccomp baseline \
  --cwd "<worktree>" \
  --env "CODEX_HOME=<tmpDir>/home/.codex" \
  -v "<tmpDir>/home/.codex":rw \
...
- `--cwd` 显式设为宿主 worktree；进程同路径运行，cwd 天然一致。
...
- **CODEX_HOME**（codex）：指向 `<tmpDir>/home/.codex`（在已放行的 worktree 内），并作为独立 rw volume 传入，满足 macOS profile 对启动期 canonicalize 的授权；避免 arapuca 默认临时 HOME 被 codex 拒绝创建 PATH helper。
...
- **认证经 env 注入，不碰订阅 / keychain**：provider 认证由 driver 通过子进程 env 传入（claude 的 `ANTHROPIC_BASE_URL` / `ANTHROPIC_API_KEY` / `ANTHROPIC_AUTH_TOKEN`，codex 的 provider 三元组）；arapuca 保留普通 env（只剥 `LD_*` / `DYLD_*` / `ARAPUCA_*`），故认证进得去。deny-by-default 下宿主订阅文件 / keychain 不放行、不可见，作为严格隔离要求。
