...
exec arapuca run \
  ...
  --env "CODEX_API_KEY=$CODEX_API_KEY" \   # codex 分支;claude 分支改透传 ANTHROPIC_* 三元组
...
- **claude 运行时目录**：Claude Code 硬编码 `/tmp/claude-<uid>`（shell-snapshot / IPC），不尊重 TMPDIR 且 arapuca 锁定 TMPDIR，故 wrapper 预建该宿主目录并按 canonical 路径放行；逐用户共享（非逐 run），放行但不清理，codex 不使用。
...
### 9.1 已知限制:transcript store 定位随 sandbox 切换错位(待修)

历史 session 展示会话记录一律从 vendor native store 读(c3 不另存):`select_session` → `works/index.ts` `loadHistoryForVendor` → codex 走 `CodexSessionStore.read`、claude 走 `loadHistory`。**codex 读取端把 sessions 根硬编码为宿主 `~/.codex/sessions/`(`session-store.ts` `os.homedir()`),不看 CODEX_HOME**;而 sandbox 下 rollout 实际写在 `getSandboxCodexHome(workspace)`。两者分处两地,后果:

- sandbox 里产生的 session:历史前端**读不到**(read 返回空)。
- 非 sandbox 老 session 切到 sandbox:历史仍可查看(读取端恒读 `~/.codex`),但 sandbox 内 resume 走 sandbox-home → `no rollout found`。

此问题 **vendor 中立**:claude 有同构变量 `CLAUDE_CONFIG_DIR`(默认 `~/.claude`,transcript 在 `<dir>/projects/<encoded-cwd>/`),只是当前 claude 沙箱化尚未跑通,还未给它设 per-workspace config dir,真跑通后同样成立。

方向(未实施):① 读取端对 sandbox 工作区同时扫宿主 `~/.codex` 与 sandbox-home 兜底(对存量/切换鲁棒);② session fact 加 vendor 中立 `storeScope: 'host' | 'sandbox'`(类比已冻结的 `vendor`),读取/续接按冻结定位;③ 把 sandbox 的 `CODEX_HOME` / `CLAUDE_CONFIG_DIR` 抽象为"每 vendor sandbox 数据根"。
