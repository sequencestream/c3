...
| `kernel/agent-config/` `resolveSandboxAgent` | sandbox run 的 agent 选择：角色专用 → sandbox 默认 → 首个启用（同 vendor 优先）；其 vendor 决定入口命令。                               |
...
11. sandbox 启用时 run 保留其正常解析出的 agent；仅当该 run **未显式绑定** agent 且默认解析落在 system agent 上，才按 `resolveSandboxAgent` 的 sandbox 角色配置换绑。被选 agent 的 vendor 决定入口命令（宿主 PATH 中的 CLI）与 provider 接线。system agent 不再构成 sandbox 冲突。
...
sandbox run **不做专门的 agent 挑选**：它保留该 run 正常解析出的 agent，沙箱只负责把该 vendor 的 CLI 包进 arapuca。唯一的例外是 sandbox 角色配置：

- run **显式绑定**了 agent（session 上有绑定）：直接使用，system / custom 一视同仁，不弹窗、不换绑、不因缺少 custom agent 而失败。
- run **未显式绑定**且默认解析落在 system agent 上：按 `resolveSandboxAgent(kind, vendor)` 换绑到工作区配置的 sandbox 角色 agent，顺序为 `sandbox<role>AgentId → sandboxDefaultAgentId → 首个启用 agent（同 vendor 优先，再任意）`。同 vendor 优先是为遵守 session vendor 冻结约束（真实 session 拒绝跨 vendor 换绑）。解析不到（无任何启用 agent）**不再是 hard-fail**：run 保留默认 agent 直接进沙箱。

候选准入是「enabled」而非「enabled custom」：system agent（订阅态）是合法的 sandbox agent，因为 wrapper 会为其打开宿主 keychain（见 §9）。归一化（`normalizeSandboxRoleId`）与系统设置页的 sandbox 角色下拉同规则——可保存、可选择 enabled system agent，仅 disabled / 不存在的引用被重置为 ""。
...
arapuca 环境变量为 **deny-by-default**:除自身管理的 `HOME` / `PATH` 外**不继承父进程 env**,只有显式 `--env KEY=VALUE` 传入的才进沙箱(裸名 `--env KEY` 被拒:`invalid --env, expected KEY=VALUE`)。因此 c3 wrapper 必须把 codex/claude 所需的每个变量(CODEX*HOME、provider 凭证等)逐个 `--env` 显式透传,不能依赖继承。

**deny-by-default 的唯一标准例外是代理变量组**:宿主存在 `HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY` / `NO_PROXY`(含小写变体)中任一非空键时,wrapper 追加 `--allow-proxy-env`,由 **arapuca 自身**转发这组标准变量——c3 不为它们生成 `--env`,也不做逐变量白名单或值脱敏。宿主未设代理时该参数不出现,env 面与此前完全一致。（早期文档误称"arapuca 保留普通 env、仅剥 `LD*\_`/`DYLD\_\_`/`ARAPUCA\_\*`",与实测 0.2.4/darwin 不符,已更正。）
...
2. **宿主 PATH**：使用方自己安装的 arapuca，版本不受 c3 控制。文件名按平台解析：POSIX 用裸名 `arapuca`；Windows 按 `PATHEXT`（未设时用默认 `.COM;.EXE;.BAT;.CMD`）依次尝试 `arapuca.exe` 等后缀名，裸名作为最后兜底——常规 Windows 安装提供的是 `arapuca.exe`，只查裸名会把已安装的宿主 arapuca 误判为缺失。
