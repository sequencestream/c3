...
- 位置:`c3.db` 的 `workspaces` 表(工作区)与 `session_configs`(每会话模式);活动会话在
  `system_configs` 的 `state.activeSessionId`。
- 惰性加载到模块缓存中;每次变更只写受影响的那一行。
- **故障软化:** 数据库不可用(或写入出错)会回退到空状态并记录日志;
  c3 仍必须能启动(ADR 0004,AVAIL）。
- 从列表移除工作区只置 `registered=0`:id 是其配置所依附的主键,删行会让配置成为孤儿,
  重新添加同一目录也就换了身份。
