...
- **SR-R4**: 工作区内的会话是从 c3.db 中的一个 session-metadata 投影缓存(ADR-0013 修订)列出的,按 `last_modified` 最新优先。该投影是一个可重建的缓存 —— 原生厂商存储仍然是存在性、历史和标题的事实来源。每个被列出的会话条目携带其所属的 `vendor` 标记,以及一个新增的 `state` 字段(`born`/`alive`/`stale`/`orphaned`/`ghost`）。线上的 `sessionId` 保持为厂商**原生**id(线上的 c3 命名空间是一个推迟到后续阶段的 ADR-0013 阶段）。按厂商的枚举访问器是重建/惰性校验来源;非 Claude 的行既可以在绑定时被填充,也可以通过重建该厂商本地 transcript 的枚举来填充(Codex 扫 `sessions/` 的 rollout,Cursor 读 `~/.cursor/chats/` 下各 chat 的 `meta.json`),并按 SR-R13 打上 `last_modified` 戳。
- **SR-R5**: 权限模式是**按会话**的、被持久化的,默认值为该项目的按厂商默认模式(该默认模式本身又回退到每个厂商各自的默认模式令牌）。变更活跃会话的模式(`set_mode`)只影响该会话,并会针对该会话的厂商模式目录进行校验。选中会话下发 `session_selected.mode` 前同样按该厂商目录校验:合法历史值原样回显且不写回;目录外 token(例如 Cursor 会话上误存的 `default`)降级为工作区该厂商默认(再不行则 catalog `defaultToken`),并写回纠正值,保证标题栏下拉非空且与生效策略一致。
...
- **SR-R15**: **显示分类可以覆盖多个真实 kind —— 「规范」= `spec` + `spec_review`,列表与计数共用同一组过滤规则。** 客户端请求的 `sessionKind` 是它要看的**分类**,不必等于行的真实 kind:唯一的聚合分类是 `spec`,它同时列出规格撰写与规格评审会话(评审没有自己的分类)。聚合发生在**投影查询**里(`session_kind IN (...)`),因此首屏、加载更多与窗口刷新看到的是**同一条**全局 `last_modified` 倒序,不是两段各自有序的结果拼接 —— 否则同一次分页会漏项或重项。**列表与角标必须共用这组过滤规则**:不允许出现列表里看得到却没计入角标、或计入角标却列不出来的情况;每个真实会话在聚合角标里**只计一次**。为线上形状兼容保留的 `spec_review` 计数字段恒为 0,避免新旧客户端把同一批会话在顶栏总数里算第二遍。返回行始终携带**真实** `sessionKind` 与所有者字段:分类只是查询与展示口径,不改变会话的种类、所有权或权限语义(评审会话的恢复入口见意图管理 RM-R37)。
...
- **各厂商的原生会话存储** —— 会话枚举与历史检索;重命名/删除按该厂商的会话能力子台账供给
  (Claude Agent SDK 全支持;Cursor 的磁盘会话库只读)。
