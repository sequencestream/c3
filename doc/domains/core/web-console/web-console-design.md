...
- **首屏只装当前视图**——App 容器是唯一的装配边界,业务页面与低频全局组件
  ...
  等待期间渲染统一的轻量占位(页面级占住内容区、弹窗级占住遮罩层),加载失败
  渲染统一兜底且**不自动重试**——弹窗兜底不吃指针事件,应用其余部分照常可用。
  条件回到关闭时包装器卸载,再次打开复用已解析组件与浏览器缓存。语言包(预编译
  的多语言消息)体量远大于业务代码,单独成 chunk 与业务代码分家缓存,但仍随首屏
  静态加载——按语言动态加载是另一件事。
