...
| Session list           | The 「会话」tab's left column — the current workspace's unified session list (no workspace tree). It renders six `session_kind` tabs (work / intent / spec / discussion / schedule / tool) with running-count badges from the `session_metadata` count path; work and intent show real rows in this phase, the other four are disabled placeholders. Rendered only on the sessions tab (the intent tab's left column is the intent list; both share the right-side chat console). Owns session pagination + prompt/confirm UX, signalling work-session create/rename/delete. The header carries only ＋ New session; the intent entry moved to the top-bar tab nav, so the session list no longer renders the 「需求录入」💡 shortcut (WC-R18)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
