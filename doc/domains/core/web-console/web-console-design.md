...
- **App header** — 顶部栏:承载工作区切换器(最左侧;桌面与移动端两处切换器同时挂载,故新增工作区弹框由顶栏单实例持有,开关受上层控制,手动「＋」与冷启动引导共写)、**项目配置入口**按钮(紧随切换器之后,打开当前工作区的项目级配置,未选中工作区时禁用)、**tab 导航**(数据驱动的「会话」/「需求」,当前项高亮,工作区不存在前禁用)、设置入口,以及连接状态。不携带会话标题或权限模式下拉框——它们已移到会话标题栏(WC-R9)
...
- **Permission prompt** — 单个权限区块。当可操作时:一个 AskUserQuestion / AskQuestion 应答面板或一个允许/拒绝提示(持有本地应答草稿,发出应答/提交-ask 信号)。当未决但不可操作时:一条单一的静态历史行(无按钮、无结论)
...
- **Settings panel** — 系统设置页,配置按 Agent / Runtime / Security / General 四个 Tab 分组(桌面可见 Tab 导航,移动端横向滚动)。每个 Tab 维护独立草稿:面板打开时以服务端设置深拷贝生成「已提交基线」,单一 draft 按字段白名单切片、与基线做深层值比较得出各 Tab 脏状态(不使用全局 dirty 标志)。每个 Tab 提供独立保存按钮与未保存标识;保存某 Tab 时仅用该 Tab 白名单字段(经其转换:Agent 回写 order_seq、Runtime 同步 proxy 表单并丢弃空名沙箱、Security 派生 auth.enabled)覆盖最新已提交基线,构造完整 `SystemSettings` 发送 `save_settings`,不携带其他 Tab 草稿,且保存后面板保持打开。面板打开期间的设置回推按字段归属合并:首次打开整体播种,之后仅刚保存的 Tab 与干净 Tab 重播种,脏 Tab 保留用户草稿、只同步即时持久化字段(UI 语言、账号列表 / 管理员)。切换存在未保存修改的 Tab 时二次确认(ConfirmDialog),确认后仅切换、既不保存也不丢弃草稿,返回可继续编辑。智能体表的图标列是一个手动文本输入,搭配一个 emoji 选择器(两者绑定同一个图标草稿字段)。逐项目控件(默认模式、开发技能、轮次、语音字符数、共识)已从设置面板移除,转移到工作区设置页面
...
会话分页;设置面板中按 Tab 分组的可编辑设置草稿(单一 draft 按字段白名单切片 + 已提交基线,派生各 Tab 脏状态)与当前激活 Tab。
