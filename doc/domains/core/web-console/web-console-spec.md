...
- **WC-R27**: **规格操作引导 + 误批准防护 + 规格标签页可见性。** 五条纯前端交互约束叠加在四状态主按钮(WC-R25)之上,均不触及规格生成、会话创建或批准状态机。**(a) 自动切换到规格会话——由会话创建成功驱动,而非定时。** 点击 `Write Spec` 会记下当前需求 id 与此刻的 `specSessionId`;当且仅当**该需求的** `specSessionId` 回填为一个非空的**新**值(即撰写会话确实创建出来了)时,详情才激活到**规格会话**标签页。创建失败、长时间没有回填、或回填的仍是旧值都不切换,页面停在当前可见处;用户在此期间导航到另一个需求即放弃该待切状态(不会把选中项抢回来,也不会被新需求的回填触发)。**不用固定延时:** 会话创建是异步且可失败的,而空白需求此刻两个规格标签页按 (d) 尚不可见,定时到点的切换会被可见性判定吞掉。**(b) Approve-Spec 延迟门禁。** 该门禁在当前会话中点击 `Write Spec` 时被激活(锚定在触发时间戳上,而不仅仅是 `specPath` 存在与否)。在激活期间,若按钮本应推进到 `Approve Spec`,它会保持隐藏,直到自触发起过去十秒,以降低在阅读生成的规格之前就批准的风险;剩余的等待时间在页面重新进入 / 状态刷新 / 组件重新挂载后依然保留,不会被这些操作绕过。一个已有规格但**并非**在本会话中撰写的需求不会被激活,因此其 `Approve Spec` 会立即可见。防误批准的十秒窗口是固定的,不可由用户配置,且没有额外增加确认对话框。**(d) 规格标签页的数据门槛。** **规格**与**规格会话**两个标签页当且仅当该需求已有规格数据(`specPath` 或 `specSessionId` 任一非空)时渲染,与工作区是否开启 SDD **无关** —— SDD 只说明「能写规格」,不代表「已经有规格」,而一个两者皆空的新建需求点进这两个标签页既无内容也无会话、无法启动任何东西,是纯粹的死入口。这与同类标签页的既有规则同构(评审见 WC-R29,工作会话取决于 `lastWorkSessionId`)。只有 `specPath` 的历史需求(撰写会话早已结束)仍保留这两个入口,已有规格内容与会话的读取能力不受影响;条件在浏览期间失效时按既有隐藏标签页规则回退到「需求」标签页。**(e) fast 需求不渲染 `Write Spec`。** 有效规格模式为 `fast` 的需求按定义不先写规格,主按钮在 `raw` 态直接呈现 `开始工作`(WC-R25、RM-R43),标题栏不提供「编写规范」;规格标签页仍只由 (d) 的数据门槛决定,不因 fast 而额外显示或隐藏。**(c) 有区分度的语义色。** `Write Spec` 与 `Approve Spec` 两种状态携带稳定的、无障碍可区分的语义色——`Write Spec` 保持主色(生成色),`Approve Spec` 使用成功/确认色——各自带有稳定的无障碍标签,使两个操作在视觉上与辅助技术上都能区分。待处理的自动切换 / 门禁计时器会在组件卸载时以及切换所选需求时被清除。所有文案均走 i18n。
- **WC-R29**: **规格评审会话的可见性:详情页「评审」标签页 + 会话页「规范」合并入口 + 只读聊天列。** **(a) 评审标签页。** 需求详情在**工作区开启 SDD 且该需求存在 `specReviewSessionId`** 时,于规格会话标签页之后渲染一个「评审」标签页;缺任一条件都不渲染(不为没有评审会话的需求显示空态入口,访问详情页也**不会创建**评审会话),条件在浏览期间失效时按既有隐藏标签页规则回退到「需求」标签页。标签内联运行中状态点与需求/规格/工作会话三类同构(非 idle/未知才显示)。**(b) 打开路径。** 选中该标签页以当前 `intentId` 发送 `open_spec_review_session`——客户端**从不**指名会话 id,目标一律由服务端按该需求保存的 `specReviewSessionId` 解析并恢复 `spec_review` 只读运行时(携带被审需求与内容指纹);沿用单一当前会话模型:只有全局当前会话与该 id 对齐后才渲染聊天列,id 异步回填补发一次,已对齐不重发。**(c) 会话页「规范」是合并入口。** 会话页**不新增**「评审」分类:「规范」标签页的服务端列表查询把 `session_kind ∈ {spec, spec_review}` 视为同一结果集,在**同一条**全局更新时间倒序上做排序/分页/刷新(首屏、加载更多、窗口刷新都不得漏项或重项),角标为两类运行中会话之和且每个会话只计一次;兼容保留的 `spec_review` 计数字段不形成第二个可见口径,也不在顶栏总数中重复计算。列表行保留**真实** `sessionKind` 与 owner:点击 `spec_review` 行必须携带投影的 `ownerKind='intent'` 与非空 `ownerId` 改发 `open_spec_review_session`(不离开会话页,回复的 `session_selected` 绑定右栏),**绝不**交给通用 `select_session`(其冷会话恢复会构造 `work` 运行时,破坏只读边界);owner 缺失或非需求所有时 fail closed——报错、不绑定、不恢复,且不回退到通用选择。评审行不获得改名/删除能力,其溯源解析为「该需求详情的评审标签页」,与规格撰写行的溯源相互区分。**(d) 只读聊天列。** 详情页评审标签页与会话页当前真实 `sessionKind='spec_review'` 的会话共用聊天列的**整列只读门**(与 `show-input` 不同,后者只隐藏输入框):隐藏输入框、不渲染待发队列、状态栏保留运行状态文字与刷新但不提供 stop/continue、历史与权限请求**完整回放**但不渲染 allow/deny/ask 回答控件;只读分支即便被程序化触发也不上抛任何变更事件。只读只是呈现层——服务端按 `sessionKind` 的续跑门禁才是防线(见 RM-R37)。所有文案均走 i18n。
