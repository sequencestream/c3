...
- **WC-R6**: 连接状态(`connecting` / `open` / `closed`)始终对用户可见,并且是运行日志页的入口(WC-R31)。
- **WC-R7**: 控制台绝不会代替用户执行工具或做出决策——它只发送用户明确选择的内容。
- **WC-R8**: 顶部栏的**工作区切换器**(最左侧)标识唯一的全局**当前工作区**,是添加/切换/移除工作区的唯一入口:一个添加控件用于新增一个工作区(`add_workspace`,路径来源见 WC-R8a),下拉列表列出所有工作区(名称 + 路径,按服务端返回的最近访问顺序排列)用于切换当前工作区,每一行都可将其移除(二次确认 → `remove_workspace`)。**新增与移除工作区仅限管理员**(AUTH-R10):服务端在 `add_workspace` / `remove_workspace` 前设有管理员校验(非管理员 → `auth.adminOnly`,未认证 → `unauthenticated`),切换器对非管理员隐藏添加控件与每行的移除控件(由 `ready` 消息上的管理员标志驱动,仅为 UX 表现);切换/列出/查看对任何已认证用户都保持开放。当前工作区是**客户端状态**,持久化在本地(刷新后恢复,若无则回退到最近访问的工作区),并与当前查看会话所属的工作区解耦。控制台标签页的**会话列表**只列出当前工作区的会话;切换当前工作区会刷新该列表——但**不会**自动选中某个会话,也不会打扰正在查看的聊天。会话列表的标题栏还带有一个手动**刷新**按钮(位于 ＋ 新建会话按钮左侧,两者仅在存在当前工作区时显示),按需重新获取当前工作区的会话列表(`list_sessions`)。移除当前工作区会把选中项回退到最近访问的那个。新建/选择/重命名/删除会话仍是 wire 消息;切换当前工作区是客户端状态变更,绝不是对服务端所拥有数据的本地修改。**空注册表的冷启动引导:** 客户端会话按首个 `ready` 的权威工作区快照与随后**第一条** `settings` 判定一次——注册表为空、已存在非 `system` 的真实智能体、且本连接是管理员时,自动打开新增工作区弹框(与手动「＋」同一个弹框、同一条 `add_workspace` 路径,自动打开本身不注册任何东西)。三种结果都消费掉这次判定:此后的设置推送、`workspaces` 增删广播(含最后一个工作区被移除)、重连 `ready`,以及用户关闭弹框,都不再自动弹出,整页刷新才算新的客户端会话;判定只在客户端内存里,不持久化、不改协议。智能体尚未配置时本次会话只留系统设置引导一个模态([智能体配置](../../settings/agent-config/agent-config-spec.md)),新增工作区既不排队也不叠加。
- **WC-R8a**: **新增工作区的磁盘路径由服务端主机的原生目录对话框点选产生,不由浏览器手敲。** 这条路径会成为一个工作区的信任根,手敲则拼错、失效、指错目录都要到 git 操作落在错误目录上才被发现。弹框里路径**只读**展示,「选择目录」发 `select_workspace_directory { requestId }`,服务端按 `process.platform` 分派(macOS `osascript choose folder` / Windows PowerShell `FolderBrowserDialog` / Linux 优先 `zenity`、缺失才退 `kdialog`),全程异步子进程——对话框可以一直开着,绝不阻塞事件循环或别的连接。回复 `workspace_directory_selection { requestId, result }` 三态互斥:`selected` 带归一化后的绝对路径,回填后按既有 basename 规则预填名称(用户改过就不覆盖);`cancelled` 是正常结果,弹框保持原状,不报错不清空;`failed` 只带一个不含命令输出的结构化错误码,就地展示并露出**仅本次有效**的手动输入兜底(不支持的平台、缺可执行文件、无 GUI/显示服务、异常退出、输出不是绝对路径都归此类;诊断信息只进服务端日志)。重新点选成功即退出兜底、回到只读。门禁与 `add_workspace` 同级(管理员限定,未认证 → `unauthenticated`)。**每连接只有一个选择槽位,后来者赢**:新请求先中止并清掉上一次仍开着的对话框再启动,不因此失败;客户端关闭弹框发 `cancel_workspace_directory_selection { requestId }`,连接断开同样中止并立即释放槽位(不等进程退出)。被取消或被顶替的子进程此后一律沉默——按 requestId 与运行代次校验,既不能回答别的表单,也不能清掉新请求的槽位;客户端同样丢弃 requestId 对不上的回复。选择不替代注册校验:目录在保存前消失,`add_workspace` 仍以 `path.notDirectory` 为准。`add_workspace` 本身仍接受绝对路径不变(兜底与非 UI 客户端依赖它)。所有文案均走 i18n。
...
- **WC-R25**: **感知 SDD 的需求操作按钮的四种状态。** 需求详情的主操作按钮为处于 `todo` 状态的需求渲染四种状态之一,由工作区的 `sddEnabled`(携带在需求列表消息上,因此无需单独获取设置)加上该需求的 `specStatus`(`raw`/`pending`/`approved`,唯一事实源)共同决定:SDD 关闭 ⇒ `Start Dev`;SDD 开启且 `specStatus` 为 `raw` ⇒ `Write Spec`(发送 `write_spec`;`raw` 包括无 spec 或仅有 `write_spec` 播种的占位——即使 `specPath` 已因播种而回填,也不显示 `Approve Spec`);`specStatus` 为 `pending`(已有真实内容且未批准)⇒ `Approve Spec`(发送 `approve_spec`);`specStatus` 为 `approved` ⇒ `Start Dev`。`Approve Spec` 是人工批准的检查点——点击它会在服务端批准该规格,随后按钮才推进到 `Start Dev`;它本身并不启动开发。见[需求管理](../intent-management/intent-management-spec.md) RM-R22。所有按钮文案均走 i18n。
- **WC-R26**: **需求详情:常驻头部 + 四个标签页。** 需求详情是一个**常驻的两行头部**加上一个**标签条**。第一行是标题行:左侧是需求标题与所属模块,右侧是优先级与状态;该头部中不显示日期前缀或运行状态徽标。第二行是一个独立的单行操作行,包含完整的操作集合:四状态主按钮(WC-R25),外加 `Refine`、打开最近一次开发会话、标记完成、取消、创建 PR、复制 PR id,以及自动化开关;它不会换行为多行,也没有单独的底部操作栏。当需求所在分支与工作区配置的主分支相同时,Create PR 会被隐藏,因为此时没有可作为审查目标的功能分支。两行头部在每个标签页上都始终可见,并在窄视口下按可预期的方式堆叠。头部下方是四个标签页:**需求**(内容 markdown + Git/PR 元信息 + 依赖编辑器)、**需求会话**(该需求的 refine/沟通会话,由 `intentSessionId` 标识)、**规格**(渲染该需求集中存放在工作区之外的 `specPath` 路径下的 `spec.md`——通过 `read_spec` 按需求 id 获取,因为受限于工作区内的 `read_file` 无法到达集中存放的规格根目录)、**规格会话**(撰写规格的会话,由 `specSessionId` 标识)。这两个会话标签页复用共享的聊天列,并遵循**单一当前会话**模型:选中该标签页会请求服务端打开对应会话(对需求会话使用 `open_intent_session` 携带该会话 id;对规格会话使用 `open_spec_session`,后者会恢复受限写入的规格运行时),只有当全局当前会话与该标签页所期望的 id 一致时聊天列才会渲染(否则展示短暂的加载态),这样一个会话的流永远不会显示在另一个的标签页下。会话/规格尚不存在的标签页会展示空态。切换所选需求会把详情重置回**需求**标签页。`Refine` 会在其会话绑定后回填该需求的 `intentSessionId`,使需求会话标签页此后可打开。所有文案均走 i18n。
- **WC-R28**: **工作台顶栏导航 + Workspace Dashboard。** 工作台视图(顶层 view-mode,经应用头部的双视图切换器进入)在 **AppHeader** 持有三页入口:「用户通知」(默认,既有 `wait_user_events` 事件流)、「Dashboard」、「聊天机器人」;切页不改变当前工作区,也不触发工作区业务标签的加载。**Dashboard** 以一次服务端聚合快照(`get_workspace_dashboard` → `workspace_dashboard`,对已认证连接可用)列出**全部**已注册工作区,顺序沿用 `listWorkspaces()`,每行展示 workspace 标识、运行中 session 数、session 总数(**覆盖全部 `SessionKind` 的 `bound=1` 真实投影**,非仅 work)、intent 总数、讨论总数、自动化配置总数,以及工作区**自动化总闸**状态(经 `normalizeWorkspaceSetting` 归一,缺省显示为开启);“运行中 session”取非 idle runtime 与仍有 running 执行日志的 automation session 之并集,按 session id 去重,终态 / 完成 / 失败不计入。首次进入、WebSocket 重连、workspace 列表变化,以及 Dashboard 激活期间收到会影响计数的 session/intent/讨论/自动化领域广播后,均触发**合并去重**的一次刷新(已有请求在途时只记录一次待刷新,禁止逐 workspace 拉取)。**逐行总闸**:管理员每行有一个可访问滑动开关(`role=switch`),直接开/关该工作区自动化——点击即以**单元素** `workspaceNames` 发 `set_workspaces_automation_enabled`,并把该行标记为在途(其开关 disabled,防重入);非管理员看不到开关但仍见真实总闸状态(on/off 徽标),服务端仍拒绝伪造写。回包 `workspaces_automation_result` 给出每项成功 / 结构化失败(`UiError`)与操作后最新快照,前端**仅清除本次结算行**的在途标记、以回包快照整体校准(成功时开关随快照翻转,无需提示),仅在存在失败时以 toast 提示;并发的多行切换各自独立回包、互不清除对方在途标记。总闸只改 `automationEnabled`,不动单条自动化的 `active/paused`、不取消在途运行、不影响手动立即运行(见[自动化](../automations/automations-spec.md) SCH-R28)。某领域 db 不可用或单个 workspace 聚合失败时整次快照返回结构化错误,前端保留上一份数据并提示“刷新失败”,不用半份快照删除正常行。桌面为表格,窄屏(≤720px)降级为逐行带列标签的卡片;每行开关具备无障碍名称。所有文案均走 i18n。
- **WC-R27**: **规格操作引导 + 误批准防护 + 规格标签页可见性。** 五条纯前端交互约束叠加在四状态主按钮(WC-R25)之上,均不触及规格生成、会话创建或批准状态机。**(a) 自动切换到规格会话——由会话创建成功驱动,而非定时。** 点击 `Write Spec` 会记下当前需求 id 与此刻的 `specSessionId`;当且仅当**该需求的** `specSessionId` 回填为一个非空的**新**值(即撰写会话确实创建出来了)时,详情才激活到**规格会话**标签页。创建失败、长时间没有回填、或回填的仍是旧值都不切换,页面停在当前可见处;用户在此期间导航到另一个需求即放弃该待切状态(不会把选中项抢回来,也不会被新需求的回填触发)。**不用固定延时:** 会话创建是异步且可失败的,而空白需求此刻两个规格标签页按 (d) 尚不可见,定时到点的切换会被可见性判定吞掉。**(b) Approve-Spec 延迟门禁。** 该门禁在当前会话中点击 `Write Spec` 时被激活(锚定在触发时间戳上,而不仅仅是 `specPath` 存在与否)。在激活期间,若按钮本应推进到 `Approve Spec`,它会保持隐藏,直到自触发起过去十秒,以降低在阅读生成的规格之前就批准的风险;剩余的等待时间在页面重新进入 / 状态刷新 / 组件重新挂载后依然保留,不会被这些操作绕过。一个已有规格但**并非**在本会话中撰写的需求不会被激活,因此其 `Approve Spec` 会立即可见。防误批准的十秒窗口是固定的,不可由用户配置,且没有额外增加确认对话框。**(d) 规格标签页的数据门槛。** **规格**与**规格会话**两个标签页当且仅当该需求已有规格数据(`specPath` 或 `specSessionId` 任一非空)时渲染,与工作区是否开启 SDD **无关** —— SDD 只说明「能写规格」,不代表「已经有规格」,而一个两者皆空的新建需求点进这两个标签页既无内容也无会话、无法启动任何东西,是纯粹的死入口。这与同类标签页的既有规则同构(规范评审会话见 WC-R29、PR 评审/修复会话见 WC-R32,工作会话取决于 `lastWorkSessionId`)。只有 `specPath` 的历史需求(撰写会话早已结束)仍保留这两个入口,已有规格内容与会话的读取能力不受影响;条件在浏览期间失效时按既有隐藏标签页规则回退到「需求」标签页。**(e) fast 需求不渲染 `Write Spec`。** 有效规格模式为 `fast` 的需求按定义不先写规格,主按钮在 `raw` 态直接呈现 `开始工作`(WC-R25、RM-R43),标题栏不提供「编写规范」;规格标签页仍只由 (d) 的数据门槛决定,不因 fast 而额外显示或隐藏。**(c) 有区分度的语义色。** `Write Spec` 与 `Approve Spec` 两种状态携带稳定的、无障碍可区分的语义色——`Write Spec` 保持主色(生成色),`Approve Spec` 使用成功/确认色——各自带有稳定的无障碍标签,使两个操作在视觉上与辅助技术上都能区分。待处理的自动切换 / 门禁计时器会在组件卸载时以及切换所选需求时被清除。所有文案均走 i18n。
- **WC-R29**: **规格评审会话的可见性:详情页「规范评审会话」标签页 + 会话页「规范」合并入口 + 只读聊天列。** **(a) 规范评审会话标签页。** 需求详情在**工作区开启 SDD 且该需求存在 `specReviewSessionId`** 时,于规格会话标签页之后渲染一个「规范评审会话」标签页;缺任一条件都不渲染(不为没有规范评审会话的需求显示空态入口,访问详情页也**不会创建**规范评审会话),条件在浏览期间失效时按既有隐藏标签页规则回退到「需求」标签页。标签内联运行中状态点与需求/规格/工作会话三类同构(非 idle/未知才显示)。**(b) 打开路径。** 选中该标签页以当前 `intentId` 发送 `open_spec_review_session`——客户端**从不**指名会话 id,目标一律由服务端按该需求保存的 `specReviewSessionId` 解析并恢复 `spec_review` 只读运行时(携带被审需求与内容指纹);沿用单一当前会话模型:只有全局当前会话与该 id 对齐后才渲染聊天列,id 异步回填补发一次,已对齐不重发。**(c) 会话页「规范」是合并入口。** 会话页**不新增**「规范评审」分类:「规范」标签页的服务端列表查询把 `session_kind ∈ {spec, spec_review}` 视为同一结果集,在**同一条**全局更新时间倒序上做排序/分页/刷新(首屏、加载更多、窗口刷新都不得漏项或重项),角标为两类运行中会话之和且每个会话只计一次;兼容保留的 `spec_review` 计数字段不形成第二个可见口径,也不在顶栏总数中重复计算。列表行保留**真实** `sessionKind` 与 owner:点击 `spec_review` 行必须携带投影的 `ownerKind='intent'` 与非空 `ownerId` 改发 `open_spec_review_session`(不离开会话页,回复的 `session_selected` 绑定右栏),**绝不**交给通用 `select_session`(其冷会话恢复会构造 `work` 运行时,破坏只读边界);owner 缺失或非需求所有时 fail closed——报错、不绑定、不恢复,且不回退到通用选择。规范评审行不获得改名/删除能力,其溯源解析为「该需求详情的规范评审会话标签页」,与规格撰写行的溯源相互区分。**(d) 只读聊天列。** 详情页规范评审会话标签页与会话页当前真实 `sessionKind='spec_review'` 的会话共用聊天列的**整列只读门**(与 `show-input` 不同,后者只隐藏输入框):隐藏输入框、不渲染待发队列、状态栏保留运行状态文字与刷新但不提供 stop/continue、历史与权限请求**完整回放**但不渲染 allow/deny/ask 回答控件;只读分支即便被程序化触发也不上抛任何变更事件。只读只是呈现层——服务端按 `sessionKind` 的续跑门禁才是防线(见 RM-R37)。所有文案均走 i18n。
- **WC-R32**: **PR 评审/修复会话的详情页入口。** 意图详情在需求存在 `reviewSessionId` 时于工作会话标签页之后渲染一个「评审会话」标签页、存在 `fixSessionId` 时渲染「修复会话」标签页,两者与工作区是否开启 SDD **无关**(区别于规范评审会话 WC-R29 的 `sddEnabled && specReviewSessionId` 门槛);缺对应 id 即不渲染(无空态入口、访问详情页也不会创建会话),条件在浏览期间失效时按既有隐藏标签页规则回退到「需求」标签页。选中标签页以**对应会话 id** 发送 `select_session`(复用普通会话选择路径,与概览「打开会话」一致,客户端指名会话 id,不同于规范评审会话按意图 id 解析):沿用单一当前会话模型,只有全局当前会话与该 id 对齐后才渲染聊天列,id 异步回填补发一次、已对齐不重发。两个标签页无内联运行中状态点(控制层派生的 review/fix 会话状态只供概览使用),聊天列不进入只读门、权限模式只读展示(模式锁与其余非工作会话一致);只指向最新 id,不提供历史会话列表。所有文案均走 i18n。
- **WC-R30**: **崩溃态一键重试。** 当当前查看会话的运行已停止(非 `running`)且末一轮以错误收尾(`turn_end { reason: 'error' }` 落成的出错活动标签)时,状态栏在出错标签旁直接渲染一个**一键重试**控件,把「从故障中恢复」压成一次点击。它是**入口封装**,不是新的重试机制:点击后走与手动 Continue 完全相同的链路——为同一会话发一条 `user_prompt("continue")`,乐观切换为 running,并清除该会话的副作用危险标志(无新协议消息、无独立重试状态、不改变重试次数/退避/错误分类/崩溃判定)。三处互斥与门控:运行中(含 `reconnecting` 这类自动 resume 在途的中间态)不出现;副作用危险态(AS-R19)保留 WC-R19 的确认式 **Continue**,一键重试**绝不**在该态出现以免绕过“确认无副作用”这一步;只读聊天列(隐藏运行控件)不渲染该入口。文案走 i18n。
- **WC-R31**: **运行日志页(独立路由 + 轮询增量)。** 顶栏的连接状态指示器(WC-R6)同时是运行日志入口:一个指向独立 hash 路由 `#/logs` 的链接,在**新的浏览器标签页**打开,原页面与主应用照常交互、互不影响(两者只共享本浏览器保存的会话令牌)。该路由挂载的是一个自成一体的页面——自建 WebSocket、自持轮询与展示缓冲区,不复用主应用的控制器。页面按固定节拍轮询 `read_runtime_log`:首次不带 `offset`(服务端回实时日志文件尾部的一段历史,页面一开出来就有内容),之后一律带上一次回复的 `nextOffset` 只取增量;带 `reset` 的回复整体替换缓冲区(服务端换了文件或文件被截断),否则续接上一轮被切断的半行。轮询按**页面可见性**门控:标签页切走即停表,切回立刻补拉一次再恢复节拍;失焦但仍可见时继续轮询——日志常被摆在旁边的窗口里盯着看,失焦不等于不看。缓冲区有行数与字符数双上限,超限丢弃最早的行并给出截断提示,长时间开着不会让标签页内存无限增长。视图默认跟随最新一行,用户向上滚动阅读时停止跟随,滚回底部即恢复。日志正文按纯文本原样渲染(时间戳前缀由服务端产出),不提供搜索、级别过滤与语法高亮。尚无内容、服务端未启用文件日志、连接未通过鉴权三种情形各有说明文案。所有文案走 i18n。读取侧契约见[可用性](../../../non-functional/availability.md) OBS-6。
...
- **危险态,手动继续(WC-R19):** Given 一个轮次以
  ...
  会话或重新选择该会话同样会清除该标志。
- **崩溃态一键重试(WC-R30):** Given 当前查看会话的一轮以
  `turn_end { reason: 'error' }` 收尾、会话已停止运行(状态栏停在出错标签),
  When 用户点击状态栏的**一键重试**,Then 同一个会话通过一条
  `user_prompt("continue")` 被重新拉起(乐观切换为 running,出错标签让位给运行活动标签)——
  与手动 Continue 是同一条链路,没有新的协议消息。**反例场景:** 运行中、
  `reconnecting`(自动 resume 在途)、只读聊天列都**绝不**出现该入口;副作用危险态
  (AS-R19)仍只给确认式 Continue,一键重试**绝不**用来绕过“确认无副作用”这一步。
...
`get_workspace_dashboard`、`set_workspaces_automation_enabled`、`read_runtime_log`、`ping`。消费 `ready`、`workspaces`、
...
`turn_end`、`settings`、`workspace_setting`、`workspace_dashboard`、`workspaces_automation_result`、`runtime_log`、`error`、`pong`。见
...
- **崩溃态** —— 当前查看会话的运行已停止且末一轮以错误收尾(出错活动标签)。
  它不是一种新的会话状态,只是「非 running + 出错标签」这一组合的名字;
  状态栏据此直出一键重试入口(WC-R30),点击复用手动 continue 链路。
