...
> 门禁用途：M1 把单条意图上的 `pr_id/pr_url/pr_status` 拆为多 PR 关系前，按本清单
> 逐项打勾迁移；未打勾的项不得宣称已硬切。
> 字段名约定：`pr_id`/`pr_url`/`pr_status` 为 SQLite 列（snake_case），
> `prId`/`prUrl`/`prStatus` 为领域模型字段（camelCase），两者是一事实的两面。
> 生成方式：人工 `rg` + 沿调用链复核（2026-08-06）。不覆盖第三方依赖、构建产物、`.git`。
> 来源事实核查见 [intent-management-pr-fact-check.md](../core/intent-management/intent-management-pr-fact-check.md)。
...
**读/写/派生分类**：`读`= 从持久化字段读取用于展示或判断；`写`= 把值写入持久化字段；
...
| 待切换 | 文件:符号                                      | 字段                         | 分类   | 用途                                                                           |
| ------ | ---------------------------------------------- | ---------------------------- | ------ | ------------------------------------------------------------------------------ |
| ☐      | `database/intents/intents.sql:20-22`           | `pr_id`/`pr_url`/`pr_status` | 持久化 | 三列 DDL 定义（v7→v8 加 `pr_id`/`pr_status`，v13→v14 加 `pr_url`）；拆表后删列 |
| ☐      | `database/migrate/2026/06/20/016-intents.sql`  | `pr_url`                     | 持久化 | v13→v14 加 `pr_url` 迁移记录（实际由 store 幂等守卫执行）                      |
| ☐      | `server/src/features/intents/store.ts:320-321` | `pr_id`/`pr_status`          | 持久化 | schema 初始化 `ensureColumn('intents','pr_id','TEXT')` / `pr_status`（v7→v8）  |
| ☐      | `server/src/features/intents/store.ts:340`     | `pr_url`                     | 持久化 | schema 初始化 `ensureColumn('intents','pr_url','TEXT')`（v13→v14）             |
| ☐      | `database/tables.md:41`                        | `pr_id`/`pr_url`/`pr_status` | 持久化 | Schema 版本史与 Git 追踪字段说明（v7→v8 / v13→v14）                            |
...
测试夹具（`fixture`）与迁移断言（`migration`）中的三字段引用是迁移验证项而非运行时读点，
M1 迁移后应随之更新断言；按文件列举，同模式重复仅记首行并注 `×N`：

| 待切换 | 文件:符号                                                                                                                                                                                                                                                                                 | 类别              | 说明                                                  |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | ----------------------------------------------------- |
| ☐      | `server/src/features/intents/store.test.ts:174,420,469-476,497-530`                                                                                                                                                                                                                       | fixture/migration | schema 夹具、v10 迁移断言、setPrInfo round-trip       |
| ☐      | `server/src/features/intents/pr-url-migration.test.ts:45-100`                                                                                                                                                                                                                             | migration         | v13→v14 加 `pr_url` 迁移全断言                        |
| ☐      | `server/src/features/intents/spec-status-migration.test.ts:51`                                                                                                                                                                                                                            | migration         | 旧 v17 schema 夹具含 pr 三列                          |
| ☐      | `server/src/features/intents/rename-intents-migration.test.ts:137-138`                                                                                                                                                                                                                    | migration         | 终端 schema 断言 `pr_id`/`pr_status` 列               |
| ☐      | `server/src/features/intents/rename-workspace-path-migration.test.ts:72,90`                                                                                                                                                                                                               | migration         | 旧 v10 schema 夹具                                    |
...
| ☐      | `database/tables.md` / `doc/domains/core/intent-management/intent-management-{models,design,spec}.md`                                                                                                                                                                                     | 文档              | 领域文档中的三字段描述，M1 后需同步                   |

---

## 已知数据质量观察（非读点，供 M1 回填参考）

- `pr_status` 与 forge 真实状态存在漂移：存量 382 行非空 `pr_status`（merged 378 /
  reviewing 3 / closed 1），而 365 个可核查 PR 的真实 forge state 为 merged 363 /
  closed 1 / open 1；另有 17 行 `pr_status='merged'` 但无 `pr_id`。详情见事实核查文档。
- `pr_id` 存在但 `pr_url` 为空的 24 行，forge/repo 需经 workspace git remote 解析。
