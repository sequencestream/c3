...
| 完整设置视图页面       | 可编辑草稿,每个智能体一行,增加/删除,拖拽重排;列表行按该行草稿 `vendor` 施加由 `VENDOR_COLOR` 派生的浅 tint 底色以区分供应商;列表下方一个**默认智能体下拉框**(仅启用的智能体,按顺序号顺序)取代了逐行的单选按钮;保存。按项目的控制项移到了 workspace-setting 视图 |
