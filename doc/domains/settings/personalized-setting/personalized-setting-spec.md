...
「个人化设置」是与系统设置、工作区设置并列的独立全屏页,入口在桌面顶栏与移动端操作菜单**恒定可见**(不受 `isAdmin` 约束),且不依赖当前工作区。每项都是即时生效 + 即时持久化,故页面没有草稿态、脏标记与保存按钮。打开页面时先丢弃任何仍在手上的明文再拉取本身份的 key 名册,关闭页面同样丢弃——名册与明文都是按身份的,让上一个身份的东西留在屏幕上是错的。
