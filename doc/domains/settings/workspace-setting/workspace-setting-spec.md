...
- **七类角色。** 工作区**配置了默认 Agent** 时工作区层优先(工作区同名角色 → 工作区默认 → 系统角色 → 系统默认),**继承**时系统角色优先(系统角色 → 工作区同名角色 → 工作区默认 → 系统默认),详见 AC-R33;页面说明这一先后,各角色选择器居首为「继承系统默认」。运行时角色保留组路由与空组硬失败;自动化、评审、修复覆盖只用于新建表单或模板的身份种子,review/fix 工作区覆盖不进入队列解析(AC-R34)。
