...
第七个 Tab「外部 MCP 接入」管理 c3 未拉起的 agent 如何通过 MCP 访问**本工作区**。域语义见 [external-mcp](../../core/external-mcp/external-mcp-spec.md);key 存储与哈希见 [system-setting](../system-setting/system-setting-spec.md#外部-mcp-api-key-存储-mcpapikeys)。
