...
│   │   │   │   ├── 人工发起评审/修复             # 意图标题栏显式发起一轮(start_intent_relay{workspaceName,intentId,phase}):准入只有 worktree 模式、有活跃 PR、阶段判定三条,automate 与 status 不是门槛、L5 也能发起;阶段判定是 Web 与服务端共读的同一个纯函数,判据不含冷却与并发配额,判定通过后仍要认领;与队列共用 admitRelayPhase/executeRelayPhase 这一份执行内核,结算各自收口——队列 runRelayPhase 记阶梯、人工 runManualRelayPhase 只释放占位,四项差异见 RM-R53;在途取发送时派生的 reviewInFlight/fixInFlight(不用裸 pending 状态推断),在途阶段按钮整体不渲染;拒绝分阶段级/请求级两类(清单见 RM-R53),阶段级同步回 error 帧就地提示、请求级沿用既有通用错误呈现;认领成功只广播账本、无专用 response 帧;current-branch 不提供
...
│   │   ├── 定时触发                              # cron 计划到点触发;每分/每时可限定执行时段(半开区间 [起:00, 止:00),可跨午夜,全天即通配)
