...
│   │   ├── IM 身份绑定                            # Web 本人发起挑战、私聊消费;未绑定/已撤销只收固定引导;群明细默认拒绝,管理员配置群工作区白名单
│   │   ├── 能力上限 L0–L3                         # L0 受控播报 / L1 只读问答 / L2 定向作答 / L3 高风险写入只回 Web 深链;分级是能力天花板与后续设计约束,不是当前启用状态——不因此提前开放身份绑定、主动播报或聊天写动作;本地工具白名单仍受运行根/沙箱约束,勾选写工具 ≠ 获得 L2/L3 业务权限
...
│   │   ├── c3 MCP 按回合绑定                      # 勾选的 c3 MCP 工具经回环 HTTP MCP 按线程绑定到机器人目录,只暴露勾选子集(未勾选不出现且直调被拒);不跨出运行根目录,find_* 空/view_* not-found 是预期
...
│   │   ├── 发送者隔离 Conversation                # 身份=(platform,robotId,threadKey,senderId);同群不同发送者互不相通;threadKey=话题→回复链根→会话;原生 sessionId 仅为同 vendor 续接缓存
│   │   ├── 数据库上下文事实源                     # 已投递的用户/回答成对落 im_robot_context_turns(ADR-0048);运行目录删除不改归属;入站凭据/4000 码点拒绝;每会话最多 50 回合/30 天硬删
│   │   ├── 一 Conversation 一回合                 # 同发送者严格串行,忙时立刻回「上一个还在处理」;不同发送者可并发。每条被接受的消息都以回复或审计原因结束(含 input_rejected)
│   │   ├── 预设权限默认只读                       # 工具白名单为空即只读(创建时取值),写/执行须管理员逐个勾选;一个机器人的多线程共享同一目录,当前只保证同线程串行
│   │   ├── 响应面默认收敛                         # 群消息默认必须 @机器人、单聊默认不响应;另有群白名单与单聊白名单;非文本消息与其它机器人的消息一律跳过
│   │   ├── 线程即一段对话                         # 线程键 = 平台话题 → 回复链根 → 会话本身(各带前缀防撞);次轮起以绑定的会话 id 续接,故 IM 线程读起来是连续对话;会话只在同 vendor 内可续接;线程不绑定工作区
│   │   ├── 一线程一回合                           # 同线程严格串行,忙时立刻回「上一个还在处理」并以 busy 落审计(未启动 run),而非排队或并发;不同线程不设上限。每条被接受的消息都以回复或审计原因结束,绝不静默丢弃
