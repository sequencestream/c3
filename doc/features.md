...
│   │   ├── Model Vendor 与模型清单               # provider 声明上游模型厂商身份(五十来家,分模型厂商/推理云/聚合网关/本地运行时/其他五组,含 Anthropic、OpenAI、Gemini、DeepSeek、Kimi、豆包、GLM、Qwen、MiniMax、小米 MiMo、混元、文心、xAI、Groq、Together 等),决定 c3 随版本内置、离线可用、不发任何发现请求的模型清单;与创建来源 template 分离且可单独改,缺失时从已知 template 推断、未知一律读为 custom;核验不到清单/端点的厂商留空而非猜,聚合网关与本地运行时按性质恒为空;自有模型条目按 id 覆盖同名内置条目,合并结果仅作候选与能力解析,不校验、不兜底、不做白名单
