...
│   │   ├── 工具权限网格                           # 与自动化表单共享的权限网格(只读/写入两组+全选/全清+网络开关);清单=SDK 内建+13 个 c3 MCP 工具,按 vendor 加载;创建默认只读、编辑保持原样(清单失败不清空)、切换 vendor 清空勾选
│   │   ├── network-access 开关                    # allowlist 里的伪条目,非工具:仅 codex+workspace-write 沙箱生效,默认关;只读沙箱置灰并解释,切回可写时已存值保留;不授予 shell 本身、不启用 web 搜索
│   │   ├── c3 MCP 按回合绑定                      # 勾选的 c3 MCP 工具经回环 HTTP MCP 按 Conversation 绑定到机器人目录,只暴露勾选子集(未勾选不出现且直调被拒);六个 L1 只读工具(find/view intents、deliveries、discussions)每次 handler 重算个人 scope ∩ 群白名单,对象型 id 反查、列举型跨 scope 标注 workspaceName;写类工具不能借 id 反查获得路径;本地读仍限运行根
