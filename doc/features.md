...
│   │   ├── 运行日志页                            # 顶栏连接状态即入口,新标签页打开独立路由 #/logs;自建连接按可见性门控轮询增量拉 c3.log 尾部,行数/字符数双上限,默认跟随最新、上滚即停
...
│   │   ├── 更新胶囊                              # 顶栏一个控件随服务端自更新状态换形:下载中显百分比、就绪时管理员点「重启以更新」并二次确认、非管理员只告知、失败可重试;不可自更新才退回外链发行页
...
│   │   │   │   ├── 人工发起评审/修复            # 意图标题栏显式发起一轮(start_intent_relay{workspaceName,intentId,phase}):准入只有 worktree 模式、有活跃 PR、阶段判定三条,automate 与 status 不是门槛、L5 也能发起;阶段判定是 Web 与服务端共读的同一个纯函数,判据不含冷却与并发配额(判定通过后仍要认领,认领失败照常回拒绝码);与队列共用 runRelayPhase 的执行内核(身份/cwd/prompt/工具面/30 分钟上限/占位释放),四项差异=不签发合并凭据、不记失败阶梯、不受 needsReview 限制、不占并发配额;在途取发送时派生的 reviewInFlight/fixInFlight(不用裸 pending 状态推断,否则 stale 占位会永久隐藏按钮),在途阶段按钮整体不渲染;认领前的阶段级拒绝(非 worktree、无活跃 PR、阶段不可发起、阶段在途、Agent 不可用、worktree 不可用、认领写失败)同步回 error 帧就地提示,不占意图动作错误对话框;请求级拒绝(工作区未知、意图不存在或跨工作区、存储不可用)沿用既有通用错误呈现,与 RM-R53 的两类分法同口径;认领成功只广播账本、无专用 response 帧,认领后会话失败不回调发起连接也不 park;current-branch 不提供
...
│   │   ├── 人类参与                              # 人可发言进入讨论、暂停/恢复、停止(终结为 cancelled 终态,先拆卸存活的编排/调研运行再落库,不可恢复)
...
│   │   ├── c3 MCP 工具                           # 意图(find/view/save_directly)+ PR 状态同步(sync_intent_pr_status,只接受 intentId,触发服务端从 forge 派生终态落库)+ PR 评审/修复结果回填(sync_intent_review_status/sync_intent_fix_status,只写终态及对应会话)+ WorkNote 历史(append_intent_worknote/list_intent_worknotes)+ 交付只读(find_deliveries/view_delivery,无写工具)+ PR 事件 + 讨论(find/view/start/continue)工具,按需挂载;claude/codex/cursor 都走同一条 loopback HTTP MCP 路由(同一批工具);列在目录里只代表可勾选,内置模板一律不默认勾交付工具
...
│   │
│   ├── im-robot 聊天机器人                        # 把 agent 能力延伸到办公 IM:群里 @机器人 提问,无人值守跑一轮,把最终回答发回群里;c3 唯一一条主动把 agent 产出送往第三方云的路径
│   │   ├── 运行诊断日志                           # 连接/入站/出站/绑定挑战统一打 `[im] …` 元数据行(机器人名+状态、chat 类型、sender 摘要、tokenish、notice key、消费成败原因);不记验证码明文与消息正文;飞书 SDK 连接生命周期经 `[c3][feishu]` 透出
│   │   ├── 部署级出入口                           # 机器人是 c3 实例共享的 IM 出入口,配置/连接/名册跨工作区一致;部署级全局 ≠ 无边界访问——运行目录是隔离工作容器而非授权范围,作用域在每次工具调用时求交;否决工作区内机器人与连接/线程级工作区绑定(后者会把一次上下文选择变成隐形授权)
│   │   ├── IM 身份绑定                            # Web 本人按 account namespace(平台+appId)发起挑战、私聊消费;同一 subject 可分别绑定多个命名空间,同命名空间内唯一;未绑定/已撤销只收固定引导;群明细默认拒绝,管理员配置群工作区白名单
│   │   ├── 能力上限 L0–L3                         # L0 受控播报 / L1 只读问答 / L2 独立 write-grant 定向作答 / L3 受控写入;六个 c3 MCP 写工具在管理员显式勾选并通过调用级作用域后执行,不替代 L2 grant、本地运行根/沙箱或领域业务门;其它高风险动作仍回 Web
│   │   ├── 外发四重授权                           # 默认停用且无「创建并启用」一步操作 / 启用前不可跳过的范围确认(时刻入库,服务端校验故前端少弹框也过不了) / 仅管理员 / 每次外发一行审计;缺任一条即拒绝启用
│   │   ├── 只发最终回答与固定进度提示            # 仅回合结束时的最终 assistant 文本作为回复外发;长回合执行期间 supervisor 按宽限/间隔/预算门控发固定进度提示(progress.*,不进上下文不占审计);工具调用/结果/中间推理/文件内容一概不发——观察者只累积 assistant 文本,是结构而非开关
│   │   ├── 本地读取限定运行根                     # 本地文件读工具只在 ~/.c3/robots/<name>/ 内放行:真实路径裁决(绝对路径/../符号链接/断链/相邻前缀均按实际解析),越界在读到前拒;每回合冻结运行根,描述表声明各工具访问位置(无描述即拒);门+执行前钩子双重强制,宿主 settings 绕不开;固定拒绝消息不含路径,审计只记原因类别
│   │   ├── 唯一出站守卫                           # 所有外发(正常回答、凭据拦截、超时/阻塞/错误、工作目录失败、线程忙、进度提示)只经平台中性受控发送入口;supervisor 拿不到可直发的 sender;发送前重读启用/外发确认/群与私聊白名单;凭据形状命中改发固定提示(不回显、不递归扫描);超长截断且明说;放行/拒绝/平台失败各一次元数据审计
│   │   ├── 无人值守不挂起                         # 专属 robot 门:读类放行(本地读限运行根)、面向人的交互工具直接拒绝(群里无人可答)、写/执行仅在冻结白名单内;叠加墙钟上限与「见到授权请求即以 blocked 收敛」共三重防线
│   │   ├── 工具权限网格                           # 与自动化表单共享的权限网格(只读/写入两组+全选/全清+网络开关);清单按使用场景加载:SDK 内建+14 个通用 c3 MCP 工具,机器人再含专属只读 list_workspaces,自动化不含;创建默认只读、编辑保持原样(清单失败不清空)、切换 vendor 清空勾选
│   │   ├── network-access 开关                    # allowlist 里的伪条目,非工具:仅 codex+workspace-write 沙箱生效,默认关;只读沙箱置灰并解释,切回可写时已存值保留;不授予 shell 本身、不启用 web 搜索
│   │   ├── c3 MCP 按回合绑定                      # 勾选的 c3 MCP 工具经回环 HTTP MCP 按 Conversation 绑定,只暴露勾选子集(未勾选不出现且直调被拒);六个 L1 账本读、机器人专属 list_workspaces 与六写工具每次 handler 重算个人 scope ∩ 群白名单;list_workspaces 仅返回注册表有序名称,对象型写工具按 id 反查候选归属后验详细可见集,新建型显式传可见 workspaceName;注册根才可进入共享业务核心,机器人运行根只供本地工具
│   │   ├── 沙箱只看本地写工具                     # codex 原生沙箱模式只由 SDK 本地写/执行工具(shell/apply_patch)决定,勾 c3 MCP 写工具不算——那会打开可写沙箱与网络边界;sessionKind 保持 robot
│   │   ├── 回合无条件进程隔离                     # 机器人回合无论工作区沙箱开关都进程隔离(rt.sandboxPaths 未置位即现造),允许集=运行根+最小 vendor 依赖,根外进程不可达;隔离建立失败以安全错误结束回合,不退化
│   │   ├── 预设权限默认只读                       # 工具白名单为空即只读(创建时取值),写/执行须管理员逐个勾选;一个机器人的多 Conversation 共享同一目录,当前只保证同 Conversation 串行
│   │   ├── 响应面默认收敛                         # 群消息默认必须 @机器人、单聊默认不响应;另有群白名单与单聊白名单;非文本消息、空白 senderId 与其它机器人的消息一律跳过
│   │   ├── 发送者隔离 Conversation                # 身份=(platform,robotId,threadKey,senderId,bindingId,subject,scope_hash)七维;须先完成 IM 身份绑定;scope_hash 反映授权版本,绑定/撤销/ACL 变化切断旧上下文;threadKey=话题→回复链根→会话;原生 sessionId 仅为同 vendor 续接缓存
│   │   ├── 数据库上下文事实源                     # 已投递的用户/回答成对落 im_robot_context_turns;运行目录删除不改归属;入站凭据/4000 码点拒绝;每会话最多 50 回合/30 天硬删
│   │   ├── 一 Conversation 一回合                 # 同七维身份严格串行,忙时立刻回「上一个还在处理」并以 busy 落审计(未启动 run);不同 Conversation 不设上限。每条被接受的消息都以回复或审计原因结束(含 input_rejected、identity_required)
...
│   │   ├── modelProvider 具名上游                # 连接从 agent 上提为具名 provider,多 agent 共用一条:账户级 key + 按 protocolType(openai/anthropic)的 urls + 可选 openai wireApi;agent 绑定时按 vendor 协议支持列表取第一个有 URL 的槽;可暂停;端点预设来自只读目录,模板新建整条预填,选厂商或勾协议时只补空槽、不覆盖已填值;删除被引用者需二次确认,悬挂引用 fail-soft 回落并告警
│   │   ├── Model Vendor 与模型清单               # provider 声明上游模型厂商身份(五十来家,分模型厂商/推理云/聚合网关/本地运行时/其他五组,含 Anthropic、OpenAI、Gemini、DeepSeek、Kimi、豆包、GLM、Qwen、MiniMax、小米 MiMo、混元、文心、xAI、Cursor、Groq、Together 等),决定 c3 随版本内置、离线可用、不发任何发现请求的模型清单;与创建来源 template 分离且可单独改,缺失时从已知 template 推断、未知一律读为 custom;核验不到清单/端点的厂商留空而非猜,聚合网关与本地运行时按性质恒为空;自有模型条目按 id 覆盖同名内置条目,合并结果仅作候选与能力解析,不校验、不兜底、不做白名单
│   │   ├── 连接两态与 model 反查                 # agent 表单选「provider / CLI 自带登录」两态,configMode 由所选派生、表单不单独展示;model 输入按所选 provider 或全部同 vendor provider 的有效模型清单给候选,接不了 provider 的 vendor(Cursor)改取同名 Model Vendor 的内置清单,清单外的 id 照样可手输并保存,换 provider 只换候选不动已填值;填入只有唯一 provider 提供的模型时自动选上它
│   │   ├── 连接可达性探测                        # 服务端拨一次端点(草稿 URL 只配草稿 key;已存 URL 才用已存 key):结构性问题就地标注,401/403 记为可达而非不可达,不消耗 token;出网面见 SEC-15
│   │   ├── 模型提供方测速                        # 对一条已保存 provider 发起真实流式生成采样:请求数 1–100(默认 10),协议候选只取已保存 urls 的非空槽;模型默认留空(请求体省略 model),可手输任意 id 或从有效模型清单选预设,目录不作校验;openai 槽按 wireApi 派生实际方言;目标开始时冻结(provider/协议/方言/请求模型/端点/账户级 key),暂停位不参与判定;三种流逐请求保存上游声明的 observedModel,运行级从明细去重排序投影 observedModels,未知绝不以请求值回填;标定报文与 128 输出上限、temperature=0、并发 1、无预热/重试/补跑、单请求 60s 超时;执行归服务端(关窗/切页/断线不停,只有显式中断才停);收束后一次事务只增落库;报告、汇总、对比均展示「请求 → 实测」,对比沿用既有轮次选择、TTFT 排序与非同时测量提示且不重算指标
...
└── distribution 分发形态                        # 同一次发布产出两个渠道,共享同一个 ~/.c3(设置/凭据/工作区/DB/会话)
    ...
    ├── 控制台自更新                              # 检查到新版即后台下载校验到 <c3Home>/update-staging,ready 前不碰已装二进制;管理员一键重启,按运行形态移交(systemd --no-block / launchd KeepAlive / 助手进程 / 前台就地派生);桌面托管、dev、包管理器、目录不可写四种形态明确让位
