...
│   │   ├── 工具权限网格                           # 与自动化表单共享的权限网格(只读/写入两组+全选/全清+网络开关);清单=SDK 内建+14 个 c3 MCP 工具,按 vendor 加载;创建默认只读、编辑保持原样(清单失败不清空)、切换 vendor 清空勾选
│   │   ├── network-access 开关                    # allowlist 里的伪条目,非工具:仅 codex+workspace-write 沙箱生效,默认关;只读沙箱置灰并解释,切回可写时已存值保留;不授予 shell 本身、不启用 web 搜索
│   │   ├── c3 MCP 按回合绑定                      # 勾选的 c3 MCP 工具经回环 HTTP MCP 按 Conversation 绑定,只暴露勾选子集(未勾选不出现且直调被拒);L1 读与六写工具每次 handler 重算个人 scope ∩ 群白名单;对象型写工具按 id 反查候选归属后验详细可见集,新建型显式传可见 workspaceName;注册根才可进入共享业务核心,机器人运行根只供本地工具
