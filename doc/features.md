...
│   │   ├── 模型提供方测速                        # 对一条已保存 provider 发起真实流式生成采样:请求数 1–100(默认 10),协议候选只取已保存 urls 的非空槽(OpenAI 优先、单槽自动选中),模型取自有效模型清单不可手输,openai 槽按 wireApi 派生实际方言;目标开始时冻结(provider/协议/方言/模型/端点/账户级 key),暂停位不参与判定也不被改动;标定报文与 128 输出上限、temperature=0、并发 1、无预热/重试/补跑、单请求 60s 超时;执行归服务端(关窗/切页/断线不停,只有显式中断才停),进度 n/N,中断记 cancelled 不推进完成数;收束后一次事务落库(只增、展示名快照、提交失败就地「重试保存」且不重拨上游);单提供方报告分页读历史,「历史报告」总入口可选到已删除的提供方;跨提供方横向对比视图一次列出所有有记录的提供方(含已删除的),每行默认取最近一次、可切到更早的轮次(备选随取数带回),按 TTFT P50 升序/降序排列且不可用的行恒垫底,并固定展示「非同一时刻测量、数字仅供参考」的口径提示;对比只投影已算好的汇总,不重算、不聚合、不做显著性检验
