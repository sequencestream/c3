...
│   │   ├── 与工作区解耦                           # 机器人按 name 拥有目录 ~/.c3/robots/<name>/,该目录不进工作区注册表;不出现在工作区切换器与会话页,其 run 需被自动化显式点名才参与触发匹配
...
│   │   ├── 出站守卫与截断                         # 凭据形状命中即不发并以 guard_refused 落审计(不回显命中内容);只用凭据规则不用记忆域的产物规则,否则带代码块的回答会被全拒;超长截断且明说被截断
