...
│   │   │   │   ├── 人工发起评审/修复            # 意图标题栏显式发起一轮(start_intent_relay{workspaceName,intentId,phase}):准入只有 worktree 模式、有活跃 PR、阶段判定三条,automate 与 status 不是门槛、L5 也能发起;阶段判定是 Web 与服务端共读的同一个纯函数(按钮出现过即点得成),判据不含冷却与并发配额;与队列共用 runRelayPhase 的执行内核(身份/cwd/prompt/工具面/30 分钟上限/占位释放),四项差异=不签发合并凭据、不记失败阶梯、不受 needsReview 限制、不占并发配额;在途取发送时派生的 reviewInFlight/fixInFlight(不用裸 pending 状态或 pending: 前缀推断,否则 stale 占位会永久隐藏按钮),在途阶段按钮整体不渲染;认领前每种拒绝同步回 error 帧就地 toast(不占意图动作错误对话框),认领成功只广播账本、无专用 response 帧,认领后会话失败不回调发起连接也不 park;current-branch 不提供
