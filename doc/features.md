...
│   │   ├── 能力上限 L0–L3                         # L0 受控播报 / L1 只读问答 / L2 定向作答 / L3 高风险写入只回 Web 深链;分级是能力天花板与后续设计约束,不是当前启用状态——身份绑定与 L1 调用级作用域已开放,主动播报与聊天写动作仍不启用;本地工具白名单仍受运行根/沙箱约束,勾选写工具 ≠ 获得 L2/L3 业务权限
│   │   ├── 外发四重授权                           # 默认停用且无「创建并启用」一步操作 / 启用前不可跳过的范围确认(时刻入库,服务端校验故前端少弹框也过不了) / 仅管理员 / 每次外发一行审计;缺任一条即拒绝启用
│   │   ├── 只发最终回答                           # 仅回合结束时的最终 assistant 文本外发,工具调用/结果/中间推理/文件内容一概不发——观察者只累积 assistant 文本,是结构而非开关
...
│   │   ├── 唯一出站守卫                           # 所有最终回复(正常回答、凭据拦截、超时/阻塞/错误、工作目录失败、线程忙)只经平台中性受控发送入口;supervisor 拿不到可直发的 sender;发送前重读启用/外发确认/群与私聊白名单;凭据形状命中改发固定提示(不回显、不递归扫描);超长截断且明说;放行/拒绝/平台失败各一次元数据审计
│   │   ├── 无人值守不挂起                         # 专属 robot 门:读类放行(本地读限运行根)、面向人的交互工具直接拒绝(群里无人可答)、写/执行仅在冻结白名单内;叠加墙钟上限与「见到授权请求即以 blocked 收敛」共三重防线
│   │   ├── 工具权限网格                           # 与自动化表单共享的权限网格(只读/写入两组+全选/全清+网络开关);清单按使用场景加载:SDK 内建+13 个通用 c3 MCP 工具,机器人再含专属只读 list_workspaces,自动化不含;创建默认只读、编辑保持原样(清单失败不清空)、切换 vendor 清空勾选
│   │   ├── network-access 开关                    # allowlist 里的伪条目,非工具:仅 codex+workspace-write 沙箱生效,默认关;只读沙箱置灰并解释,切回可写时已存值保留;不授予 shell 本身、不启用 web 搜索
│   │   ├── c3 MCP 按回合绑定                      # 勾选的 c3 MCP 工具经回环 HTTP MCP 按 Conversation 绑定到机器人目录,只暴露勾选子集(未勾选不出现且直调被拒);六个 L1 账本读工具每次 handler 重算个人 scope ∩ 群白名单;机器人专属 list_workspaces 同样逐次求交并仅按注册表顺序返回名称(无路径/身份/whoami,空交集为空数组,scope/群变化返回最新集合并使整回合失效,binding 变化拒绝),运行根永不解释为工作区;写类工具不能借 id 反查获得路径
