...
│   │   ├── 线程即一段对话                         # 线程键 = 平台话题 → 回复链根 → 会话本身(各带前缀防撞);次轮起以绑定的会话 id 续接,故 IM 线程读起来是连续对话;会话只在同 vendor 内可续接
│   │   ├── 一线程一回合                           # 同线程严格串行,忙时立刻回「上一个还在处理」而非排队或并发;不同线程不设上限。每条被接受的消息都以回复或审计原因结束,绝不静默丢弃
