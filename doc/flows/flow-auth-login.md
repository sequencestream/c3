...
2. **改密码门。** 修改一个既有账号的密码需要先证明该账号的当前密码正确
   ...
   (请求级别的鉴权尚在延后)。
3. **首次登录验证。** 首个管理员凭据持久化后,服务器先返回成功结果,再把当前连接置为未认证并
   发送 `unauthenticated { reason: 'missing' }`。控制台显示登录门,操作者必须用刚设置的凭据登录。
   修改既有密码或添加后续账号不会清理当前连接,其他连接也不会收到该消息。
4. **唯一管理员的完整性。** `set_admin_account { username }` 指定唯一的管理员；
...
5. **账号存储的归属。** 整个 `basic` 账号集合**只**由这些专用消息修改;通用的
