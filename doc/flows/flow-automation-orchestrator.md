...
点击「启动评审 / 启动修复」
  ...
  → 结论只由 MCP 工具调用回填;无结论退出不记失败阶梯
```

**按钮与服务端读同一个函数。** 界面据投影台账字段与 `reviewInFlight` / `fixInFlight` 调用共享判定;
服务端不信任客户端缓存,用同一函数对当前台账**现场重算**。因此「按钮出现过」与「点了能成」不可能分叉
—— 唯一的分歧窗口是两次读取之间别处刚认领了该阶段,此时以服务端为准并如实回一个拒绝码。在途阶段
按钮**整体不渲染**:一个看得见却点不动的按钮,与「点了没反应」无从区分。

**在途只认下发字段。** `reviewInFlight` / `fixInFlight` 是发送时派生的投影布尔(不落库)。`reviewStatus
=== 'pending'` 与 `pending:` 前缀都不能代替它 —— 占位在会话已死时仍留在台账上,而关闭自动化时没有队列
会去回收,拿它们判在途会让按钮永久消失,把「失败后可以再点一次」直接取消掉。

**发起是同步的,阶段是异步的。** handler 等到认领落定才返回:认领前的每一种拒绝(非 `worktree`、无活跃
PR、阶段不可发起或在途、Agent 不可用、worktree 不可用、认领写失败)都以 `error` 帧当场回到发起连接,
经 i18n 就地提示,不用意图动作错误对话框 —— 那次点击没有打断任何在途启动。认领成功只广播账本,没有
专用 response 帧。认领之后会话可能跑上半小时,它无结论退出或异常时**不回调发起连接**:那一次点击早已
被答复,再补一个 `error` 只会读作「刚才那次成功作废了」;失败从意图投影与会话 UI 观察,并且不记失败
阶梯、不 park。认领**前**的失败绝不会被这样吞掉 —— 那正是同步等待认领要换来的一致性。

**人工轮次不产生合并凭据。** 手动评审给出的 `approved` 只写结论:凭据的归因要求会话是队列自己那次
评审 run(RM-A27),人工轮次不在其中,因此没有 `merge_grant`、不会有 `merge_prs` 动作,合并仍由人做。
