...
2. **`done` ⇒ 提交并推送(`RM-A5`)。** 队列提交任何未提交的工作(`feat: <title>`,若工作树
   ...
   建 PR 以重读到的意图状态为准(非 `done` 则整体跳过),base 由关联交付解析而来(与手动
   建 PR 同一份解析);**未关联交付时不建 PR**,只记一条 `pr_skipped` 日志,代码留在意图
   分支上;目标不可用(分支未就绪 / 多关联 / 交付未知)时不建 PR 并推一条待办说明原因,
   **绝不回退到主线**。建 PR 的结果不改变已达成的 `done`。若提交被
