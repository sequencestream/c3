...
- **启动后原地落到工作会话 Tab。** 在 `start_development` 完成且启动遮罩关闭
  (`ready` 终态)后,经过一段约 1 秒的「已就绪」缓冲,前端**停留在意图页**:
  它把该意图的 `lastWorkSessionId` 通过 `select_session` 绑定为全局活动会话
  (不进控制台标签页、不切换活动会话类型),并请求意图详情切到**工作会话 Tab**。
  详情页的会话监听在活动会话尚未对齐时补发绑定,只有对齐后才渲染内嵌聊天列,
  避免串台。缓冲到期时若意图已不存在、工作区已切换或 `lastWorkSessionId` 尚未
  回填,则静默放弃本次跳转 — 不建立等待状态,后续广播也不补跳。
  从会话页 / WorkCenter 等外部入口打开工作会话仍走控制台标签页;那条路径上的
  待选等待(`consumePendingWorkSessionSelect`)保持不变。
