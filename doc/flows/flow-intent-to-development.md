...
- `fast` = **仅**绕过手动启动/恢复时的 specApproved 准入闸门;其余闸门全部
  原样保留 —— 依赖判据(`RM-R40`)、交付写入窗口(`RM-R41`)、并发/续跑预算、
  人工决策守卫、权限与工具闭包闸门一条不放松。`approve_spec` 仍是唯一人工验收检查点,只是时序从
  「开发前」移到「diff 产出后」。
...
2. **会话的交付上下文。** 决定 base 的是会话而非意图:`start_development` 可携带
   ...
   复用同一值。上下文定下后,准入闸门依次为交付写入窗口(`RM-R41`)与依赖判据
   (`RM-R40`),后者可被一次性强制放行。worktree 基线不在闸门之列(`RM-R42`)。
3. **Git 分支模式(`WorkspaceSetting.gitBranchMode`)。** `worktree` ⇒ 在 c3 home 目录下
   ...
   不会影响新 worktree 基点的选择。**已存在的 worktree 只做基线检测:不符时照常
   启动并附一条提示,从不自动重建、从不暗中 merge**(`RM-R42`)。工作会话的有效
   工作目录会相应设置(`RM-R8`)。
...
5. **启动反馈(仅限手动启动)。** 因为上述步骤可能耗时数秒
   ...
   **立即显示,并在最短时长内保持可见以防闪烁**,按顺序步进一个
   ...
   不发出任何进度。**留有出口的拒绝另走一条通道**:依赖闸门与「必须选定交付」
   这两类拒绝不只展示文案,而是弹出对应的出口弹窗 —— 前者给带风险说明的强制放行,
   后者给交付选择器。每次拒绝只弹一个弹窗,出口全部是用户的显式选择,系统不代做。
   worktree 基线不符不在此列:它不是拒绝,启动照常,提示与「重建 worktree」/
   「合入基准分支」两个动作常驻意图详情(`RM-R42`)。范围:仅限手动启动 —
