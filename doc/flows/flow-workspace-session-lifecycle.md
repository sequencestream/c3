...
3. Sessions are listed from the `session_metadata` projection, newest-first, one unified
   ...
   counts from the same projection; work, intent, spec, and discussion are live in this phase, while
   schedule / tool remain placeholder tabs. Spec rows carry
   ...
   the discussion page instead of opening as editable work sessions. Each row carries its owning `vendor` tag, `state`, `sessionKind`, and
