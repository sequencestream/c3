...
2. **从厂商原生存储重放。** 一个被追踪的 Codex 会话(`read: 'full'`)从
   `~/.codex/sessions/` 重放其本地 JSONL 基线;一个 Cursor 会话(`read: 'full'`)
   从 `~/.cursor/chats/` 下 Cursor 自己的会话库重放。两者都再加上运行时实时缓冲的
   尾部(`SR-R8`)。解不出的事件/记录会被跳过并让转录更短,而不是使会话选择失败。
