...
3. Sessions are listed from the `session_metadata` projection, newest-first, one unified
   ...
   `sessionKind` slice (work / intent / spec / discussion / automation / tool) and reads running
   counts from the same projection plus domain liveness where needed; work, intent, spec, discussion,
   and automation are live in this phase, while tool remains a placeholder tab. Spec rows carry
   ...
   the discussion page instead of opening as editable work sessions. Automation rows carry
   `ownerKind='automation'` / `ownerId=<automation.id>` and jump back to the automations page. Each row carries its owning `vendor` tag, `state`, `sessionKind`, and
