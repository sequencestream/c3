...
| [intent → development](flow-intent-to-development.md)                | An idea is refined into verifiable intents, then one is launched into a background work session             | intent-management · agent-session · permission-gateway · session-registry · agent-config |
...
| [automation execution](flow-automation-execution.md)                 | A cron/event trigger fires a command or LLM-prompt task under an execution identity; the outcome is logged  | automations · session-registry · agent-session                                           |
...
| [activation & entitlement](flow-activation-and-entitlement.md)       | A user purchases and activates; heartbeats keep the install entitled; lapse gates only new-session creation | product-license · web-console · session-registry · license-server                        |
