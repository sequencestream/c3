...
> 产物、`release:keygen`、`publish:binaries` 均已删除)。分发经**公开 GitHub Release**,
> 完整性由 **sha256 校验和(`.sha256` / `SHA256SUMS`)+ GitHub HTTPS** 提供;发布经
> CI 的 GitHub Release,release notes 由 GitHub `--generate-notes` 基于 PR 历史自动生成。
> 下文凡涉及 minisign / `.minisig` / `c3 verify` / 内嵌公钥的描述均为**历史设计**,已被
> sha256 校验和方案取代。
>
...
> **开源版:** 已移除 minisign 签名与 `c3 verify`。信任底线是 **sha256 校验和 +
> 公开 GitHub Release(HTTPS)**(见 security.md DIST-1/SEC-8)。历史设计以 minisign
> Ed25519 签名 + 内嵌公钥为强制信任锚,已删除。
...
- **内层 sidecar**(在包内,紧挨着 `c3` 二进制文件):`c3.sha256`——由打包步骤
  ...
  会用它校验这个二进制文件。
- **外层 sidecar**(与包并列):`<package>.sha256`——由 `release:sign`(以及
...
> **注意(开源版):** 早期版本在收集完成后会询问是否上传到自托管的产物 store
> (由已下线的 license-server 提供 `/v1/artifact/upload`),或用 `publish:binaries`
> 在本地用 minisign 私钥签名后发到公共镜像仓库。这两条路径都已随 license-server /
> minisign 子系统移除,**开源版的产物分发以 CI 切割的公开 GitHub Release 为准**。
...
pnpm release:sign                                    # SHA256SUMS + 每产物 .sha256(读取 manifest)
...
- **校验和、notes、publish** —— sha256 校验和生成(`release:sign`)以及
  notes/publish 步骤。
- **运行时版本** —— 二进制文件报告的版本字符串。
