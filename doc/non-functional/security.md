...
- **SEC-2**: c3 只持久化**被显式选择的结构化运行数据**——意图/交付/讨论台账、配置、审计轨迹与[工作区记忆](../domains/core/memory/memory-overview.md),全部落在本机单文件 `~/.c3/c3.db`。原始 prompt、对话转录、代码与工具的输入/输出**一律不落盘**:不进磁盘日志,也不得以记忆的形式持久化(记忆的写入路径为此拒绝代码围栏、工具调用/返回框架与角色转录行)。会话与权限的运行态仍只在连接生命周期内存于内存。裁决与其可复核的保障条件见 ADR-0045。
