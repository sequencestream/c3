...
| SEC-14 | **外部 MCP 路由 `/mcp/v1` 以长期 API key 为唯一凭据,且只读。** 该路由服务于 c3 未拉起的 agent,因此不做 loopback 判断;Web 登录会话与内部 per-run token 都不能替代 key。key 以每条独立随机盐 + `scrypt` 哈希落盘,**磁盘上无明文**,明文只在生成响应里出现一次且不可恢复;记录存于 `settings.json` 的顶层兄弟键 `mcpApiKeys`,故整对象 `save_settings` 既不携带也无法注入/读出哈希。每次请求重新校验(进程不缓存"此 key 有效"的结论),吊销下一次请求即失败并关闭已建立的活动 transport。作用域每次请求由 key 授权集合 + `?workspace=` 重建,路径规范化后判等(软链/尾斜杠/点段不可绕过);凭据先于参数校验、403 先于 404,故未授权方既无法探测 key 是否存在、也无法据状态码枚举工作区。工具面是**显式 allowlist**(五个只读工具 + `publish_event`),写工具/会话启动/评审工具一律不注册,新增内部工具不会因遗漏而外泄。已接受的风险:key 走查询参数(为兼容一行式客户端配置),可能进入代理/访问日志;本期不内建也不强制 HTTPS,明文 HTTP 下可被同网络嗅探,远程暴露须由使用者自管 TLS 反代。服务端日志不打印 token,也不打印任何可能带 token 的完整 MCP URL。 |
