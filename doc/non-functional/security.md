...
- **SEC-14**: **外部 MCP 路由 `/mcp/<api-key>` 以长期 API key 为唯一凭据,key 即地址段且绑定单一工作区。** 该路由服务于 c3 未拉起的 agent,因此不做 loopback 判断;Web 登录会话与内部 per-run token 都不能替代 key。key 以每条独立随机盐 + `scrypt` 哈希落盘,**磁盘上无明文**,明文只在生成响应里出现一次且不可恢复;记录存于 `settings.json` 的顶层兄弟键 `mcpApiKeys`,故整对象 `save_settings` 既不携带也无法注入/读出哈希。每次请求重新校验(进程不缓存"此 key 有效"的结论),吊销下一次请求即失败并关闭已建立的活动 transport。作用域由 key 绑定(单一工作区 + 该 key 工具范围)每次请求重建;**凭据先于一切校验**,缺失/格式错/未知/吊销一律同一 401,未授权方无法探测 key 是否存在;工作区不可用 403,会话换 key 403,旧 `/mcp/v1?token=` 明确 410 停用。工具面是**显式 allowlist** + **逐 key 勾选**:默认只读(`find_intents`/`view_intent`/`find_discussions`/`view_discussion`/`publish_event`),写工具(`save_intents`/`submit_spec_review`/`start_session_for_intent` 等)必须由管理员在创建后显式勾选并确认风险;`tools/list` 精确等于勾选子集,未勾选工具调用返回稳定 forbidden 且不执行 handler,新增内部工具不会因遗漏而外泄。写接口会真实修改 c3 状态,属有意放宽——key 默认只读、可吊销、写接口须显式勾选。已接受的风险:key 本身即地址、走 URL 路径,可能进入代理/访问日志;本期不内建也不强制 HTTPS,明文 HTTP 下可被同网络嗅探,远程暴露须由使用者自管 TLS 反代。服务端日志不打印 key,也不打印任何可能带 key 的完整 MCP URL。
...
- 外部 MCP 路由上出现任何写工具、会话启动工具或评审工具;或者一把 key 读到了它未被授权的工作区;或者已吊销的 key 仍能通过一条早先建立的 MCP 会话继续调用(SEC-14)。
