...
| SEC-8  | 分发信任(DIST-1):发布的二进制文件携带逐工件的 sha256 校验和(每产物 `.sha256` + 汇总 `SHA256SUMS`);产物经**公开 GitHub Release**(HTTPS)分发,使用者可用 `shasum -a 256 -c` 校验完整性。开源版**不含** minisign 签名与 `c3 verify` 命令。                                                                                                                                                                                                                                                                                                                                                         |
...
由于 SEC-6 已经把凭据挡在二进制之外,真正的分发威胁是**工件冒充/供应链篡改**(恶意镜像或 MITM 分发被植入木马的 `c3`),而不是逆向工程。**开源版**的信任由**公开的 GitHub Release(HTTPS)+ sha256 校验和**提供:
...
> **历史设计(已移除):** 早期版本以一把**离线的 Ed25519 minisign 签名密钥**为强制信任锚——每个工件与 `SHA256SUMS` 都带 `.minisig` 签名,二进制内嵌公钥并提供 `c3 verify <file>` 离线自校验,`c3 upgrade` 也以 minisign 为强制门控。**开源版已移除 minisign 签名与 `c3 verify`**,完整性改由上述 sha256 校验和 + GitHub HTTPS 提供。
