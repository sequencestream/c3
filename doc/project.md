...
- 不是一个托管的/多租户服务。
- 不是厂商 CLI 认证的替代品。c3 在 `~/.c3/vendor` 下管理受管厂商(claude / codex)的默认
  CLI 二进制文件,但厂商 CLI 的登录状态仍归厂商所有;`cursor-agent` 连分发也不归 c3。
- 不是一个多项目工作区。工作区通过 Web UI 管理，一个服务端进程可服务多个工作区。
...
- 任何被 SDK 归类为敏感的工具,都绝不会在没有浏览器明确决策(或没有授权自动执行的
  明确模式)的情况下执行。
- 单一二进制文件可以配合 `~/.c3/vendor` 下由 c3 管理的 Claude Code / Codex CLI,以及使用方
  自装的 `cursor-agent` 运行;环境变量覆盖与宿主 PATH 回退仍然可用于运维与迁移场景。
