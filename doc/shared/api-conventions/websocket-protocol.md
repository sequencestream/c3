...
`workspaceName` 把清单收窄到某个工作区已配置的 MCP 服务器（其 `mcp__<server>__` 命名空间前缀并入清单）；省略它——聊天机器人场景，机器人是部署级管理对象、清单不含工作区命名空间——这是管理契约，不是数据访问安全保证。`scope` 标明提问的是哪张网格，服务器原样回显：回复可能在提问的表单关闭之后才到达，客户端据此路由到对应缓存而不是靠猜。
...
- **`ProposedIntent`** — `{ id?, status?: 'todo', automate?, title, shortEnTitle, content, priority, module?, dependsOn?, dependsOnIndexes?, intentSessionId? }`。`save_intents` 调用中的一个项。`shortEnTitle` 是必填的简短英文 ASCII 标题，用作后续分支/worktree 命名的稳定来源。有 `id` 时 upsert（更新同项目已存在的 intent）；无 `id` 时插入新 `Intent`（省略字段时为 `todo + automate=false`）。显式 `status` 只支持 `todo`，仅用于 `draft`/`cancelled` 激活及 todo 幂等保存；显式 `automate=true` 要求最终状态为 todo。upsert 省略 `automate` 保留原值，省略 `status` 沿用 draft/todo 保持与 cancelled 恢复 todo 的兼容规则。`intentSessionId` 是把本条意图回链到产出它的沟通会话的可选字段，**仅当本批只保存 1 条意图时才生效**（批量 >1 条时落库核心一律忽略，不写入任何行）；模型填入提示中注入的当前会话 id，保存处理器再将其归一化为 bind 后的真实会话 id（`open_intent_session` 可解析）。`save_intent_directly`（automation 路径）的 schema 不含 `id`、`status`、`automate` 或 `intentSessionId`，固定新建 `draft + automate=false`。
