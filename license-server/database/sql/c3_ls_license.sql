...
CREATE TABLE IF NOT EXISTS c3_ls_license (
    ...
    buyer_id         BIGINT      NOT NULL,         -- Buyer that owns the license.
    order_id         BIGINT,                       -- Purchase order that last extended this license, if any.
    plan_id          TEXT        NOT NULL,         -- License plan id.
    license_key      TEXT        NOT NULL UNIQUE,  -- Random unique handle passed on the c3<->LS bind/heartbeat API.
    status           TEXT        NOT NULL DEFAULT 'active', -- License status: active, revoked, expired.
    ...
    revoked_at       TIMESTAMPTZ,                  -- Time the license was revoked, when applicable.
    created_at       TIMESTAMPTZ NOT NULL DEFAULT now() -- Record creation time.
);

CREATE INDEX IF NOT EXISTS idx_c3_ls_license_buyer ON c3_ls_license(buyer_id);
...
COMMENT ON COLUMN c3_ls_license.buyer_id IS 'Buyer that owns the license.';
COMMENT ON COLUMN c3_ls_license.order_id IS 'Purchase order that last extended this license, if any.';
COMMENT ON COLUMN c3_ls_license.plan_id IS 'License plan id.';
COMMENT ON COLUMN c3_ls_license.license_key IS 'Random unique handle passed on the c3<->LS bind/heartbeat API.';
COMMENT ON COLUMN c3_ls_license.status IS 'License status: active, revoked, or expired.';
...
COMMENT ON COLUMN c3_ls_license.revoked_at IS 'Time the license was revoked, when applicable.';
