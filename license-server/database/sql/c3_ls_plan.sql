...
-- Business rules: PL-R9 (orders reference a plan_id from this catalog).
--
...
-- id is the internal auto-increment identity; plan_id is the stable public plan
...
CREATE TABLE IF NOT EXISTS c3_ls_plan (
    ...
    plan_id         TEXT        NOT NULL UNIQUE,      -- Stable public plan id (referenced by orders/licenses); never changes once published.
    name            TEXT        NOT NULL,             -- Short human-readable label.
    ...
    currency        TEXT        NOT NULL DEFAULT 'CNY', -- ISO-4217 currency code the price is denominated in.
...
COMMENT ON COLUMN c3_ls_plan.plan_id IS 'Stable public plan id, referenced by orders/licenses; never changes once published.';
