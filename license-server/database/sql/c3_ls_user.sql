-- c3_ls_user — GitHub user/admin identity.
...
CREATE TABLE IF NOT EXISTS c3_ls_user (
    id           BIGSERIAL PRIMARY KEY,       -- Internal user identity.
    ...
    email        VARCHAR(128),                 -- User email when GitHub provides it.
    created_at   TIMESTAMPTZ NOT NULL DEFAULT now() -- Record creation time.
);

COMMENT ON TABLE c3_ls_user IS 'GitHub identity for license users and license-server admins; GitHub is used for account login/registration only.';
COMMENT ON COLUMN c3_ls_user.id IS 'Internal user identity.';
...
COMMENT ON COLUMN c3_ls_user.email IS 'User email when provided by GitHub.';
