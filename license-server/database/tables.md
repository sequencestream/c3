...
| c3_ls_plan     | Persisted public purchasable plan catalog; bootstrapped from code, served at GET /v1/plans | id (PK, auto), plan_key (unique), name, duration_months, price_cents, currency, sort_order, tier |
| c3_ls_order    | Purchase record + service-agreement acceptance; status machine `pending→paid/failed/expired`, a paid order renews/upgrades the linked license (PL-R9) | order_no (unique; WeChat out_trade_no), user_id, license_id, plan_key, amount_cents, agreement_version, agreement_accepted_at, status, payment_ref |
| c3_ls_license  | Authoritative entitlement keyed by license_key, carrying its tier and exclusive live binding | license_key (unique), user_id, status, tier, alive_install_id, alive_token, alive_time, term_end |
...
callback and the reconcile job resolve an order by `order_no`, not the internal id.

An order moves through `pending → paid` (or `pending → failed`/`expired`). A
verified WeChat payment success — via the async callback or the 20-minute
reconcile job — records the WeChat transaction id in `payment_ref`, flips
`status` to `paid`, and renews or upgrades the linked license's `term_end`/`status`/`tier` in one
transaction. Same-tier paid/enterprise renewals extend from the later of now and the current term end.
Free→paid/enterprise purchases and paid→enterprise upgrades replace the term from now because the
free tier is long-lived and paid→enterprise remaining value is converted into checkout credit. The
transition is idempotent (a redelivered callback finds the order already settled
and does not re-extend), and `payment_ref` is the **only** payment artifact
stored — no credentials (PL-R12). A pending order whose 15-minute payment window
lapses (WeChat closed it, or the reconcile job ages it out) moves to `expired`
and never extends a license.

The plan catalog is bootstrapped from the code-owned paid/enterprise set (`internal/plans`) into
...
when the database is unavailable. The free tier is not purchasable and is represented by
`c3_ls_license.tier='free'`.
