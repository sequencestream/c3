...
describe('signArtifacts', () => {
  function fixture() {
    ...
    // Release 8/7: the signer signs the PACKAGE, not the raw binary. The
    // outer sidecars (`<pkg>.sha256`, `<pkg>.minisig`) sit next to the
    // package at the dist root; the inner sidecars (`c3.sha256`,
    // `c3.minisig`) are generated by pack.mjs and live INSIDE the package.
    const a = {
      name: 'c3-v9.9.9-test-x64.tar.gz',
      path: resolve(dir, 'c3-v9.9.9-test-x64.tar.gz'),
    }
    writeFileSync(a.path, Buffer.from('fake package payload ' + 'z'.repeat(1000)))
    return { dir, a }
  }
...
describe('c3 verify — runVerify over sidecar files', () => {
  function artifactFixture() {
    ...
    // Release 8/7: the artifact `c3 verify` is invoked on is the PACKAGE.
    // The sidecars name the package (the consumer-visible artifact).
    const path = resolve(dir, 'c3-v9.9.9-test-x64.tar.gz')
    const content = Buffer.from('payload ' + 'q'.repeat(2000))
    writeFileSync(path, content)
    const kp = generateKeypair()
    writeFileSync(
      `${path}.sha256`,
      `${createHash('sha256').update(content).digest('hex')}  c3-v9.9.9-test-x64.tar.gz\n`,
    )
...
describe('release:publish --dry-run', () => {
  it('plans the publish without creating a tag or calling gh', () => {
    ...
    writeFileSync(
      manifestPath,
      JSON.stringify({ version: '9.9.9', artifacts: [{ file: 'c3-v9.9.9-test-x64.tar.gz' }] }),
    )
