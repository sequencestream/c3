...
export function buildAdvisorC3Tools(
  ...
  return [
    ...
    define(
      'create_pr',
      '为绑定意图创建 PR(需人工确认)。复用人工路径的全部前置校验:仅 worktree 模式、' +
        '必须有分支、目标交付必须可用(存在且分支就绪)、该目标已有活跃 PR 则幂等拒绝、' +
        '工作树必须与目标 base 有差异;提交或推送失败时不会创建 PR。目标由服务端解析:' +
        '意图未关联交付则提向工作区主分支,恰好关联一个则提向该交付分支,关联多个则拒绝。',
      {},
      async () => {
        const blocked = guard(scope)
        if (blocked) return blocked
