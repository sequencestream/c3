...
describe('automation orchestrator', () => {
  it('develops eligible intents P0→P1, commits each, ends done', async () => {
  ...
  it('flips the intent to in_progress as soon as the dev session binds (before the turn ends)', async () => {
    const [r] = insertIntents(proj, [{ title: 'early', content: 'c', priority: 'P0' }])
    setAutomate(r.id, true)
    judgeMock.mockResolvedValue({ verdict: 'done', reason: 'ok' })
      ...
      const hooks: AutomationHooks = {
        ...
        emitStatus: (s) => {
          ...
          if (s.currentSessionId === 'sess-early' && statusAtBind === undefined) {
            statusAtBind = getIntent(r.id)?.status
            sessionAtBind = getIntent(r.id)?.lastDevSessionId
  ...
  it('resumes an in_progress intent whose lastDevSessionId still exists on disk', async () => {
    const [r] = insertIntents(proj, [{ title: 'resume-me', content: 'c', priority: 'P0' }])
    setAutomate(r.id, true)
    // Simulate a half-built dev session linked to the intent.
  ...
  it('falls back to a fresh session when the lastDevSessionId is dangling', async () => {
    const [missing] = insertIntents(proj, [{ title: 'gone', content: 'c', priority: 'P0' }])
    const [blank] = insertIntents(proj, [{ title: 'blank', content: 'c', priority: 'P1' }])
    ...
    // No session exists on disk → both dangling intents start fresh (null).
  ...
  it('attaches to (does not relaunch) a intent whose dev session is already running', async () => {
    const [r] = insertIntents(proj, [{ title: 'attach-me', content: 'c', priority: 'P0' }])
    setAutomate(r.id, true)
  ...
  it('does NOT attach when the dev session is not running (falls back to resume/fresh)', async () => {
    const [r] = insertIntents(proj, [{ title: 'not-live', content: 'c', priority: 'P0' }])
    setAutomate(r.id, true)
    updateStatus(r.id, 'in_progress')
  ...
  it('global gate blocks when a non-automate manual run is in progress, waits for it, then proceeds', async () => {
    // A non-automate intent already in_progress with a running dev session.
    const [manual] = insertIntents(proj, [{ title: 'manual-work', content: 'c', priority: 'P0' }])
    updateStatus(manual.id, 'in_progress')
    setLastDevSession(manual.id, 'sess-manual')

    // An automate intent eligible for the orchestrator.
    const [auto] = insertIntents(proj, [{ title: 'auto-work', content: 'c', priority: 'P1' }])
    ...
    // Second call: develop picks the automate intent (fresh launch).
  ...
  it('dangling (exists but not running) does not block the global gate', async () => {
    // A non-automate intent in_progress but NOT running (dangling on disk).
    const [manual] = insertIntents(proj, [{ title: 'dangling', content: 'c', priority: 'P0' }])
    updateStatus(manual.id, 'in_progress')
  ...
  it('skips intents without the automate flag', async () => {
    const [on, off] = insertIntents(proj, [
    ...
    expect(getIntent(off.id)?.status).toBe('todo') // untouched
  })

  it('does not pick a intent whose dependency is unfinished', async () => {
  ...
  it('stops (does NOT continue) when the turn ends on an unanswered question, even if the judge says in_progress', async () => {
    const [r] = insertIntents(proj, [{ title: 'asks', content: 'c', priority: 'P0' }])
    setAutomate(r.id, true)
  ...
  it('stops (single agent attempt only) when the lint failure survives the agent fix; not marked done', async () => {
    const [r] = insertIntents(proj, [{ title: 'stubborn', content: 'c', priority: 'P0' }])
    setAutomate(r.id, true)
    judgeMock.mockResolvedValue({ verdict: 'done', reason: 'ok' })
  ...
  it('does NOT self-heal a non-lint commit failure (other) — surfaces it directly, no agent turn', async () => {
    const [r] = insertIntents(proj, [{ title: 'other-fail', content: 'c', priority: 'P0' }])
    setAutomate(r.id, true)
    judgeMock.mockResolvedValue({ verdict: 'done', reason: 'ok' })
    commitMock.mockResolvedValue({
      ok: false,
  ...
  it('stop() aborts and returns to idle', async () => {
    const [r] = insertIntents(proj, [{ title: 'long', content: 'c', priority: 'P0' }])
    setAutomate(r.id, true)
    judgeMock.mockResolvedValue({ verdict: 'done', reason: 'ok' })
