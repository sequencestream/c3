 ...
 * `automate` intents one at a time, by priority then dependency order,
 * fully unattended.
 *
 * For each eligible intent it:
 ...
 *   3. if done → commit & push → mark `done` → next intent;
 ...
 * is `done`. Sorted P0→P3 then oldest-first. If the intent's linked dev
 ...
 * preempt). Otherwise an `in_progress` intent whose `lastDevSessionId` still
 ...
 */
import type { AutomationStatus, Intent, ServerToClient } from '@ccc/shared/protocol'
...
export interface RunDevTurnInput {
   ...
   * the turn ends), so the orchestrator can flip the intent to `in_progress`
...
export interface AutomationHooks {
  ...
  /** Push the project's updated intent list to all connections. */
   ...
   * for a intent's `lastDevSessionId`, the orchestrator ATTACHES to that
 ...
 * `intents` (only the automation handlers read it) — NOT carried on the
...
/** Max continue resumes per intent before giving up (clears dev-skill checkpoints). */
...
/** Eligible intents, best candidate first; null when none remain. */
...
class AutomationController {
   ...
   * intent, check whether ANY in_progress intent (including
  ...
  private async awaitProjectRunning(): Promise<boolean> {
    ...
    // Track which intent we're waiting on for status display.
    this.status.currentIntentId = running.id
    ...
    // A non-automate intent that errored or blocked on a permission:
    // log the event but do NOT stop the orchestrator — the gate only waits;
    // the manual intent's lifecycle is outside our scope.
    if (turn.outcome !== 'complete') {
      console.warn(
        `[c3:automation] gate: "${running.title}" turn ended (outcome=${turn.outcome}); non-automate intent, continuing`,
      )
    }

    // Loop back to re-check the gate (another intent may have started
  ...
  async run(): Promise<void> {
    ...
    while (!this.abort.signal.aborted) {
      // RM-A12: Global concurrency gate — wait for any in_progress intent
      // (including non-automate manual runs) with a truly running dev session
      // to settle before picking the next automate intent.
      ...
      if (this.abort.signal.aborted) {
        // Stopped by the user mid-intent: go quiet (idle), keep no error.
        this.status.state = 'idle'
        this.status.currentIntentId = null
        this.status.currentSessionId = null
        this.emit()
  ...
  /**
   * Develop one intent to a terminal state. Returns true if it was committed
   * (move to the next), false if the loop must stop (error already recorded).
   */
  private async develop(req: Intent): Promise<boolean> {
    // If the intent's linked dev session is ALREADY running a turn in the
    ...
    // Otherwise resume the intent's existing dev session when it's still on
   ...
   * the error is surfaced and the loop stops (RM-A6, intent not `done`).
   *
