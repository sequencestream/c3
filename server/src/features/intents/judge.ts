 ...
 * intent is done" — the dev skill is often checkpoint-driven, so a turn often ends
 * paused, not complete. This module asks a fresh, tool-less Claude
 * (see {@link askOneShot}) to judge the intent primarily against the agent's
 ...
 *
 *  - `done`        — the intent is achieved per a credible agent report;
 ...
 * call (a tool-free one-shot), NOT a user-facing run — it does NOT go through the
 * run bus (its execution form is `runKind: 'internal'`). Tagged `'tool'` so
 * logs/audit can distinguish it from user sessions.
 */
