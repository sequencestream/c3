/**
 * Tests for the requirement reconcile logic. The reconcile is dependency-injected,
...
// Shared base requirement (in_progress) reused across tests; tests extend it.
...
describe('reconcileInProgress', () => {
  describe('branch 1 — process still running', () => {
    it('returns running for a requirement whose dev session is in-flight', async () => {
  ...
  describe('edge cases', () => {
    ...
    it('processes multiple requirements independently', async () => {
      // Three requirements: one running, one done, one dangling
      ...
      expect(results[1]).toEqual({ requirementId: 'r2', runStatus: 'idle', autoCompleted: true })
