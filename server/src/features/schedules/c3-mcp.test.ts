/**
 * Tests for the schedule-only c3 MCP profile and its `save_intent_directly`
 * write. Two contracts:
 ...
 *  - Injection boundary: `createScheduleMcpServer` registers `save_intent_directly`
 *    while the interactive `createIntentMcpServer` does NOT — the gate-bypassing
 *    write is pinned to unattended schedule executions only.
 */
...
describe('schedule c3 MCP injection boundary', () => {
  it('registers save_intent_directly on the schedule server', () => {
    const names = registeredToolNames(createScheduleMcpServer(proj, 'exec-1'))
