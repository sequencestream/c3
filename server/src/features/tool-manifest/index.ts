 ...
 *
 * Two surfaces consume it: the automation form and the chat-robot form. Both ask
 * the same question ("what can this vendor be widened to, and which of those
 * write?"), so they get the same answer from the same place. A manifest that
 * lived inside `features/automations` would have made the robot's copy either an
 * import across a domain that does not own it, or a second list free to drift —
 * and a drifting classification is exactly how a write tool ends up rendered as
 * read-only.
 *
...
/**
 * c3's own MCP tools, offered to every known vendor regardless of workspace MCP
 * configuration: they are served over c3's loopback HTTP MCP route, not by a
 * user-configured server, so no adapter's `listTools()` knows about them.
 *
...
/** The c3 MCP tools an allowlist selected, as bare names (`find_intents`, …). */
export function selectedC3McpToolNames(toolAllowlist: readonly string[]): string[] {
  return C3_MCP_TOOLS.filter((tool) => toolAllowlist.includes(tool.name)).map((tool) =>
 ...
 * configured MCP servers contribute their namespace prefixes; without one — the
 * chat-robot case, which is not scoped to a workspace — only the vendor's
 * built-ins and c3's own tools are offered.
 */
export function buildToolManifest(vendor: string, workspaceName?: string): ToolManifestEntry[] {
  ...
  return [...vendorSdkTools(vendor, workspaceName, mcpServers), ...C3_MCP_TOOLS]
}
...
export const getToolManifest: Handler<'get_tool_manifest'> = (_ctx, conn, msg) => {
  conn.send({
    type: 'tool_manifest',
    vendor: msg.vendor,
    tools: buildToolManifest(msg.vendor, msg.workspaceName),
