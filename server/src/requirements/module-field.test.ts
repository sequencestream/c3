/**
 * Integration tests for the requirement `module` field (新增"模块名称"字段).
 *
 * Covers the acceptance criteria from requirement.md scenarios 1–5 and design.md
 ...
 *   - end-to-end module round-trip through the REAL save_requirements handler
 *     incl. the omitted-module '' fallback (scenarios 3 & 4),
 *   - module carried by listRequirements / getRequirement / insertRequirements
...
// --- save_requirements handler plumbing (mirrors save-tool.test.ts) -----------
...
describe('module field — fresh-schema create (scenario 1)', () => {
  it('a brand-new db gets a requirements.module column declared NOT NULL DEFAULT ""', () => {
    // Scenario 1 / design §3.1: first store access on a fresh db runs SCHEMA, so
    // PRAGMA table_info(requirements) must list `module` with notnull=1 and a
...
describe('module field — pre-v2 migration extensions (scenario 2)', () => {
  /** Build an old-schema (no `module`) requirements table with N historic rows. */
  ...
  it('backfills module="" for MULTIPLE historic rows (none lost) and lands at current version', () => {
    ...
    const raw = getDb()!
    expect(raw.get<{ user_version: number }>('PRAGMA user_version')?.user_version).toBe(5)
  })
  ...
  it('a row inserted AFTER migration coexists with legacy rows and carries its module', () => {
    ...
    // Trigger migration, then insert a fresh requirement with a module.
    resetStoreForTests()
    const [fresh] = insertRequirements(proj, [
      { title: 'New', content: '', priority: 'P0', module: '需求管理' },
    ])

    expect(getRequirement('old-1')?.module).toBe('') // legacy backfilled
    expect(getRequirement(fresh.id)?.module).toBe('需求管理') // new value kept

    // Both visible through the list, each with the right module.
    const byTitle = new Map(listRequirements(proj).map((r) => [r.title, r.module]))
    expect(byTitle.get('Legacy')).toBe('')
    expect(byTitle.get('New')).toBe('需求管理')
  })
})

describe('module field — save_requirements end-to-end (scenarios 3 & 4)', () => {
  it('round-trips a batch WITH module values through the real handler per row', async () => {
    ...
    // module must equal what was given, readable via listRequirements.
    const onSaved = vi.fn()
...
describe('module field — read-back carries module (scenario 5)', () => {
  it('insertRequirements / listRequirements / getRequirement all return module per row', () => {
    ...
    // insertRequirements return value carries module.
    ...
    // listRequirements carries module.
    const listed = new Map(listRequirements(proj).map((r) => [r.title, r.module]))
    expect(listed.get('A')).toBe('mod-a')
    expect(listed.get('B')).toBe('mod-b')
    // getRequirement carries module.
    const a = saved.find((r) => r.title === 'A')!
    expect(getRequirement(a.id)?.module).toBe('mod-a')
  })
})
...
describe('module field — degradation contract not regressed (constraint)', () => {
  it('with the db unavailable, reads stay empty and writes still throw', () => {
    ...
    expect(() =>
      insertRequirements(proj, [{ title: 'X', content: '', priority: 'P0', module: 'm' }]),
    ).toThrow()
  })
})
