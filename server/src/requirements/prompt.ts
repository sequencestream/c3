...
- Optional: ids of other requirements it depends on (within the same project)
...
5. After the user approves, call the \`save_requirements\` tool to submit the batch, attaching each item's inferred \`module\` name (omit if unsure). The system shows a confirmation dialog; **nothing is persisted unless the user clicks "Save"**.
