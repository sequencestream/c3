...
`broadcastRequirements`, `broadcastDiscussions`, `broadcastSchedules`, `broadcastDiscussionMessage`,
...
- **kernel/** — pure domain: the session-runtime registry, settings lookup, requirements /
...
| R5  | A new `mode` (permission mode, run `kind`, etc.) MUST NOT cross a kernel boundary as an implicit switch — every kernel function that depends on mode MUST narrow it explicitly at the call site (`if (rt.kind === 'requirement') ...` in the kernel, not `if (msg.mode === 'default')` in a feature). | review + contract tests pinning the narrow; becomes a lint rule in 2/3.                                                                           |
