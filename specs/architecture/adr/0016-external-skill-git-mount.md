...
2. **vendor 范围**:`claude` 与 `codex` 为已验证可建链 vendor;`opencode` 暂缓(未实证,`detectSkillSupport` 非 `full` → 自动跳过)。
...
4. ~~**pinned 防伪**~~ **已移除**(简化):不再有 `trust='pinned'`/`pinCommit`,统一静默挂载 `ref` 最新版;
...
- 协议 `SkillRepoConfig`(`id`/`repo`/`ref`/`subpath?`)+ `ProjectConfig.skillRepos?` 落 `shared/protocol.ts`(`SkillTrust`/`SkillVendor` 已删)。
- `getSkillRepos(projectPath)`(`server/src/kernel/config/index.ts`)从 `loadProjectConfig` 读取并 fail-hard 校验(id/repo/ref);git 操作层落 `server/src/skill-repo.ts`。
