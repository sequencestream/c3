...
- 协议三类型 `SkillTrust`/`SkillVendor`/`SkillRepoConfig` + `SystemSettings.skillRepos?` 落 `shared/protocol.ts`。
- `getSkillRepos()`(`server/src/kernel/config/index.ts`)fail-hard 校验;git 操作层落 `server/src/skill-repo.ts`。
