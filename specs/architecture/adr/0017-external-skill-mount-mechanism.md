...
### D3 — ~~trust 三档审批~~ → 静默挂载最新版(2026-06-07 简化)

**已废弃** pinned/review-on-update/unreviewed 三档。现状:`ensureLinksForLaunch` 对每个 config 解析 `ref` 最新
SHA(`lsRemote`)→ 命中已记录且 ref 未变的软链则 skip → 否则 `ensureSkillRepo`(clone/pull head)+ `ensureLink`
软链,**全程无 trust 弹窗**。`detectSkillSupport` 非 `full` 的 vendor(如 opencode)自动跳过(标灰,不阻断启动)。

唯一保留的人工闸是 `.gitignore` 追加:按**项目维度**一次性 ack,落 `state.json.skillAcks[projectDir].gitignore`,
确认后永久静默;取消则该 skill 不挂载(session 仍启动)。
...
- 3/3 仅渲染一个 `.gitignore` 模态框(简化后 trust、orphan 模态框已删)。
- codex 项目级缺口暴露一个已知风险:需要一个由 codex 用户验证的补证 spike。
- 软链方案使 skill 升级成本低(`git fetch + reset --hard` 即可更新),且三 vendor 共享同一份克隆。
- 静默挂载下不再有「未消费链接」概念,orphan 启动扫描随之删除。

## Compliance

- `SkillLoader` 接口 + 三 vendor 实现落 `adapters/{claude,codex,opencode}/skill.ts`。
- `ensureLinksForLaunch` 落 `kernel/skill-loader/`(`scanOrphans`/`markMountsConsumed` 已删)。
- `.gitignore` ack 状态机落 `kernel/skill-loader/approval.ts`(trust 网关已删)。
- 写操作守卫落 `kernel/permission/gateway.ts`(`skillWriteGuard`,基于 `hasMountedSkills`,与 trust 无关)。
- 所有模块各有单测。
