...
ADR-0021 确立了双层沙箱配置：管理员定义 `SystemSandboxDef` 模板，项目（工作区）通过 `WorkspaceSandboxConfig` 引用并覆盖少量字段。当时 `networkDisabled`（禁用网络）和 `readonlyRootfs`（只读根文件系统）都放在 `SystemSandboxDef` 上。

实践中暴露三个问题：

1. **语义错位**：网络是否放通、根文件系统是否可写，取决于具体工作区跑什么活（某些构建/缓存需要可写根、需要拉依赖），属于工作区各自的安全姿态，而非全安装共享的模板属性。放在系统定义里既不灵活，又与「工作区 override」语义重叠让人困惑。
2. **死字段**：`mergeSandboxConfig` 当时只读 `systemDef.networkDisabled`，完全忽略 `WorkspaceSandboxConfig` 上已声明的同名 override —— 协议里有这个字段，但从未接线，用户在工作区里改它毫无效果。
...
`networkDisabled`/`readonlyRootfs` 从 `SystemSandboxDef` 移除，仅在 `WorkspaceSandboxConfig` 承载；`mergeSandboxConfig` 从工作区配置解析，缺省 `networkDisabled: true`、`readonlyRootfs: true`。
...
1. **协议 / 类型**：`SystemSandboxDef`（`shared/src/protocol.ts` + `server/src/kernel/sandbox/types.ts`）删除 `networkDisabled`、`readonlyRootfs`；`WorkspaceSandboxConfig` 保留 `networkDisabled` 并新增 `readonlyRootfs?`。`networkAllowlist` **仍留在系统定义**（管理员管控的 egress SPI，Phase 2 未支持）。
2. **合并**：`mergeSandboxConfig` 从 `projectCfg` 解析这两项，**deny-by-default**：
   - 缺省 `networkDisabled = true`（`--network none`）。
   - 缺省 `readonlyRootfs = true`（只读根文件系统）。
3. **持久化**：`normalizeSandboxConfig` 对这两项持久化显式布尔值（`true` 与 `false` 都保留），使工作区可以**放开**某项策略（例如 `networkDisabled: false` 真正放通网络），而不是只能收紧。
4. **旧值剔除**：系统定义里设过这两项的旧值，在 zod parse 时直接被 strip（首读即丢弃），统一回退新默认，不做迁移。
...
- `mergeSandboxConfig` 须从 `projectCfg` 解析 `networkDisabled`/`readonlyRootfs`，缺省 `true`/`true`；`_AssertEqual` 类型 pin 仍通过。
- `SandboxRegistry.test` 须覆盖：工作区 override 生效（`false` 放开）、未设置 deny-by-default（`true`）、系统定义旧值被剔除。
- `normalize-sandbox.test` 须覆盖：显式 `true`/`false` 双向持久化、未设置时省略（交由 merge 兜底）。
- `i18n:check` 通过：文案从 `settings.sandboxes.*` 迁至 `workspaceSetting.sandbox.*`，五语同步。
...
- [SandboxConfig.ts](../../../server/src/kernel/sandbox/SandboxConfig.ts) — merge + zod schema
- [config index](../../../server/src/kernel/config/index.ts) — `normalizeSandboxConfig`
