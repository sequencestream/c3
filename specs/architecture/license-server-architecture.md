...
c3                                       LS
 ...
 │◄── status(active/disabled/revoked/expired) + (active 时)刷新实体令牌 + 下次 interval
```
...
- `status` 在签发时恒为 `active`;**失效(grace/expired/revoked)由 c3 端**从令牌有效期窗口与心跳推导,
  绝不签发一个非 active 的令牌。
...
| 变量                               | 何时必需 | 说明                                    |
| ---------------------------------- | -------- | --------------------------------------- |
| `LS_DATABASE_URL`                  | 启用库时 | PostgreSQL DSN(机密);省略则 dbless 运行 |
| `LS_LISTEN_ADDR`                   | 否       | 监听地址,默认 `:8787`                   |
| `LS_PUBLIC_URL`                    | 激活时   | 外部基址(OAuth 回调)                    |
| `LS_ED25519_PRIVATE_KEY`           | 激活时   | 令牌签名私钥(机密,仅 LS 持有)           |
| `LS_ED25519_PUBLIC_KEY`            | 激活时   | 校验公钥(发布给 c3 内嵌)                |
| `LS_GITHUB_OAUTH_CLIENT_ID/SECRET` | 激活时   | GitHub OAuth 应用凭证(secret 为机密)    |
| `LS_GRACE_MINUTES`                 | 否       | 离线 grace 窗口,默认 `30`(PL-R4)        |
| `LS_ADMIN_ALLOWLIST`               | 否       | 逗号分隔的 admin GitHub login(PL-R11)   |
...
make test       # 单元/构建检查;设 LS_TEST_DATABASE_URL 则跑实库迁移测试
```

迁移在 `LS_DATABASE_URL` 配置时**启动自动应用**(幂等,重跑为 no-op)。二进制由本目录自有 Go module 构建,
...
- ✅ **心跳**:`/v1/license/heartbeat` 以 alive token 周期确认绑定 + 刷新令牌 + grace 窗口;绑定被取代/吊销/到期分别返回 `disabled`/`revoked`/`expired`。
- ⏳ **支付/续期**:WeChat Pay(CNY 结算)付费订单延长 license 的 term/status;MVP 为**无退款**(故意业务非目标,PL-R9/PL-R10)。
- ⏳ **admin 后台**:GitHub OAuth + allowlist 鉴权下的许可证/订单审查与 issue/revoke。
