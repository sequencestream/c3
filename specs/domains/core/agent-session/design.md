...
| `cwd`                             | project path (`--project`) | where Claude reads/writes (AS-R1)                                                |
| `settingSources`                  | `[]`                       | ignore external settings — ADR 0001 / C-SEC-1                                    |
| `permissionMode`                  | session's current mode     | starting policy (AS-R3)                                                          |
...
`set_mode` can call `q.setPermissionMode(mode)` (AS-R4).

## Per-connection state (server.ts)

| Field         | Type                      | Lifetime                                   |
| ------------- | ------------------------- | ------------------------------------------ |
| `currentMode` | `PermissionMode`          | whole connection; starts `default` (AS-R3) |
| `runAbort`    | `AbortController \| null` | per run; `null` between runs               |
| `runHandle`   | `RunHandle \| null`       | per run; `null` between runs               |

On `user_prompt`: `runAbort?.abort()` (AS-R2), create a fresh `AbortController`, call
`runClaude`. In `finally`, if the controller is still current, clear `runAbort`/`runHandle`
so a later prompt never aborts an already-closed query (AS-R6).
...
- **No persistence** — all state in-memory per connection (SEC-2).
