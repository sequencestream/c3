...
| [requirement-management](requirement-management/requirement-management-overview.md) | Project-scoped requirement ledger (SQLite); read-only requirement-communication agent; `save_requirements` confirmation; launch `/develop-pipeline` | WebSocket `/ws` (see shared protocol) | active |
