...
| `participantAgentIds` | string[]                | Agents selected at creation to participate; the orchestrator nominates from this set only (∪ the always-included organizer). `[]` = unset/legacy ⇒ fall back to the whole `enabledAgents()` roster. Persisted as JSON (`participant_agent_ids`) |
