...
managed via `PRAGMA user_version`. Tables: `requirements`, `requirement_deps`, and
`requirement_chats` (current-session map + hidden set in one table). See [design.md](design.md)
for the cross-runtime driver adapter and migration handling.
