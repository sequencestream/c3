...
3. **Launch development** — turning a `todo` requirement into a background normal session that
   runs `/develop-pipeline`, with a back-link to that development session.
...
(background `/develop-pipeline`); development back-link; the `draft→todo→in_progress→done/cancelled`
...
| Entity                 | Description                                                                                                                       |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| Requirement            | A ledger item: `title`, `content`, `priority` (P0–P3), `status`, `dependsOn[]`, `lastDevSessionId`, scoped to a `projectPath`     |
...
| RM-R2  | The communication agent is **read-only**. It may use read-class tools (Read/Grep/Glob/WebFetch/WebSearch, auto-allowed) but **never** edits, writes, runs commands, spawns sub-agents, or runs slash commands. This is enforced at the tool layer, not by prompt alone (ADR 0007).                                                                                      |
...
- **US-3 Read-only refinement chat.** The right pane is an independent agent session with its own
  ...
  slash attempt (RM-R2). It proposes discrete, verifiable, right-sized items for confirmation. It
  does not appear in the normal session list (RM-R4).
...
- **US-6 Launch development.** A `todo` item has a Launch-development button; clicking creates a
  background session running `/develop-pipeline` with the requirement content, moves it to
...
the save confirmation is `permission_request` / `permission_response`
(`toolName = mcp__c3__save_requirements`); the development back-link is `select_session`. See the
...
- **permission-gateway** — gates `save_requirements` through the existing `canUseTool` flow; the
  same flow denies any non-read tool by default for the communication agent (ADR 0007).
