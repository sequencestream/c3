...
- **Automation orchestrator.** A per-project, in-memory background loop. **Eligibility:** a
  ...
  **resuming** its `lastDevSessionId` when that dev session still exists on disk (continuing the
  half-built context), and otherwise starting a **fresh** `/sdd-lite` session — a `todo` item or a
  dangling session, the same dangling rule as manual launch (RM-R8). **Completion judge:** because `/sdd-lite`
...
| RM-A3 | The orchestrator develops eligible requirements **one at a time**, ordered **priority (P0→P3) then oldest-first**. Eligible = `automate` AND `status ∈ {todo, in_progress}` AND every known `dependsOn` item is `done`. An unknown dependency id (cross-project/deleted) does not block. An `in_progress` requirement whose `lastDevSessionId` **still exists on disk** is **resumed** (its half-built `/sdd-lite` context is continued via `runDevTurn` with the real id, first prompt "继续"); a `todo` item or a **dangling** one (empty/deleted `lastDevSessionId`) starts a **fresh** `/sdd-lite` session — the same dangling rule as manual `start_development` (RM-R8). |
