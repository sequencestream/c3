...
| SND-R15 | When sandbox is enabled, exactly **one custom agent is picked at random** from the normalized `agentIds` pool and **pinned** onto the (pending) run; its vendor decides the container binary and its provider env flows into the env-file. This iteration supports **claude-vendor only** (ADR-0024). |
| SND-R16 | **Hard isolation (deny-by-default):** an empty pool, a deleted or non-claude pick, or a container start failure **hard-fails the run** (`turn_end` error + `finalizeRun` + `run:settled` error). There is no fallback to a bare host run (ADR-0024).                                                  |
...
2. **Pick** — `pickSandboxAgent(agentIds, resolveAgent)` (pure, `kernel/run/sandbox-agent.ts`) randomly selects one pool agent. Rejections (`empty-pool` / `unavailable` / `unsupported-vendor`) each hard-fail the run.
3. **Pin** — `setSessionAgent(runId, pickedId)` pins the pick onto the pending dev session, so every downstream `resolveSessionLaunch(runId)` (the vendor fork, the degradation chain, the SDK launch) resolves to it and its provider env (`ANTHROPIC_*`) reaches the container env-file via the unchanged claude wrapper path.
...
> Non-claude (codex/opencode) agents in the pool hard-fail in this iteration — their provider plumbing (codex relay / SDK-constructor `baseUrl`/`apiKey`) does not yet translate into the container env-file. Multi-vendor container provider wiring is a follow-up.
