...
| SND-R15 | When sandbox is enabled, exactly **one custom agent is picked at random** from the normalized `agentIds` pool and **pinned** onto the (pending) run; its vendor decides the container binary and its provider connection flows into the container. Supported picks: **claude** (provider env via the claude wrapper) and **codex DIRECT** (`wireApi=responses`) — for codex DIRECT, `baseUrl`/`model` ride the wrapper's `"$@"` argv (SDK emits `--config openai_base_url` / `--model`) and `apiKey` is mirrored into the env-file as `CODEX_API_KEY` (`codexDirectSandboxEnv`, ADR-0024 follow-up 2026-06-13). |
| SND-R16 | **Hard isolation (deny-by-default):** an empty pool, a deleted pick (`unavailable`), an unsupported vendor (`unsupported-vendor` — opencode), an unsupported codex wire (`unsupported-wire` — codex `chat`/relay or system-login codex), or a container start failure **hard-fails the run** (`turn_end` error + `finalizeRun` + `run:settled` error). There is no fallback to a bare host run (ADR-0024).                                                                                                                                                                                                      |
...
> Supported in the container today: **claude** and **codex DIRECT** (`wireApi=responses`, ADR-0024 follow-up 2026-06-13). Still unsupported (each hard-fails on pick): **codex RELAY** (`wireApi=chat` — needs c3's in-process Responses→Chat relay bridged into the container), **system-login codex** (no injected creds in the container), and **opencode**. Bridging the relay / opencode provider plumbing into the container is a follow-up.
