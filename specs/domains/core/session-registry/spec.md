...
**workspace** is a project directory (the agent's working directory); a **session** is a Claude
conversation inside it, persisted by the agent vendor. The registry owns the c3-specific metadata
the vendor does not track — the workspace list, recent-access order, per-session permission mode, and the last
...
**Boundary:** it does not drive the agent run loop and does not own run lifetime (agent-session), and
...
| SR-R4  | Sessions within a workspace are listed from a session-metadata projection cache in c3.db (ADR-0013 amendment), newest (`last_modified`) first. The projection is a rebuildable cache — the native vendor stores remain the source of truth for existence, history, and title. Each listed session entry carries its owning `vendor` tag and an additive `state` field (`born`/`alive`/`stale`/`orphaned`/`ghost`). The wire `sessionId` stays the vendor-**native** id (the c3 namespace on the wire is a deferred ADR-0013 phase). An env flag can roll the listing back to the legacy claude-only enumeration path. The per-vendor enumeration accessors are the rebuild / lazy-validation source; Codex rows can be populated either at bind time or by rebuilding from local Codex transcript enumeration, stamped with `last_modified` per SR-R13.                                                                                                                                                                                                                                                                      |
| SR-R5  | Permission mode is **per session**, persisted, and defaults to the project's per-vendor default mode (which itself falls back to each vendor's default mode token). Changing the active session's mode (`set_mode`) affects only that session and is validated against the session's vendor mode catalog.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| SR-R6  | `create_session` makes a Pending Session the viewed session with empty history and the per-vendor **default mode** (falling back to the vendor's default mode token). It has a `pending:` id and is not yet on disk. It does **not** stop any other session's run.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
- **agent-session** — supplies the session working directory, per-session mode, and resume id to each
  run (via its runtime); receives the bound `sessionId` back when a run reports its session id.
- **Claude Agent SDK** — session enumeration, history retrieval, rename, and delete.
- **web-console** — renders the workspace/session tree and sends the management events above.

## Data dictionary

- **Workspace** — a registered working directory; the key sessions are enumerated under.
