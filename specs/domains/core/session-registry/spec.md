...
| SR-R6  | `create_session` makes a Pending Session the viewed session with empty history and `default` mode. It has a `pending:` id and is not yet on disk. It does **not** stop any other session's run.                                                                                                            |
...
| SR-R8  | `select_session` makes the session the viewed session and replays its full record: `session_selected.history` (on-disk baseline) plus the runtime's live buffer tail for an in-flight/background turn. It reports the session's stored mode and whether a turn is `running`. It does **not** stop any run. |
...
runtime `statuses`), `workspaces`, `sessions`, `session_selected` (with `running`),
