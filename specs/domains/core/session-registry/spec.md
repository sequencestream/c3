...
| SR-R4  | Sessions within a workspace are listed across every available vendor via the read-only `SessionAccessor` union (ADR-0013), newest (`lastModified`) first (global, cross-vendor). Each native vendor store is the source of truth for existence, history, and title; each listed `SessionInfo` carries its owning `vendor` tag. The wire `sessionId` stays the vendor-**native** id (the c3 namespace on the wire is a deferred ADR-0013 phase). An env flag (`C3_SESSION_LIST_ACCESSOR=0`) rolls the listing back to the legacy claude-only `listSessions({ dir })` path during the transition. Codex is **not** enumerable (its list entries depend on the projection table) — it is excluded from the listing union. |
