...
| AppHeader         | `components/AppHeader.vue`         | Top bar: hosts WorkspaceSwitcher (far left), session breadcrumb (`› title`), permission-mode dropdown, settings entry, connection status                                                                                                                                                                                                                                                                                                                                                                                 |
...
| SessionSidebar    | `components/SessionSidebar.vue`    | The current workspace's session list (no workspace tree); owns session pagination + prompt/confirm UX, emits session CRUD intents                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `currentWorkspace`    | `string \| null`                        | The single global current workspace path (WC-R8); resolved via `resolveCurrentWorkspace`, persisted to `localStorage`, drives the sidebar (`currentSessions` = its `sessionsByWorkspace` slice). Decoupled from `activeWorkspace` (the viewed session's workspace) |
...
| `selectWorkspace(path)`                        | path ≠ current (WC-R8)                                                                   | nothing on switch itself — sets `currentWorkspace` + persists + `list_sessions` if unloaded |
