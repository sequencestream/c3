...
| WorkspaceSettingPage | `pages/workspacesetting/WorkspaceSetting.vue` | Workspace setting full-page overlay: edits 7 per-project controls (defaultMode, devSkill, maxRoundsPerStage, maxSpeechChars, consensus{enabled,majority}, gitBranchMode, defaultMainBranch). Owns editable draft seeded from the `workspace_setting` server reply; a `detectedMainBranch` prop (the reply's server-probed default branch) pre-fills defaultMainBranch when no saved value exists. Emits `save` → App sends `save_workspace_setting`. Entry button in AppHeader after WorkspaceSwitcher, disabled without a workspace. Follows the same draft-editing pattern as SettingsPanel.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `mode`                    | `ModeToken`                                  | Current vendor-native mode token; synced from `session_selected`/`mode_changed` (WC-R4). Interpreted via the active session's vendor catalog.                                                                                                                                                     |
| `modeOptions`             | computed `{value:ModeToken,label:string}[]`  | Options for the mode dropdown, derived from the active session's vendor `VendorModeCatalog` (from `settings.vendorModes`). Falls back to the built-in Claude mode list when the vendor catalog is not yet loaded.                                                                                 |
| `vendorModes`             | `Record<VendorId,VendorModeCatalog> \| null` | Per-vendor mode catalogs (2026-06-07-012), seeded from `settings.vendorModes`. Drives per-vendor mode options in the picker.                                                                                                                                                                      |
...
| `intents`                 | `Record<workspacePath, Intent[]>`            | Per-project intent lists; updated on `intents` push or `list_intents` reply                                                                                                                                                                                                                       |
| `automation`              | `Record<workspacePath, AutomationStatus>`    | Per-project automation-orchestrator status; updated on `automation_status` push                                                                                                                                                                                                                   |
...
| `workspaceSettingOpen`    | `boolean`                                    | Whether the WorkspaceSettingPage overlay is open; toggled by the AppHeader workspace-setting button. Closed on workspace switch and WS reconnect                                                                                                                                                  |
| `currentWorkspaceSetting` | `WorkspaceSetting \| null`                   | The last `workspace_setting` reply from the server, seeded into WorkspaceSettingPage's draft. Cleared on workspace switch and WS reconnect                                                                                                                                                        |
| `detectedMainBranch`      | `string \| null`                             | The server-probed default branch carried on the `workspace_setting` reply; passed to WorkspaceSettingPage to pre-fill `defaultMainBranch` when no saved value exists. Cleared on workspace switch and WS reconnect                                                                                |
...
| `intents`                  | replace `intents[workspacePath]` with the pushed list (WC-R10)                                                                                               |
| `automation_status`        | replace `automation[workspacePath]` with the pushed orchestrator status (WC-R11)                                                                             |
| `workspace_setting`        | set `currentWorkspaceSetting` to the returned config and `detectedMainBranch` to the reply's probed branch; consumed by WorkspaceSettingPage's draft watcher |
...
| `openWorkspaceSetting()`                       | workspace selected                                                                                    | opens WorkspaceSettingPage overlay; sends `load_workspace_setting` for the current workspace                                                                                                                                                               |
| `saveWorkspaceSetting(config)`                 | workspace selected                                                                                    | sends `save_workspace_setting` with the project path and edited config; closes WorkspaceSettingPage                                                                                                                                                        |
