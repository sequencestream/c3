...
| `toolAgentId`       | text                    | Id of the agent that runs background tool sessions (completion judge, name derivation; exception-handling not yet agent-driven). Empty string ⇒ "follow the default agent" (kept empty on store); a set value falls through by `order_seq` like the default (AC-R21).        |
| `intentAgentId`     | text                    | Id of the agent that runs intent-communication sessions (the intent analyst's requirement-breakdown conversation). Empty string ⇒ "follow the default agent" (kept empty on store); a set value falls through by `order_seq` like the default (AC-R23).                      |
