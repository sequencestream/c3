...
consensus `askAgentOnce` / `launchForAgent` paradigm but runs **no** consensus and **no** agent
...
1. **Auto-start.** On research success the server **auto-starts** orchestration
   (`startDiscussionRun`, equivalent to an automatic `start_discussion`), re-validating on the
   freshest record via `canAutoStartDiscussion` (skips if the human Started/cancelled mid-research).
...
2. **Organizer engine.** A background loop walks `draft → in_progress → completed` over the type's
   `discuss → summarize → confirm → conclude` workflow (`shared/src/discussion-types.ts`). The
   organizer nominates speakers among the discussion's **selected participants**
   (`Discussion.participantAgentIds`, chosen in the create modal, resolved against `enabledAgents()`,
   ∪ the always-included organizer; empty ⇒ legacy fallback to the whole pool) — a **heterogeneous,
   multi-vendor roundtable** (each `agent` bubble carries a vendor tag derived from its config;
   `AC-R10` gates the pool, `AC-R12` the vendor). Each turn is a one-shot `askAgentOnce`, appended
...
- **Pause is a round-boundary effect.** An already in-flight `askAgentOnce` finishes, so one more
