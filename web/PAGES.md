...
│   ├── dashboard-actions.ts                        # 工作台 Dashboard 动作:setWorkcenterPage(切页并加载对应页)、loadDashboard(发 get_workspace_dashboard,单请求在途时只记一次待刷新做合并去重)、maybeRefreshDashboard(仅 Dashboard 激活时刷新,供领域广播钩子)、toggle/toggleAll 选择、setWorkspacesAutomation(管理员+≥1选中+无在途时发 set_workspaces_automation_enabled);回包在 message-handler 落库(workspace_dashboard 采用/保留旧数据+错误、按现存 id 收敛选择;workspaces_automation_result 结算逐项失败标识、失败行保留选中、以服务端快照校准、成功/失败/部分成功 toast)
...
│   ├── workcenter/                                  # 工作台页(顶层 view-mode;页内导航 Dashboard / 用户通知,App.vue 持有 workcenterPage 态并渲染)
...
│   │       └── WorkspaceDashboard.vue              # 「Dashboard」页(纯展示):全 workspace 运行总览表——每行 workspace 名/绝对路径 + 运行中 session 数/session 总数/intent 总数/讨论总数/自动化总数 + 自动化总闸状态徽标(on/off);管理员可勾选多行(全选 checkbox + 逐行 checkbox)并统一「开启/关闭自动化」(无选择或在途时禁用),非管理员只读(不渲染勾选/批量控件,仍见总闸状态);失败行 dash-row-failed 高亮 + 失败标识,刷新失败显示 banner + 重试;窄屏(≤720px)表格降级为逐行 data-label 卡片。经 props 接 rows/loading/refreshFailed/selected/failedIds/busy/isAdmin,emit toggle-workspace/toggle-all/bulk/refresh
...
│   │       ├── AutomationForm/AutomationForm.vue        # 创建/编辑任务表单(弹窗):cron 或事件触发、高级 cron 构造器、实时 next-run 预览;事件触发下是通用过滤——事件类型输入框(datalist 建议 run:started/run:settled/pr:operation/intent:lifecycle,接受任意字符串)+ statuses 多值行 + metadata 条件构建器(增删行+AND/OR),仅当类型为 run:started/run:settled 时额外显示必填 sessionKind 多选(无默认勾选,未选禁止保存),pr:operation 类型显示 MCP 集成说明;通用 metadata 标注编辑器(增删 key/value);编辑态可改标题(清空回退自动命名),创建态自动命名;vendor 下拉(host 缺失灰显)+工具勾选面板(读写分区,读默认勾,全选/全清按钮);创建态用系统 automationAgentId 预填 vendor+agent(跟随链 automationAgentId→defaultAgentId→首个启用),编辑态保留记录自身 vendor/agentId;移动端全屏 sheet 且紧凑表单单列堆叠
