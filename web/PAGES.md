...
│   ├── session-actions.ts                          # 工作区/会话/顶栏 tab 导航:按 session_kind 缓存的游标分页刷新(窗口/首页)/加载更多;selectSession 任意行统一 enterConsole+select_session 在右侧展示详情(无跳走分支),已是活动会话时不重复发送;openSourceTarget 单一路径按 resolveSessionJumpTarget 目标打开意图/spec tab/意图会话(有 owner 选意图、无 owner 的独立 chat 经 requestedIntentSessionId 在意图页右栏打开该会话)/讨论/定时任务页,供 jumpSessionSource(行 ↗,传 row)与 jumpActiveSessionSource(标题栏溯源按钮,读 activeSessionSource+活动会话)复用;六类会话计数、新建工作会话弹窗、乐观删除改名、会话 tab 进入与重绑、清空视图会话
