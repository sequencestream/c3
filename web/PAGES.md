...
│   │   │   │   ├── IntentDetailTabs.vue             # Tab 导航条:渲染可见 Tab、高亮激活项,意图/工作会话 Tab 标签内联运行中状态点(复用全局 .session-status,非 idle/未知才显示),select 事件上抛
...
│   │   │   │   ├── useIntentDetailTabs.ts          # Tab 状态机 composable:activeTab/可见 Tab(SDD 或历史 spec 数据决定 spec/specSession 显隐、lastWorkSessionId 决定 workSession)/空正文默认进意图会话/切意图复位/隐藏 Tab 回退 intent/外部 requestedSubTab 覆盖并恰好消费一次(不可见目标静默忽略仍消费)/changelog·spec 懒加载信号/三类会话 ID 异步回填补发一次 open 且对齐去重/expectedSessionId·chatReady·firstIntentTurn·modeLocked/pendingSpecSwitch 待新会话回填自动切;只返回声明式结果,打开/读取/日志经回调交回控制层
