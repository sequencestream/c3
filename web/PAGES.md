...
│   ├── message-handler.ts                          # installMessageHandler():唯一入站 WS switch(handleMessage)折叠所有 ServerToClient 事件 + applyStatuses/notifyAwaitingPermission;session_selected 的 owner 元数据派生会话标题栏跳回目标;session_counts 仅接受当前 workspace 的响应(丢弃切换后到达的旧包),session_status 的运行集合变化触发一次 get_session_counts 重取,顶部角标据此无刷新收敛
...
│   ├── queue-actions.ts                            # installQueueActions():自动化队列页动作。openQueuePage/closeQueuePage 切换意图页与队列页(queuePageOpen),打开时发一次 get_queue_detail;refreshQueueDetail 手动重取;queueControl(action, intentId?) 原样发 queue_control(pause/resume/force_skip/unskip/unpark/override_continue/override_block),客户端不预测结果——后续刷新一律由服务端在每轮对账与每次人工控制后推送的 queue_detail 驱动,被拒绝的控制走既有 error 通道弹框,不会看起来像成功
...
│   ├── settings-actions.ts                         # 系统/工作区设置、技能安装、运行时语言切换(回滚)、workspace↔workcenter 视图模式、技能加载审批、工作区设置页只读观测(loadParkRecoveryStats:按当前工作区拉 park 恢复率,开页时随设置一起发出、失败态可重试;与设置保存完全分离);另含 loadParkRecoveryStats():工作区设置页「本机观测」区块的只读取数(打开页面时与 load_workspace_setting 并列各发一次、重试按钮复用),发出前清掉上次错误,与设置草稿/保存完全无关
...
│   ├── dashboard-actions.ts                        # 工作台 Dashboard 动作:setWorkcenterPage(切页并加载对应页)、loadDashboard(发 get_workspace_dashboard,单请求在途时只记一次待刷新做合并去重)、maybeRefreshDashboard(仅 Dashboard 激活时刷新,供领域广播钩子)、toggleWorkspaceAutomation(管理员+该行无在途时,以单元素 workspaceIds 发 set_workspaces_automation_enabled,并把该行标记为在途 pending);回包在 message-handler 落库(workspace_dashboard 采用/保留旧数据+错误、按现存 id 收敛 pending;workspaces_automation_result 仅清除本次结算行的 pending、以服务端快照校准、仅失败时 toast)
...
│   │   │   ├── IntentMergedList/IntentMergedList.vue # 意图列表左栏容器:接管 IntentList 头区;可折叠(960px/480px);透传 selectedIntentId 高亮与 select-intent 选中事件;头区右域为自动化按钮+「队列」入口(emit open-queue,进队列页)+状态过滤(移动端收进 ⋮ 菜单,桌面直显)+「+」增加意图(经 create_intent 轻量创建空 draft);无分段控件、无意图会话列表(已挪到会话页「意图」tab)
│   │   │   ├── IntentList/IntentList.vue            # 需求列表:接受 hideHeader prop 嵌入合并栏;按状态过滤、终止态分页、自动化编排启停(移动端头区收进 ⋮ 菜单,桌面直显);行点击=选中(emit select-intent,selectedId 高亮并滚动已渲染选中行入视野);行头状态区 .req-status 状态徽标旁并列:.req-run-status(仅 in_progress 的 running/dangling 文字标签,由 runStatus 派生)与一个仅读 sessionActive 的无文案闪动绿点 .req-session-active(意图/spec/工作会话任一此刻有 turn 在跑即显示,复用 status-pulse 节奏,不受 status/runStatus 限制,可与状态徽标及 running/dangling 标签共存);行右侧有两个轻量 icon 控件——模式 icon(⏳自动/✋手动,即点即切 emit set-automate(id,!automate))与仅 todo 行可见的 refine 编辑入口(✎,emit refine(id)),均 @click.stop 不误触选中;复用 IntentDetail 既有 set-automate/refine 事件流,不新增后端写入路径
│   │   │   ├── IntentDetail/                         # 右栏意图详情面板(容器 + 子单元):god-component 已按职责拆分
│   │   │   │   ├── IntentDetail.vue                 # 详情页薄容器:装配 Tab 状态机与编写Spec门/延迟切Tab组合逻辑,编排四态主按钮(SDD×specPath×specApproved:关→start-dev/开无spec→write-spec/有spec未批准→只切开 spec tab/已批准→start-dev,含 in-flight 守卫)与会话重置弹框(intent/spec 分流,spec 提交记 pendingSpecSwitch 待新会话回填自动切),常驻头部标题(标题+模块+优先级+状态徽标)后把动作/进度/Tab导航/内容区/会话面板分派给子单元;对 Intents.vue 的 props/emits/prefill(经 defineExpose 转发会话面板)/__resetWriteSpecGuards 保持兼容;无选中(列表空)时空态
│   │   │   │   ├── IntentTitleBarActions.vue        # 标题栏动作区(所有 tab 恒可见):状态切换(markTodo/backToDraft/markDone(仅 lastWorkSessionId 存在)/cancel)+四态主按钮(语义/禁用/标题经 props,点击 emit main-action 交回容器)+我要修改(无 lastWorkSessionId 显示,emit modify)+create-pr(仅 worktree、lastWorkSessionId 存在、prId 空、branchName 存在且异于主分支)+pr-link(有 prUrl 为跳转锚点,否则回退复制 prId)+sync(done+reviewing+PR,防重)+share(🔗,data-testid=share-button)+automate 切换+删除(仅 status !== done 渲染);删除二次确认 ConfirmDialog 及 in_progress 工作产物强化提示归本组件所有并防双发
│   │   │   │   ├── IntentEngineeringProgress.vue    # 只读工程进度条(意图→[规范]→工作→[PR]及未开始/进行中/已完成/已关闭失败文本态):状态派生唯一以 intent-engineering-progress 纯函数为规则源,组件只做阶段/状态本地化+aria-label+窄屏横向适配;SDD 关闭省略规范段,仅 worktree 模式追加 PR 段(工作完成与 PR 状态独立)
│   │   │   │   ├── IntentDetailTabs.vue             # Tab 导航条:渲染可见 Tab、高亮激活项,意图/规范/工作会话 Tab 标签内联运行中状态点(复用全局 .session-status,非 idle/未知才显示),select 事件上抛
│   │   │   │   ├── IntentOverviewTab.vue           # intent tab:元信息(ID→分支+commit→PR+状态/同步→已创建→已完成→已更新→依赖,顺序稳定)+正文 markdown/直接编辑(仅 draft/todo 显示「编辑」,textarea 预填 content+框下左侧蓝色保存/取消,保存 emit save-intent-content 透传 update_intent_content、服务端回填 updatedAt 变化退出编辑态、被拒 intentActionErrorSeq 自增释放守卫留草稿、切意图丢草稿)+依赖逐行完成态/类型(标题 emit select-dependency,单条类型编辑整组 emit update-deps,弹窗内删除单条依赖:ConfirmDialog 危险二次确认渲染于编辑 overlay 子树内盖住编辑框,确认剔除该项整组 emit update-deps 剩余集并关弹窗,取消/遮罩/Esc 只关确认层留编辑态);元信息 PR 行 sync 按钮(done+reviewing)
│   │   │   │   ├── IntentSpecTab.vue               # spec tab:顶部批准/编辑/我要修改操作区(批准/我要修改可见性经 props 由容器算四态主按钮+10秒防误审门+依赖门)+渲染 specPath 指向 spec.md(经 read-spec 拉 intentSpecContent);「批准」emit approve-spec、「我要修改」emit modify 交回容器;「编辑」仅三门禁(specPath+status=todo 且无 lastWorkSessionId+specSessionRunning 为假)全满足显示,textarea 预填 intentSpecContent+保存 emit save-spec-content 透传 update_spec_content、服务端回填 updatedAt 变化退出编辑并重发 read-spec、被拒释放守卫留草稿;已批准时额外渲染「撤销批准」(人工/机器批准共用一个入口)emit revoke-spec-approval;正文上方渲染审核事实带(结论徽标 pass/changes_requested + 返工轮次 + 审核理由全文 + 批准身份,机器批准按机器身份文案展示、绝不把保留常量当用户名直出);编辑态隐藏批准/撤销/我要修改与审核事实带
│   │   │   │   ├── IntentChangelogTab.vue          # changelog tab:生命周期变更日志倒序列表(操作类型本地化标签+摘要+操作人+时间),懒加载/空态/加载态文案由容器透传数据决定
│   │   │   │   ├── IntentSessionPanel.vue          # 三类会话 tab(intent/spec/work session)内容面板:复用 ChatColumn;单一活动会话模型——期望 id 与 activeSession 对齐(chatReady)才渲染防串台,意图会话首轮(firstIntentTurn,期望 id 空)渲染首条输入并以 intentTitle 为题、submit 交回容器分流 start-intent-session;标题栏 show-mode=true,modeLocked(非 workSession)只读、work session 可切模式;打开会话本身由容器补发
│   │   │   │   ├── useIntentDetailTabs.ts          # Tab 状态机 composable:activeTab/可见 Tab(SDD 或历史 spec 数据决定 spec/specSession 显隐、lastWorkSessionId 决定 workSession)/空正文默认进意图会话/切意图复位/隐藏 Tab 回退 intent/外部 requestedSubTab 覆盖并恰好消费一次(不可见目标静默忽略仍消费)/changelog·spec 懒加载信号/三类会话 ID 异步回填补发一次 open 且对齐去重/意图·规范·工作三类会话标签状态点(非 idle/未知才显示,仅呈现不参与 spec 编辑门禁)/expectedSessionId·chatReady·firstIntentTurn·modeLocked/pendingSpecSwitch 待新会话回填自动切;只返回声明式结果,打开/读取/日志经回调交回控制层
