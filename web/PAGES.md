...
│   ├── session-actions.ts                          # 工作区/会话/顶栏 tab 导航:按 session_kind 缓存的游标分页刷新(窗口/首页)/加载更多;selectSession 任意行统一 enterConsole+select_session 在右侧展示详情(无跳走分支),已是活动会话时不重复发送;openSourceTarget 单一路径按 resolveSessionJumpTarget 目标打开意图/spec tab/意图会话/讨论/定时任务页,供 jumpSessionSource(行 ↗,传 row)与 jumpActiveSessionSource(标题栏溯源按钮,读 activeSessionSource+活动会话)复用;六类会话计数、新建工作会话弹窗、乐观删除改名、会话 tab 进入与重绑、清空视图会话
...
│   │
│   ├── intents/                                     # 需求页
│   │   ├── Intents.vue                              # 需求容器页:桌面两栏(左意图列表 + 右栏双态);右栏默认展示选中意图的 IntentDetail(首次默认选首条,聊天列 props/活动会话经其透传给两会话 tab),点左栏标题栏「+」(handleNewIntentSession)切到独立意图会话 ChatColumn(viewingNewIntentSession=true,绑定服务端 session_selected 的活动会话),点任一意图行复位回 IntentDetail;requestedIntentId(work session 标题栏跳转来的一次性外部选中请求)命中已加载意图时改选并 emit requested-intent-consumed 让父清空,目标未出现则保留默认选中;prefill 经 defineExpose 按双态路由到 ChatColumn 或 IntentDetail;移动端 MobileStack 二级 drill-down(列表→详情/会话)
│   │   ├── components/
