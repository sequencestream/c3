...
│
├── components/                                      # 跨页面通用组件
│   ├── AppHeader/AppHeader.vue                      # 应用导航壳:桌面顶部栏与移动端底栏共用 HEADER_TABS;基础顺序=需求/讨论/自动化/代码,服务端 showSessionsPage=true 时才在代码后追加会话(含六类运行中数角标),未知/关闭时两端均不渲染会话入口。整行最左为 viewMode 工作区/工作台两图标切换器;工作台模式显示总览/用户通知入口及待处理角标;右侧保留项目配置/系统设置/账户/连接状态、升级提示与许可状态菜单;移动顶部使用同一模式切换和工作区/工作台入口
