...
│   │       └── AutomationForm/AutomationForm.vue        # 创建/编辑任务表单(弹窗):cron 或事件触发(run:started/run:settled/pr:operation/intent:lifecycle)、高级 cron 构造器、实时 next-run 预览;run:settled 显示 reason 过滤,run-lifecycle 显示 sessionKind 多选(无默认勾选,未选禁止保存)+metadata 条件构建器(增删行+AND/OR),pr:operation 显示 MCP 集成说明+操作/结果过滤面板;通用 metadata 标注编辑器(增删 key/value);编辑态可改标题(清空回退自动命名),创建态自动命名;vendor 下拉(host 缺失灰显)+工具勾选面板(读写分区,读默认勾,全选/全清按钮);移动端全屏 sheet 且紧凑表单单列堆叠
│   │
│   ├── codes/                                       # 代码浏览页
