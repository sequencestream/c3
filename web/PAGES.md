...
│   ├── SpecStartupOverlay/SpecStartupOverlay.vue    # Spec 会话启动遮罩(App 根级):撰写/重置 Spec 点击即阻断,展示检查依赖/拉取代码/启动会话的粗粒度进度;就绪、动作失败或安全超时后收敛关闭
...
│   │   │   ├── IntentList/IntentList.vue            # 需求列表:接受 hideHeader prop 嵌入合并栏;按状态过滤、终止态分页、自动化编排启停;行点击=选中(emit select-intent,selectedId 高亮并滚动已渲染选中行入视野),不再行内展开/行内操作(详情与操作迁至右栏 IntentDetail)
