 ...
 *
 * 仅服务于需求详情页「Start Work」触发的 start_development 启动等待:点击后立即以全屏
...
const STEP_LABEL_KEY: Record<DevLaunchStep, Parameters<typeof t>[0]> = {
  'fetch-base-branch': 'intent.devLaunch.step.fetchBaseBranch',
...
const steps = computed<{ key: DevLaunchStep; label: string; status: StepStatus }[]>(() => {
  const phase = props.model?.phase ?? 'fetching-base-branch'
