...
/*
 * automation-refresh — pure decision logic for live-refreshing a running automation
 * execution on the history page.
 *
 * The history page polls the selected, running execution: every interval it
 * re-fetches the automation detail (status / duration) and the execution
 ...
 */
export function isExecutionRunning(log: AutomationExecutionLog | null): boolean {
  if (!log) return false
  if (log.status) return log.status === 'running'
  return log.finishedAt === null
}

export interface AutomationRefreshInput {
  /** The selected execution is running (and is a refreshable llm session). */
  running: boolean
  /** The automations tab is the active view. */
  tabActive: boolean
...
export interface AutomationRefreshDecision {
 ...
 */
export function decideAutomationRefresh(input: AutomationRefreshInput): AutomationRefreshDecision {
