 ...
 * URL 契约:`<baseUrl>/#/<kind>/<workspaceName>/<id>`(与 parseDeepLink 的三段式一致)。
 */
...
export function buildShareText(
  ...
  if (!base) return null
  const workspaceName = encodeURIComponent(input.workspaceName)
  const id = encodeURIComponent(input.id)
  const url = `${base}/#/${input.kind}/${workspaceName}/${id}`
