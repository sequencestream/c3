...
// 与 AutomationForm.renderDow 同规则:`1-5` 压缩,其余升序逗号拼接。
...
watch(
  () =>
    [
      props.open,
      props.automation?.id,
      props.cronExpression ?? props.automation?.cronExpression,
    ] as const,
  ([open, , cronExpression]) => {
...
<template>
  <div v-if="open" class="sce-overlay" @click.self="emit('close')">
    <section
      ...
      role="dialog"
      aria-modal="true"
      :aria-label="t('automation.form.automation.label')"
    >
      <header class="sce-head">
        <h3>{{ t('automation.form.automation.label') }}</h3>
        <button
          type="button"
          class="sce-close"
      ...
      <div class="sce-body">
        <label class="sce-field">
          <span>{{ t('automation.form.frequency.label') }}</span>
        ...
        <div v-if="frequency === 'weekly'" class="sce-field sce-field--stacked">
          ...
          <span v-if="daysInvalid" class="sce-warn">{{ t('automation.form.days.required') }}</span>
        </div>
...
.sce-overlay {
  position: fixed;
  inset: 0;
  /* Must cover the full automation-edit modal (z-index: 200) when opened from it. */
  z-index: 400;
  display: grid;
  place-items: center;
  padding: var(--sp-4);
