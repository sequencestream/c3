...
/**
 * ExecutionDetail.vue — automations 视图右栏:选中执行的 Tab 化详情面板。
 *
 * Tabs:
 ...
 *
 * 数据流:execution 由 App.vue 经 Automations.vue 传入;transcripts 与 load-session
 * 的沿用路径不变,只是渲染从旧 inline 方式改为复用 ChatMessages 组件。
 */
import { computed, ref, watch, onMounted, onUnmounted } from 'vue'
import type { AutomationExecutionLog, TranscriptItem } from '@ccc/shared/protocol'
...
const tabs = computed(() => {
  if (props.executionType === 'command') {
    return [
      { id: TAB_EXEC_INFO, label: t('automation.execution.tab.execInfo') },
      { id: TAB_COMMAND_LOG, label: t('automation.execution.tab.commandLog') },
    ]
  }
  // llm: 执行信息 + Session 会话记录
  return [
    { id: TAB_EXEC_INFO, label: t('automation.execution.tab.execInfo') },
    { id: TAB_SESSION, label: t('automation.execution.tab.session') },
  ]
})
...
function fmtDuration(log: AutomationExecutionLog): string {
...
function logStatus(log: AutomationExecutionLog): string {
  ...
  <div class="exec-detail-wrap">
    ...
    <template v-if="!execution">
      ...
      <div class="exec-detail-empty">
        <p>{{ t('automation.execution.empty') }}</p>
      </div>
    </template>
    ...
    <template v-else>
      ...
      <div v-if="activeTab === TAB_EXEC_INFO" class="exec-detail-body">
        <div class="exec-info-grid">
          ...
          <div class="exec-info-row">
            <span class="exec-info-label">{{ t('automation.detail.history.label') }}</span>
          ...
          <!-- Started -->
          <div class="exec-info-row">
            <span class="exec-info-label">{{ t('automation.detail.started.label') }}</span>
            <span class="exec-info-val">{{ fmtDate(execution.startedAt) }}</span>
          </div>
          <!-- Finished -->
          <div class="exec-info-row">
            <span class="exec-info-label">{{ t('automation.detail.finished.label') }}</span>
            <span class="exec-info-val">{{
              execution.finishedAt === null
                ? t('automation.detail.running')
                : fmtDate(execution.finishedAt)
            }}</span>
          </div>
          <!-- Duration -->
          <div class="exec-info-row">
            <span class="exec-info-label">{{ t('automation.execution.info.duration') }}</span>
            <span class="exec-info-val">{{ fmtDuration(execution) }}</span>
          </div>
          <!-- Exit code -->
          <div class="exec-info-row">
            <span class="exec-info-label">{{ t('automation.execution.info.exitCode') }}</span>
            <span class="exec-info-val">{{
        ...
        <div v-if="execution.output" class="exec-info-section">
          <h4 class="exec-info-section-title">{{ t('automation.execution.info.output') }}</h4>
          <pre class="exec-output-text">{{ execution.output }}</pre>
        </div>

        <!-- Error -->
        <div v-if="execution.error" class="exec-info-section">
          <h4 class="exec-info-section-title">{{ t('automation.execution.info.error') }}</h4>
      ...
      <div v-if="activeTab === TAB_SESSION" class="exec-detail-body">
        ...
        <p v-if="transcriptLoading" class="exec-section-empty">
          {{ t('automation.execution.session.loading') }}
        </p>
        <!-- 无记录 -->
        <p v-else-if="transcriptEmpty" class="exec-section-empty">
          {{ t('automation.execution.session.noRecord') }}
        </p>
      ...
      <div v-if="activeTab === TAB_COMMAND_LOG" class="exec-detail-body">
        ...
        <p v-else class="exec-section-empty">{{ t('automation.execution.commandLog.empty') }}</p>
