 ...
 *
 * 历史 Tab 不再常驻执行列表,改由本弹框选择要查看的一次执行:在选中 automation 的
...
<template>
  ...
  >
    <div class="ehd-modal" role="dialog" aria-modal="true">
      <div class="ehd-head">
        <span class="ehd-title">{{ t('automation.history.dialog.title') }}</span>
        <button
          type="button"
          class="ehd-close"
      ...
      <div class="ehd-body">
        <p v-if="logs.length === 0" class="ehd-empty">{{ t('automation.detail.noHistory') }}</p>
        <template v-else>
          <div
          ...
          >
            ...
            <div class="ehd-item-meta">
              <span class="ehd-item-duration">{{
                t('automation.detail.duration', { duration: fmtDuration(log) })
              }}</span>
              <span v-if="log.exitCode !== null" class="ehd-item-exit">{{
                t('automation.detail.exit', { code: log.exitCode })
              }}</span>
            </div>
          </div>
        </template>
      ...
      <div v-if="logs.length > 0" class="ehd-pager">
        <button type="button" class="ehd-page-btn" :disabled="page === 0" @click="prevPage">
          {{ t('automation.history.dialog.prev') }}
        </button>
        <span class="ehd-page-indicator" data-testid="history-dialog-page">{{
          t('automation.history.dialog.page', { current: page + 1, total: pageCount })
        }}</span>
        <button
          type="button"
          class="ehd-page-btn"
          ...
          @click="nextPage"
        >
          {{ t('automation.history.dialog.next') }}
        </button>
      </div>
    </div>
  </div>
