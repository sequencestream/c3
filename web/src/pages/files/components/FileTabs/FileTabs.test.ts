...
/*
 * FileTabs 的 Markdown 视图模式记忆:按 path 独立、跨 tab 切换保留、关闭即遗忘。
 *
 * 视图模式是纯前端内存态(容器持有的 Map),不落库、不进协议。FileView 用
 * :key="path" 逐 tab 重挂载,故状态必须由 FileTabs 持有并透传。此处以 FileView
...
function mdTab(path: string): FileTab {
  const content = `# ${path}\n`
  return {
    path,
    loading: false,
...
function activeViewMode(wrapper: ReturnType<typeof mount>): string {
  return wrapper.getComponent(FileView).props('viewMode') as string
}

// 让激活 tab 内的原文按钮触发 FileView 的 update:viewMode,模拟用户点「原文」。
async function selectSource(wrapper: ReturnType<typeof mount>): Promise<void> {
  await wrapper.findAll('.file-view-btn')[0].trigger('click')
}

describe('FileTabs Markdown 视图模式记忆', () => {
